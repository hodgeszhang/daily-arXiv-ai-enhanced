...
TL;DR: 该文提出TRACE——首个面向火灾后物体理解的变换感知基准，包含21.4K合成场景与物体级"完好-退化"配对数据，并设计即插即用的特征恢复模块（FRM）以缓解严重退化下的性能骤降。


<details>
  <summary>Details</summary>
Motivation: 火灾后环境中的物体经历不可逆的物理变换，改变其几何、材质状态与视觉外观。检测并识别这些残留物对定位危险、重建事故前内容与清点损失至关重要。与常规图像损坏不同，此类退化作用于物体自身的物理结构。现有模型在此设定下性能随退化程度急剧下降，缺乏专门基准与方法。

Method: 构建TRACE基准：21.4K真实图像驱动的合成场景，覆盖499个物体身份、189个类别，提供物体级"完好—退化"渐进配对。定义五项任务：退化物体检测、完好状态恢复与检索、原始材质恢复、完好描述生成、功能推理。提出特征恢复模块（FRM）：即插即用，将退化编码器特征映射为与完好状态对齐的表示，宿主模型保持冻结，仅用配对特征监督训练。

Result: 现有模型随退化加剧性能显著下降：从最轻到最重等级，RF-DETR的mAP相对下降71%，InternVL3.5检索R@1由93.85降至28.11。FRM在场景级检测、CLIP/SigLIP2特征恢复及四项物体级VLM任务上均带来提升，退化越严重增益越大；跨VLM宿主与严重等级，检索相对增益平均12.5%、材质恢复20.1%、描述生成13.2%、功能推理12.4%。

Conclusion: TRACE填补了火灾后物体理解基准的空白，揭示了现有模型对物理性退化的脆弱性；FRM以冻结宿主、仅特征监督的低成本方式有效恢复完好对齐表示，具有即插即用与退化程度自适应的增益特性。
...
TL;DR: 本文提出"读取时记忆替换"方法，通过替换被消费的记忆值，将"记忆带来的收益"与"收益对具体检索内容的依赖"区分开来，并在多个视频世界模型与分割模型上验证了记忆增益来源的多样性。


<details>
  <summary>Details</summary>
Motivation: 现有记忆消融实验只能检验记忆是否有帮助，无法判断增益是否真正来自被检索到的正确内容。作者希望直接分离"记忆收益"与"记忆特异性"，即增益在多大程度上依赖于检索到的具体内容。

Method: 提出读取时记忆替换（read-time memory substitution）：在保持其余计算不变的前提下，替换被消费的记忆值，并设置身份无关的对照内容、同轨迹错误记忆、来自不相交轨迹与人种的供体记忆等条件，同时设计剂量响应实验（控制替换值偏离训练记忆表示的幅度，如从102%到1%）。

Result: 冻结视频世界模型中，无身份对照在Ego-Exo4D与7-Scenes几乎恢复全部收益、TUM约70%；剂量响应显示恢复率随偏离训练表示从102%降至1%，支持"表示修复"解释。WorldMem呈梯度依赖：同轨迹错误记忆恢复94.1% PSNR收益，不相交轨迹供体恢复43.7%。SAM 2呈强内容依赖：DAVIS上均值区域与边界分数从0.926降至0.182，MOSEv2重现处从0.459降至0.000。

Conclusion: 记忆增益可能依赖通用表示支持、更广上下文或精确的情节内容，三者需加以区分；读取时替换为直接判别这三类来源提供了有效手段。
...
TL;DR: 提出统一点云运动生成框架UniMo与大规模人兽运动-语言数据集UniML3D，解决动物骨骼拓扑多样与数据不足问题，在多个基准上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 将文本驱动的人体运动生成扩展到动物领域面临两大瓶颈：一是动物骨骼拓扑高度多样，缺乏统一建模方式，导致需为每个物种单独建模，效率低下；二是现有动物运动数据集规模小、标注质量差，限制模型性能。

Method: 提出UniMo：基于点云的统一运动生成框架，将参数化骨骼转换为非参数化表示以绕开拓扑差异，并引入动态采样，为活跃关节分配更多点。同时构建UniML3D大规模运动-语言数据集，覆盖人与动物类别，含145,907条运动序列与433,388条文本描述。

Result: UniML3D规模超过现有动物数据集102倍以上；方法在UniML3D及HumanML3D、KIT-ML、AnimalML3D三个公开基准上均达到SOTA，验证了统一人-动物运动生成的可行性与有效性。

Conclusion: 通过非参数化点云表示与动态采样，UniMo实现了跨物种统一建模，配合大规模UniML3D数据集，证明统一的人-动物运动生成切实可行且性能优越。
...
TL;DR: 本文重新审视"分数级融合"与"硬决策投票"的经典区分，在冻结基础模型推理期组合的广义少样本3D分割任务中，证明将异构证据过早坍缩为单一类别会丢失大量语义信息，保留紧凑的候选类别集合即可恢复大部分性能。


<details>
  <summary>Details</summary>
Motivation: 经典分类器组合研究区分分数级融合与硬决策级投票，但在推理期组合独立预训练、冻结的基础模型这一新场景中，该区分未被重新考察。作者关注的问题：当异构来源在相互作用之前就被坍缩为单一类别时，会丢失多少有用的语义信息？

Method: 提出同输入的"语义保留干预"实验。冻结密集的RegionPLC（dense）与稀疏的跨视图SAM3（sparse）证据，同时冻结模型权重、掩码、几何、词表与融合规则，仅通过匹配的top-k阶梯改变交互前保留的语义候选数量。评估涵盖156个留出ScanNet200场景与50个ScanNet++场景，并进行稀疏源权重敏感性分析、替代算子（max、几何池化）对比、GroundingDINO–SAM2.1源替换诊断及校准诊断。

Result: ScanNet200上top-1达到28.47 HM IoU，全分布融合达34.87 HM（+6.40，95% CI [+5.24,+7.64]）；ScanNet++上为23.02 vs. 26.50 HM（+3.48，95% CI [+1.64,+5.93]）。全分布HM在稀疏源权重0.3–0.7间稳定；替代算子同样优于top-1；源替换诊断显示HM随全保留从14.77单调增至18.75；校准诊断发现两源存在相反的误校准，但校正校准并未消除保留优势。

Conclusion: 跨数据集与源栈，保留紧凑的合理候选集合即可恢复大部分信息。本文的贡献是对"过早语义坍缩"这一异构冻结模型组合中可复现的信息瓶颈进行了受控诊断。
...
TL;DR: 该论文为HSI-Road数据集引入六类人工标注分类体系与RGB-to-NIR配准流程，并在四种输入配置下评估六种语义分割模型，发现原始分辨率RGB性能最高，而在匹配分辨率下RGB-NIR通道堆叠优于单独NIR和配准RGB。


<details>
  <summary>Details</summary>
Motivation: HSI-Road数据集虽提供配对的RGB与25通道NIR图像及二值掩码，但缺乏表面级别的类别标签，无法支持细粒度语义分割研究，因此需要构建更精细的标注体系并评估不同光谱与分辨率输入的影响。

Method: 引入人工标注的六类分类体系（Background、Asphalt、Concrete、Dirt、Water、Grass）及RGB-to-NIR配准流程与相应标注；选取六种语义分割模型（SSMs），在四种输入配置下评估：原始分辨率RGB、配准低分辨率RGB、NIR，以及RGB_reg-NIR通道堆叠（RGBN_stk），采用逐类与平均IoU及F1分数报告结果。

Result: RGB_ori整体性能最高，但其像素数量是匹配分辨率输入的12倍；在192×384匹配分辨率下，RGBN_stk在全部六种SSM上均优于NIR，并在六种中的五种优于RGB_reg，其中Water类增益最为一致。

Conclusion: 结果强调了空间分辨率的重要性，同时表明NIR能够为RGB提供互补信息，RGB-NIR融合在匹配分辨率条件下具有稳定优势。
...
TL;DR: 提出以人为中心的双手交互建模，用单一查询预测人物框、姿态、手框与状态及交互目标，统一检测与交互推理。


<details>
  <summary>Details</summary>
Motivation: 现有手-物交互方法多为以手为中心，将每只手当作独立实例，在多人物场景中会导致归属歧义，难以判断哪两只手属于同一人以及每只手与什么交互。

Method: 采用以人为中心的表述：一个查询输出该人的结构化结果（人体框、身体姿态、手框与手部状态、交互目标）。引入部件感知可变形注意力，在人体、手部与姿态特定的参考区域间分配注意力，使单个查询捕获完整人体结构。并用"手到查询关系矩阵"统一检测与交互推理：每只手从检测查询集合加一个可学习的 off token 中选择交互目标，直接恢复目标框和类别，无需单独的目标回归。

Result: 构建了基于 COCO 的以人为中心双手交互标注数据集，并定义了评估手部状态与完整手-物元组的结构化指标。基于 Transformer 检测器的实验表明该表述提升了人物级双手交互解析效果。

Conclusion: 该以人为中心、部件感知注意力与手-查询关系矩阵的设计，为联合检测、姿态估计与手部推理提供了一个有效的统一框架。
...
TL;DR: 本文提出USPLIT-VQA——一种面向隐私保护VQA的U型拆分学习框架，客户端保留初始层与分类头，服务器托管中间重计算层，并引入基于梯度相似度的贡献感知加权聚合（CAWA）以抑制恶意更新。


<details>
  <summary>Details</summary>
Motivation: VQA系统需联合处理图像与自然语言，应用前景广，但用户数据隐私敏感：集中式训练需汇聚全部数据，联邦学习又要求每个客户端承载完整模型，均存在隐私或资源负担。因此需要一种既能保护原始数据与标签、又能降低客户端资源开销的隐私保护VQA方案。

Method: 提出USPLIT-VQA：U型拆分学习框架，客户端保留初始层和分类头，服务器托管计算量大的中间层，使原始输入与标签留在本地。同时提出CAWA（Contribution-Aware Weighted Aggregation），一种基于梯度相似度的客户端评分加权聚合机制，用于削弱恶意更新的影响。

Result: 在四个VQA数据集（VQA-RAD、SLAKE、PathVQA、VizWiz）与两种骨干网络上实验：Custom模型精度优于联邦学习，BiomedCLIP在既定固定拆分下精度下降；客户端内存降低最高5.8倍，通信降低最高10.8倍。存在单个恶意客户端时，CAWA将攻击者影响降低逾98%，更高污染水平下暴露其局限。重建实验表明在所评估攻击下具有更低的反演质量。

Conclusion: USPLIT-VQA在隐私保护、资源效率与一定攻击鲁棒性之间取得较好折中，能显著降低客户端内存与通信开销，并通过CAWA有效抑制单恶意客户端影响，但面对更高污染水平与特定骨干网络时仍存在精度与鲁棒性局限。
...
TL;DR: 该研究构建了基于无人机多光谱影像与ViT的玉米褪绿斑点病毒（WSMV）植株级检测流程，发现高精度主要源于标签偏差而非真实病害识别。


<details>
  <summary>Details</summary>
Motivation: WSMV是甜玉米等谷物的破坏性病原，且新西兰、智利等国检疫法规要求种子批无病毒，具有监管重要性。田间症状空间变异大且易与非生物胁迫混淆，人工巡田不可靠；ELISA准确但昂贵、费工、难以规模化。因此需要可扩展的自动化检测手段。

Method: 提出无人机系统（UAS）多光谱影像的自动化流水线：正射影像重建、地理空间配准、植株提取，以及采用七通道输入（五个光谱波段、NDVI、NDRE）的Vision Transformer分类模型。分别以处理组标签、行级症状严重度和植株级ELISA标签作为评价基准进行对比。

Result: 基于处理组标签，模型在多个生育期的6500余个测试图块上达89%准确率。但ELISA地面真值显示标签噪声严重：接种小区中仅少部分抽样植株实际感染。处理组标签不能可靠反映感染状态，高准确率主要由标签偏差驱动。面对行级症状严重度和植株级ELISA标签时性能显著下降；在高保真但小样本条件下，深度学习与经典机器学习泛化能力均有限，ELISA确认的模拟接种株与感染株可分离性弱。

Conclusion: 无人机病害检测受限于标签保真度与数据可得性。应强调基于生物学的标签以及契合真实世界条件的模型评价，避免因标签偏差而高估检测性能。
...
TL;DR: 提出无标注基准 MatPCR，用程序化 oracle 检验材料推理链的物理一致性，定义 Physical-Consistency Rate 并给出误差位移恒等式。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言与语言模型在解释材料数据时会幻觉出无效性质、违反物理定律；而现有评测仅将最终答案与稀缺人工标注比对，或由发现型智能体验证最终提案/DFT 执行，均未衡量模型推理链本身的物理一致性。材料数据自带物理规律，使一大类材料推理无需标注即可验证。

Method: MatPCR：无标注基准，程序化 oracle 依据布拉格定律检验衍射几何、标尺、光谱峰，并基于 Materials Project 检验近凸包稳定性、带隙类别与净磁化。定义覆盖图像与结构输入的 Physical-Consistency Rate；提出 Constraint-Grounded Self-Verification 智能体循环；发布开放验证器；推导 oracle 误差如何偏移报告率的精确恒等式。

Result: 自验证循环的收益在自我精炼与等算力重提示对照下依然成立；开放验证器在分布内有用，但在全部六类留出约束上接近随机水平。

Conclusion: 材料领域的物理一致性可无需人工标注进行验证，MatPCR 提供了推理链层面的评测框架与误差分析方法。
...
TL;DR: 提出物理感知多模态框架QuPAINT，实现二维量子材料剥离薄片的可迁移表征（定位与层数识别）。


<details>
  <summary>Details</summary>
Motivation: 光学显微表征2D量子材料需定位剥离薄片并从微弱光学对比与干涉色推断层厚，以筛选合格薄片用于器件制备；但现有模型面临合成到真实的域偏移，以及跨材料、衬底、实验室和成像条件的变化。

Method: 1) 构建Synthia（合成材料框架），生成多样合成显微图像并保持层依赖的光学行为；2) 基于Synthia构建QMat-Instruct多模态指令数据集，其推理轨迹来自经校验的标注并受限于可观测光学线索；3) 以Physics-Informed Attention (PIA)注入衬底相对光学先验至视觉表示，支持有据可依的多模态推理；4) 提出QF-Bench真实世界基准以进行评估。

Result: QF-Bench为据称该问题规模最大的真实世界基准，覆盖多样显微与衬底条件，评估计数、视觉定位、推理质量、置信度校准及向未见过材料的迁移。QuPAINT-8B显著优于既有方法，在通用与单层薄片检测上均达SOTA；图像接地监督提升严格空间定位与置信度校准，同时在未见过材料上保持稳健的通用薄片检测。

Conclusion: 物理感知的合成数据、指令微调与光学先验注意力相结合，可有效缓解跨域差异，实现可迁移、可解释的量子薄片表征，并具备向新材料泛化的能力。
...
TL;DR: VS-Splat：一种前馈高斯泼溅框架，通过可学习体素选择将高斯基元集中放置在物体相关体素内，仅用2D渲染监督，在稀疏视图重建上超越多个SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有前馈高斯泼溅方法在3D空间中均匀预测高斯基元，导致大量基元落在非物体区域，从而妨碍了对物体精细细节的表示。

Method: 提出VS-Splat端到端前馈高斯泼溅框架：引入新的可学习体素选择方法，在无3D结构监督的情况下，仅依靠2D渲染监督识别以物体为中心的体素，并仅在这些被选中的体素内预测大量高斯基元。

Result: 在三个基准数据集上的稀疏视图渲染实验表明，VS-Splat优于多个现有最先进方法；作为现有致密化方法的骨干网络亦有效；可选扩展提升了其对不准确相机位姿估计的鲁棒性。

Conclusion: 通过体素选择性放置高斯基元，可在无3D结构监督下显著提升前馈稀疏视图重建质量，并具备良好的可扩展性与鲁棒性。
...
TL;DR: 本文提出EgoMaize，一个面向近距第一人称玉米图像实例分割的紧凑基准，采用"证据封闭"标注流程处理遮挡区域，并将不可靠区域标为ignore。基线实验表明现有架构无法同时解决精细结构恢复、同类实例归属与遮挡推理三大耦合挑战。


<details>
  <summary>Details</summary>
Motivation: 移动玉米表型分析中，许多植株级性状依赖冠层内部结构，难以从俯视视角观测，因此近距第一人称图像十分重要。但苗后玉米田的实例分割存在困难：茎、叶、雄穗及邻近植株细长、重复且遮挡严重。此外，现有"仅可见"标注会把同一物理植株割裂为不连续的监督信号，而"全非模态"标注则需对相邻植株或田间物体后的区域进行不可验证的补全。

Method: 构建EgoMaize基准，任务为从存在严重同类重叠的近距田间图像中预测"归属一致"的植株掩码及植株所属的茎/雄穗线索。提出证据封闭（evidence-closed）的遮挡玉米区域标注流程，并将不可靠玉米区域标为ignore而非背景。

Result: 基线结果显示，预训练的查询式分组、边界细化与高分辨率裁剪细化分别有助于任务的不同方面，但没有任何架构能解决精细结构恢复、同类实例归属与遮挡推理相互耦合的难题；遮挡层级分析进一步表明，随植株可见性降低，性能下降。

Conclusion: EgoMaize为第一人称玉米实例分割提供了紧凑基准与证据封闭标注范式，揭示了当前方法在同类重叠与遮挡场景下的根本局限，数据集与代码已公开于 https://github.com/JaaaaaaaD/EgoMaize。
...
TL;DR: 本文提出"因果注视恢复"任务，并设计CCGF模型在驾驶员头部大幅转动导致注视丢失时进行在线预测，实车数据上误差较仅用历史基线降低33%。


<details>
  <summary>Details</summary>
Motivation: 仪表盘式注视追踪器在驾驶员大幅转头（如回头观察、看后视镜、路口扫视）时会丢失眼部信号，而这些时刻恰是判断视觉注意力最关键的时段。离线填补方法可借助缺失区间两侧的观测进行重建，但在线驾驶监控系统无法使用尚未发生的测量，因此需要一种仅依赖当前及历史信息的因果预测方案。

Method: 形式化"因果注视恢复"任务：在时刻t不使用t及以后的真实追踪器注视数据来预测缺失注视。提出Causal Context-Gated Forecaster (CCGF)，编码60帧丢失前的注视与头部姿态历史，并结合DINOv3场景特征；通过可学习的可靠性门控（reliability gate）随丢失进程动态调节历史与场景表示的贡献。设置两种场景条件：Live（丢失期间场景表示持续更新）与Frozen（全程沿用丢失前最后一帧表示）。数据来自10名驾驶员10.5小时自然驾驶中的2,047个自然发生的GazeSense head_lost事件，头戴Neon追踪器提供同步注视坐标仅用于监督与评估，从不作为模型输入，采用留一驾驶员（leave-one-driver-out）评估。

Result: head_lost占GazeSense记录总时长的8.5%。留一驾驶员评估下，CCGF在Live场景更新下取得每驾驶员中位误差均值175.7 px（10.5°），较仅用历史的因果预测基线降低33%；在Frozen场景输入下误差升至210.8 px（12.9°），表明丢失期间获取的场景观测具有有用的预测信息。

Conclusion: 因果注视恢复可在追踪器丢失期间实时预测驾驶员注视，场景上下文尤其是丢失期间持续更新的场景信息能显著提升预测精度。作者将公开数据集、评估协议与因果基线。
...
TL;DR: 本文提出UFO——首个面向多模态图像生成的全条件对齐统一评估框架，并构建UFO-Bench基准。


<details>
  <summary>Details</summary>
Motivation: 现有多模态图像生成（尤其是主题驱动定制）的评估方法（基于嵌入或基于MLLM）均对每种模态条件进行孤立对齐评估，与多模态生成中"同时满足多条件对齐"的目标相矛盾，导致与人判断的一致性较差。

Method: UFO引入新颖的"原子化评估链"（Atomized Chain-of-Evaluation）范式：先将全条件对齐分解为一系列细粒度、解耦的原子评估单元（AEU），将其归类为不同模态相关性类别，再针对不同AEU类型采用通用或专用功能调用进行准确验证。

Result: 实验表明UFO与人类评估偏好的相关性最高，平均提升15.25%；同时提出UFO-Bench，用于在文本与视觉条件多样交互下全面评估现有定制模型性能。

Conclusion: UFO实现了对全条件对齐的同时评估，显著优于现有评估方法，并配套UFO-Bench为多模态定制模型提供更贴合人类偏好的综合评估方案。
...
TL;DR: GSO-Net：首个面向石化危化品装卸场景SOP视觉理解的大规模公开基准，含6.4万真实高速物流节点、5万余帧、9宏观步骤/15微观状态双层标注，并定义联合检测与帧级分类两项任务。


<details>
  <summary>Details</summary>
Motivation: 石化物流节点危化品作业属安全攸关场景，但现有视觉基准少有涉及真实部署约束下的流程合规性；大规模网络中摄像头常采用稀疏轮询，须从不完整观测与局部证据推断转输状态。

Method: 构建GSO-Net：从64个真实高速公路石化物流节点独立采样超5万帧；依据SOP建立层级体系，将9个宏观流程步骤与15个微观操作状态关联；定义两项任务——微观状态与宏观步骤联合检测为核心基准，帧级步骤分类为诊断参考；并在轻量化、Transformer类、开放词汇与整体式模型上开展实验。

Result: 实验显示物体感知与转输阶段理解之间存在明显差距；现有模型在接触级状态定位、瞬态步骤识别与阶段一致性方面表现薄弱，在稀疏轮询、微小关键目标与长尾操作证据条件下尤为明显。

Conclusion: GSO-Net为危化品运输中的细粒度状态感知与视觉安全监控提供了实用基准，数据集已公开于https://github.com/yuxieHarrison/GSO-Net。
...
TL;DR: 提出 OphBiWSSD 框架，利用双向状态空间对偶（Bidirectional State Space Duality）重构眼科手术时序动作定位，以线性复杂度建模长程双向依赖，在 OphNet 基准上取得 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 眼科高频手术操作需要高保真时序建模，但注意力架构刻画长程序列依赖的计算代价过高；现有模型往往依赖激进的时间下采样，损害了对细粒度动作边界与器械-组织交互的检测能力。因此需要在避免 Transformer 二次内存开销的前提下，兼顾局部边界精度与长程流程上下文。

Method: 提出 OphBiWSSD 框架，采用双向状态空间对偶重新表述手术时序动作定位。核心是权重共享（weight-tied）的选择性扫描机制，同时融合前向与后向手术上下文，以线性复杂度实现非因果时序线索的全局综合，从而在无需二次内存开销的情况下捕获眼科工作流中的双向依赖。

Result: 在 OphNet 基准上进行大量实验，OphBiWSSD 达到最先进的时序定位性能：阶段（phases）mAP 为 44.42%，操作（operations）mAP 为 43.08%，分别超过基线 6.80% 和 6.66%。

Conclusion: 该工作实现了精确的时序定位，并为临床环境下部署手术智能系统提供了计算可行的路径。代码已开源于 https://github.com/yo3nglau/OphBiWSSD。
...
TL;DR: 本研究构建了可解释的多模态深度学习框架（3D CNN处理T1加权MRI + 前馈网络处理协调化临床与人口学数据），在ADNI（6,479例）与OASIS-3（1,703例）上比较不同融合策略在三分类及两两诊断任务中的表现与解释性。结果显示多模态性能与解释均依赖任务、模态、融合方式与队列：认知信号（MMSE）贡献稳定，但特征贡献与CAM解释不稳定，说明可解释性应作为队列迁移下需评估的性质而非模型的固有属性。


<details>
  <summary>Details</summary>
Motivation: 痴呆是全球重大且不断增长的健康负担，其中阿尔茨海默病（AD）占多数病例。及时准确诊断依赖临床与影像等互补信息的整合，多模态深度学习可实现这一整合，但其解释在不同模态、融合策略与队列间的行为尚不明确。

Method: 构建可解释多模态框架：3D CNN编码器处理T1加权MRI，前馈网络处理协调化的临床与人口学数据；在ADNI内部数据（6,479例）与OASIS-3独立数据（1,703例）上，针对三分类与两两诊断任务比较多种模型配置与融合策略；采用SHAP与Integrated Gradients进行特征归因，并辅以CAM类可视化解释。

Result: ADNI上，仅表格模型三分类AUC-ROC最高（0.879），CN vs MCI判别最佳（0.903），交叉注意力在MCI vs AD最佳（0.861），CN vs AD整体判别性强；OASIS-3上仅视觉模型最佳（三分类AUC-ROC 0.910），CN vs MCI仍困难，且无融合策略在所有任务与队列中稳定优于单模态。SHAP与Integrated Gradients均显示MMSE为两队列中最主要的表格特征，全局特征排序在ADNI（ρ=0.94）与OASIS-3（ρ=0.96）高度一致；但CAM解释随模型配置与队列变化。

Conclusion: 多模态性能与解释是任务、模态、融合方式和队列依赖的：主导性认知信号在队列间持续存在，但特征贡献与CAM解释并不稳定。这凸显出应在队列迁移（cohort shift）条件下评估可解释性，而非将其视为稳定、内在的模型属性。
...
TL;DR: 本文提出DERA（Detached Edge-Residual Adaptation）框架，用于X射线图像中的违禁品检测，通过解耦的边缘残差适应设计提升检测精度。


<details>
  <summary>Details</summary>
Motivation: X射线图像中物体叠加、纹理微弱和材质杂乱，遮蔽了语义外观和物体边界，使违禁品检测仍具挑战性。

Method: DERA结合分层视觉特征与并行的像素差边缘金字塔，并从实例掩码的训练期轮廓中学习物体特定的边界先验；该解耦先验对边缘敏感特征进行门控，并通过残差头注入早期视觉阶段。该分阶段设计在适应初期保留基础检测器，将边界监督与语义特征学习隔离，并仅使最后适应阶段有14.7K可训练参数。

Result: 在PIDray、CLCXray和STCray数据集上，DERA相比基线分别提升3.1、1.6和2.4个AP点。

Conclusion: DERA通过解耦的边界先验与残差适应机制，有效应对X射线图像中的遮挡与弱纹理问题，在多个基准上显著提升违禁品检测性能。
...
TL;DR: 提出GMPCR：一种基于谱一致性引导的非学习式多视图点云配准框架，通过二阶兼容性结构与主导谱响应筛选可靠对应与信息量大的扫描对，实现低重叠场景下高效鲁棒的配准。


<details>
  <summary>Details</summary>
Motivation: 多视图点云配准在低重叠场景中尤为困难：可靠对应关系有限，错误的成对变换会影响全局位姿估计；同时，配准所有扫描对计算代价高昂，因为许多扫描对提供的几何信息很弱。

Method: GMPCR由初始对应构建精炼的二阶兼容性结构，利用其主导谱响应同时评估对应可靠性与扫描对置信度，从而在相对变换估计前过滤不可靠对应并选择有信息量的扫描对，形成稀疏位姿图并降低成对配准开销。对保留的扫描对采用最大团假设生成估计可靠的相对变换；所得位姿图通过自适应历史感知同步方案进一步精炼，其中残差历史的影响依据全局旋转残差变化进行调整，并设计恢复机制使被降权的边在全局一致性改善时重新获得置信。

Result: 在3DMatch、3DLoMatch、ScanNet和ETH数据集上的实验验证了GMPCR的有效性，在3DMatch和3DLoMatch上分别达到97.2%和89.6%的配准召回率，同时在ScanNet和ETH上保持有竞争力的性能，在配准精度、低重叠鲁棒性与计算效率之间取得良好平衡。

Conclusion: GMPCR通过谱一致性引导的稀疏位姿图构建与自适应同步机制，有效解决了低重叠多视图点云配准中可靠对应稀缺、错误变换传播和计算代价高的问题，兼顾精度、鲁棒性与效率。
...
TL;DR: 本文提出端到端可控裂缝数据合成流水线：用贝塞尔曲线骨架经GAN生成真实裂缝掩码，再用双ControlNet扩散框架分离外观与几何引导，实现可控裂缝图像生成与场景修补。


<details>
  <summary>Details</summary>
Motivation: 深度学习裂缝检测受限于缺陷数据稀缺且难以可控生成；现有生成式增强方法将裂缝合成视为通用图像生成任务，对形态、边界保真度和场景上下文控制不足。

Method: 将裂缝几何与检测上下文形式化为可复用计算约束：1）程序化采样贝塞尔曲线骨架，经GAN转换为真实裂缝掩码，无需人工设计掩码即可规模化生成多样裂缝形态；2）双ControlNet扩散框架，将外观引导与几何引导解耦，其中基于边缘的分支强制严格边界一致性；支持无背景合成与上下文感知修补两种模式。

Result: 在CRACK500和CrackTree200数据集上，相较现有增强基线取得一致性提升。

Conclusion: 该工作验证了面向自动化裂缝检测数据生成的可扩展工程信息学流程的有效性。
...
TL;DR: Spatial-FAD通过融合VFM空间先验与CLIP语义，提升少样本医学异常检测的病灶定位能力。


<details>
  <summary>Details</summary>
Motivation: CLIP等视觉语言模型凭借强图像-文本对齐实现有效的少样本医学异常检测，但其全局对比预训练缺乏显式空间监督，限制病灶精确定位；而DINO等视觉基础模型通过自蒸馏和局部-全局一致性学习空间连贯的patch表征，能更好捕捉细粒度解剖结构。二者具有互补性。

Method: 提出Spatial-FAD框架：1) VFM增强适配器，将DINO衍生结构亲和先验注入CLIP特征，进行结构引导细化；2) 滑动窗口聚合策略，生成高分辨率、空间稠密嵌入以缓解patch化细节损失和CLIP输入分辨率限制；3) 原型增强支持记忆方案，存储正常/异常紧凑原型，融合patch-to-prototype与图像-文本相似度。

Result: 在Liver CT、Retinal OCT、Brain MRI三个基准数据集上显著优于SOTA，尤其病灶分割任务；4-shot场景下Dice平均提升超11.4%，AUC提升1.8%。

Conclusion: 结合VFM空间先验与CLIP语义可有效增强少样本医学异常检测的定位粒度与分割性能，且原型记忆机制兼顾效率与效果。
...
TL;DR: 提出 PointPiT，一种面向场景级点云的划分不变性参数高效微调框架，通过 SSA 与 GSO 缓解序列化导致的分区表示漂移，参数量不足主干 1% 即达到甚至超越全量微调效果。


<details>
  <summary>Details</summary>
Motivation: 场景级点云几何与空间布局复杂，预训练 3D 点云基础模型虽具强迁移性，但全量微调计算与存储开销大；现有 PEFT 方法多聚焦物体级点云，忽视大规模场景中序列化引起的分区变化问题。

Method: 提出 PointPiT 划分不变微调框架，包含两个核心模块：(1) Scene-aware Structural Adapter (SSA)，融合局部几何模式与全局场景上下文，缓解分区导致的表示偏移；(2) Gradient Subspace Optimization (GSO)，筛选信息丰富且对划分稳定的更新方向，在优化过程中抑制分区相关波动。

Result: 在多个场景级基准上，PointPiT 以不足主干网络 1% 的可训练参数取得与全量微调相当甚至更优的性能，并在代表性 PEFT 方法中保持一致的最优表现。

Conclusion: PointPiT 有效解决了场景级点云 PEFT 中的划分不变性问题，验证了在极低参数预算下实现场景级点云高效适配的可行性，为 3D 基础模型的参数高效迁移提供了新思路。
...
TL;DR: PhysioAI通过将结构化临床知识词典（CKD）经冻结CLIP编码为训练期语义锚点，指导骨架时空表征学习，在康复动作识别基准上取得领先性能，且推理仅需骨架输入。


<details>
  <summary>Details</summary>
Motivation: 康复理疗动作的自动追踪有临床价值，但现有骨架动作识别方法多针对大规模日常动作基准开发，不适配康复场景。公开康复数据集规模小、类间运动学差异细微；运动障碍受试者的动作在幅度、速度和协调性上偏离标准模式，导致类内差异增大，识别可靠性下降。

Method: 提出PhysioAI，一种临床知识引导的语义监督框架。方法将基于图的人体运动时空建模与训练期语义锚点结合；语义锚点来源于结构化临床知识词典（CKD）的文本描述，由冻结的CLIP模型编码并投影到锚空间，为骨架表征学习提供类别特定的语义目标。CKD派生锚点仅在骨架模型训练阶段使用，推理时只需骨架输入。

Result: 在受试者不相交（subject-disjoint）评估下，PhysioAI在KiMoRe Overall达到99.03±1.34%，Hard-67压力测试达到94.64±7.36%，UI-PRMD Overall达到87.44±7.69%；相比各指标最强对比方法分别高出0.27、2.87和1.33个百分点。

Conclusion: 结构化临床知识可作为理疗动作识别中有效的训练期监督来源，缓解康复场景下数据稀缺与类内差异大的问题，同时保持推理阶段的低输入成本。
...
TL;DR: 提出RoES旋转等变选择性频率融合网络，通过可学习频率解耦与双分支融合，在红外-可见光融合及下游检测上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有红外-可见光融合方法因任务病态性依赖结构先验，但通常对所有特征统一施加旋转等变约束，忽视了低频共享结构需严格等变、高频模态细节需更大灵活性的关键差异。

Method: 设计可训练的旋转增强更新/预测模块动态解耦高低频分量，替代静态分解；采用双分支融合模块实现频谱一致性：低频域用旋转等变Mamba捕获长程结构依赖，高频域用极坐标谱注意力Dual-Fourier块在低频显式引导下细化细节。

Result: 大量实验表明RoES在融合质量和下游目标检测上均持续达到SOTA性能。

Conclusion: 通过协调频率选择性特征与等变约束，RoES为多模态融合提供了鲁棒解决方案；代码已开源。
...
TL;DR: 本文开发并验证了LettuceVisSim——一个生成带标签生菜时序图像的生长模拟器，用于缓解温室/可控环境农业中视觉强化学习训练数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 视觉强化学习在可控环境农业（CEA）决策中潜力巨大，但受限于带标签作物图像的稀缺，训练数据不足阻碍了其发展。

Method: 构建LettuceVisSim模拟器，包含三部分：基于过程的模型（PBM）预测地上部干重动态；冠层布局算法由干重推导冠层布局表示；Unity渲染引擎生成图像。随后用实验数据对PBM、干重-投影面积映射、冠层布局表示及渲染速度进行验证，并演示光照控制策略学习。

Result: 五项验证结果：①PBM在动态种植密度管理下复现地上部干重，R²=0.84；②分段三次回归将干重复现为潜在投影面积，R²=0.94；③冠层布局表示在12组不同动态环境与间距条件数据集中复现地面覆盖度动态，以实测干重驱动时R²=0.84，以PBM模拟值驱动时R²=0.40（剔除一个异常值后为0.76）；④Unity引擎生成RGB与分割图像耗时低于10 ms；⑤仅凭LettuceVisSim生成图像即可学习并应用光照控制策略，验证了概念可行性。

Conclusion: LettuceVisSim能生成带标签的作物时序图像，验证了其在CEA中支撑视觉强化学习的可行性，为缓解图像数据稀缺提供了有效工具。
...
TL;DR: 本文提出孟加拉语平面几何的多模态基准 ChitraMiti-12.8k（12,874 道合成题，配 15 属性结构化描述）与 NCTB-500（500 道教材人工抽取图），并设计三阶段评估协议，发现"仅描述"与"图+描述"性能无显著差异，而模型跨模态验证能力薄弱。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLM）在多模态数学推理上的评估仍受限于两点：低资源语言覆盖不足；几何题需同时读图与读题，现有基准难以受控地分离图文贡献。作者希望为孟加拉语几何推理提供标准化评测手段，并检验 VLM 是否真正将文本与所见图示相互核对。

Method: 构建 ChitraMiti-12.8k 合成基准（12,874 道孟加拉语平面几何题，每题附 15 属性结构化描述）与 NCTB-500（500 幅孟加拉语教材中人工抽取的图）。采用三阶段协议分离输入模态：仅图、图+描述、仅描述；在 5 个开源与闭源 VLM 上评测，并做监督微调实验；另设空间关系被替换的扰动项以考察跨模态验证。

Result: 仅描述输入的性能与图+描述输入在统计上无显著差异，说明结构化描述可作为受控评测的充分文本代理；但模型跨模态验证能力差，即使原题回答正确，也常被替换后的空间关系误导。在 ChitraMiti-12.8k 上微调可提升 ChitraMiti-1k 与 NCTB-500 的表现，但与最强零样本模型仍有明显差距。

Conclusion: ChitraMiti-12.8k、NCTB-500 与三阶段协议为孟加拉语多模态几何推理提供了标准化研究框架，并可推广用于检验 VLM 是否真正核对文本与图像的一致性。数据集与代码已公开于 Hugging Face。
...
TL;DR: AutoSkill 是一个源监督框架，自动发现并路由可执行的帧选择技能，用无标签的目标问题文本将问题归入语义类别并匹配技能，在五个长视频基准上提升了冻结视频 MLLM 的问答性能。


<details>
  <summary>Details</summary>
Motivation: 长视频问答需在有限帧预算下定位关键证据，但多数免训练方法对所有问题使用相同的帧选择策略，忽视了不同问题类型所需证据的显著差异。分析表明各帧选择策略的相对有效性随语义类别和基准而变化，因此需要自适应证据获取。

Method: AutoSkill 从少量有标注源池出发，由 LLM 智能体迭代地提出、实现、评估并精炼候选帧选择技能；针对目标基准，仅利用无标注的问题与选项文本归纳共享语义分类体系，将已标注源示例改写为目标风格，并估计类别到技能的映射，全程不使用目标视频与答案。推理时每个问题分配一个技能，由该技能选择帧并交由冻结的视频 MLLM 单次推理。

Result: 在五个长视频基准划分上，AutoSkill 分别将 Qwen2.5-VL-7B 和 Qwen3.5-4B 提升 2.4% 和 1.2%。

Conclusion: 结果表明 AutoSkill 能够有效实现自适应的帧选择技能发现与路由，在无需目标视频与答案监督的条件下提升长视频问答性能。
...
TL;DR: 提出传感器级RGB-热辐射配准与有符号局部对比度编码流程，构建M3T数据集，用紧凑单流检测器评估无人机立面异常筛查，显示类别相关的收益与局限。


<details>
  <summary>Details</summary>
Motivation: 无人机立面巡检可结合RGB与热成像筛查表面及次表面异常，但传感器间几何差异与热图渲染会破坏空间对应关系、削弱温度对比，因此需要一套对齐与辐射对比度编码流程来提升检测有效性。

Method: 传感器级流水线：逐传感器校正、RGB到热成像配准、公共支撑区域裁剪、16位辐射测量值的有符号局部对比度编码（作为紧凑单流检测器第4输入通道）；构建M3T数据集（5个立面巡检项目、674对配对样本、8类构件与异常类别）；进行控制位移分析、按项目分组四折评估，以及单划分消融与RGBT-Tiny评估。

Result: 配准残差中位数3.384像素；四折评估在IoU 0.5–0.95上mAP为0.168，每图28.50 GFLOPs；消融显示分层/脱粘检测较RGB-only与替代热输入有改善，但总体精度未超过RGB-only；在RGBT-Tiny上使用渲染热图表现参差。

Conclusion: 对齐的辐射对比度输入对紧凑立面筛查具有类别特异性收益与局限，说明该流程在特定异常类型上有价值，但尚不能整体优于RGB方案，渲染热图泛化性有限。
...
TL;DR: 提出Earth-Agent-Pro执行自适应Plan-and-Execute框架与Earth-Bench-Pro基准，测试全链条开放式地球观测执行能力。


<details>
  <summary>Details</summary>
Motivation: 现有EO智能体多从给定观测出发，基准也常提供现成输入或候选答案，导致从科学问题到观测获取、数据处理与结论推导的全链条开放世界执行能力缺乏检验。

Method: Earth-Agent-Pro采用专家编写技能约束规划与运行时工具调用；以工作流为中心的结构化记忆记录计划步骤、被采纳证据及其依赖，支持仅修复受影响的后缀；规划器用序列级监督微调，执行器用节点级GRPO与局部可验证奖励进行工具参数落地。Earth-Bench-Pro将248个专家任务核心实例化为744个问题，含三个匹配机制，248道开放世界执行题覆盖RGB影像、光谱观测与遥感产品。

Result: 共享GPT-5骨干下，LLM-as-Judge准确率达66.13%，较ReAct高20.95个百分点，Tools-In-Order高24.44个百分点；联合适配器微调使Qwen3.5-9B从38.31%提升至50.00%，增益11.69个百分点。仅规划评估与参考工作流执行分别验证了工作流组合与参数落地能力的提升。

Conclusion: 该工作提出面向全链条开放世界EO执行的框架与基准，通过规划-执行解耦的适配器微调与结构化记忆修复机制显著提升执行表现。
...
TL;DR: 提出 RelateAnything（53M 参数），把谓词词表变为推理期文本输入，实现开放词表关系预测，并配套构建 RA-4M 语料与 OV-SGG-Bench 基准。


<details>
  <summary>Details</summary>
Motivation: 开放词表检测与可提示分割已使类别体系从模型内部转为推理输入，但关系预测仍被固定谓词集（50/56 类、单一标注风格）和物体标签条件化所束缚，无法泛化到新词表与新检测器。其三大障碍并非建模问题：缺少自由文本且经核实的关系语料、标签条件化架构无法接受未训练词表、标准指标只奖励与训练语料一致（扩大词表反被判为退化）。

Method: RelateAnything 接受一张图像与来自任意来源的区域，输出在推理期以字符串给出的谓词词表上的带分数关系；物体标签从不是输入，词表以文本嵌入库而非学习式分类器表示。训练覆盖 19,103 个谓词，采用正-未标注（PU）监督，并需能区分反义词的文本编码器（对比编码器会把反义词嵌入到余弦相似度 0.95）。为此构建 RA-4M：474k 图像、4.3M 关系、10,102 个自由文本谓词，基于编号框标记生成并做几何验证。评测构建 OV-SGG-Bench，含六个维度、跨数据集打分。

Result: 在三个跨数据集基准及第四个零样本基准上，RelateAnything 的平均召回率为同类规模最强开放词表方法的 2.3–3.5 倍，该优势在真实检测器下依然保持；以不到 2% 的参数量在两个指标上领先一个 3B 视觉语言模型的场景图模型。域内评测会把迁移增益高估约 5 倍。模型、语料与基准均已公开。

Conclusion: 关系预测可被改造为开放词表任务：将谓词词表外置为推理输入、去除物体标签条件化，并辅以自由文本且经验证的语料与多维度跨数据集基准，即可在极小参数规模下获得显著跨域与零样本泛化，同时表明域内指标会严重高估迁移性能。
...
TL;DR: 提出DRS-VPT，一种前馈Transformer架构，用于基础性的图像到扫描配准任务，统一了自动驾驶相机-LiDAR标定与室内相机到地图重定位等下游任务。


<details>
  <summary>Details</summary>
Motivation: 现有图像到扫描配准任务通常各自为政：自动驾驶相机-LiDAR标定与室内相机到地图重定位被视为分开的问题，且室内方法往往依赖针对特定地图训练的网络权重，缺乏统一且可零样本迁移的通用模型。

Method: 输入查询图像与参考3D点云，模型在第一相机坐标系下预测扫描位姿与点图，以及每个相机的位姿与点图；同时预测由粗到细的逐点与逐像素特征金字塔，用于将扫描直接重投影对齐到第一张图像。采用前馈Transformer架构。

Result: 单一DRS-VPT模型在自动驾驶图像到LiDAR配准上达到SOTA；在无需训练地图专属权重的情况下取得有竞争力的室内重定位性能；对未见环境具备较强的零样本迁移能力；定性上学习到背面点遮挡等复杂的扫描到图像投影特性。

Conclusion: 该统一formulation可覆盖多种图像到扫描配准下游任务，且以单一模型实现跨场景与跨域的良好泛化，验证了前馈Transformer在此基础配准问题上的有效性。
...
TL;DR: 本文提出KAD-Net，通过手指拓扑约束模块与任务解耦的多任务框架，缓解单张深度图3D手部姿态估计中的遮挡与任务干扰问题，在ICVL、NYU、MSRA上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 手部运动学复杂且存在自遮挡，现有基于单张深度图的方法难以全面建模手部关节间的拓扑依赖关系；同时传统分层多任务架构让2D关节定位与深度估计共享特征空间，导致二者相互干扰、产生负迁移。

Method: 提出Kinematics-Aware Decoupled Learning Network（KAD-Net）。其一，设计Finger Topology Constraint（FTC）模块，用三个连续手指关节构建局部运动学表示以施加拓扑约束，补充远端关节的运动学特征，并借助可见关节的结构上下文定位被遮挡的远端关节；其二，提出任务解耦的分层多任务框架，将2D关节定位与深度估计分离，并针对深度回归引入专门的多任务学习策略，从而隔离UV特征与深度特征。

Result: 在ICVL、NYU、MSRA等多个基准数据集上的大量实验表明，KAD-Net优于现有方法，在3D手部姿态估计上取得当前最优精度。

Conclusion: KAD-Net通过运动学感知的拓扑约束与任务解耦设计，有效提升了遮挡条件下的3D手部姿态估计鲁棒性与精度，可应用于人机交互、虚拟现实及手势控制系统等场景。
...
TL;DR: 该论文发布了首个公开的超广角SS-OCTA血管分割数据集WOIVES（206只眼，24×20mm²视野，覆盖正视到高度近视，提供软概率标注），并提出PG-Mamba模型，通过极坐标双扫描顺序与动态FOV门控模块提升分割性能及定量血管分析精度。


<details>
  <summary>Details</summary>
Motivation: 超广角扫频OCTA可实现大范围视网膜血管成像，但该尺度下缺乏专门的公开基准数据集，也缺少面向定量血管分析的全面评估体系，制约了相关方法的发展与比较。

Method: 构建公开数据集WOIVES，含152名参与者共206只眼的UWF SS-OCTA图像，视野24×20mm²，屈光状态覆盖正视至高度近视，并提供软概率血管标注；提出视觉状态空间模型PG-Mamba，在常规方向扫描基础上引入两种互补的极坐标扫描顺序，并在瓶颈处加入辅助的Dynamic FOV Gating模块进行空间调制。

Result: 在交叉验证下，PG-Mamba在多项分割指标上优于7种竞争方法，并在血管密度、分形维数和血管长度密度上取得最低中位绝对误差。

Conclusion: WOIVES填补了UWF SS-OCTA血管分割公开基准的空白，PG-Mamba在大范围血管分割与定量分析中表现优越；数据集与代码均已公开。
...
TL;DR: 该论文针对衣物混合型终身行人重识别（CH-LReID）中灾难性遗忘加剧的问题，提出SCORE框架，通过显式建模身份内多样性并协作强化新旧知识，取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: LReID旨在从非平稳数据流中训练统一的行人检索模型，现有方法多假设各行人衣物一致。新出现的CH-LReID场景中衣物一致与衣物变化数据交替出现，更贴近实际也更困难。由于衣物相关知识与衣物无关知识之间存在冲突，灾难性遗忘在此任务中被显著放大，因此需要专门方法加以解决。

Method: 提出SCORE（SubDistribution-aware COllaborative Knowledge REinforcing）框架，核心思想是显式建模身份内多样性以持续巩固两类知识。包含两个机制：一是自适应子分布建模（Adaptive SubDistribution Modeling），为每个身份分配一组分布式子原型以捕捉身份内多样性，提升衣物一致与衣物变化知识间的兼容性；二是分布式知识强化（Distributional Knowledge Reinforcement），通过协作对齐机制将旧子原型知识保留到新子原型中。

Result: 大量实验表明SCORE取得了state-of-the-art性能。代码已开源（https://github.com/zhoujiahuan1991/ECCV2026-SCORE）。

Conclusion: 通过显式建模身份内多样性并对子分布知识进行协作强化，SCORE有效缓解了CH-LReID中的灾难性遗忘问题，实现了衣物一致与衣物变化知识的持续兼容与巩固，达到领先水平。
...
TL;DR: 本文提出RA-SOD，一种可靠性感知的RGB-T显著目标检测框架，通过显式建模模态可靠性并融入特征学习与跨模态融合，在四个基准上取得SOTA并展现出对严重模态退化的强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: RGB-T显著目标检测依赖可见光与热红外的互补信息，但真实场景中两模态可靠性不稳定：RGB在低照度、运动模糊和噪声下退化，热红外存在对比度压缩与传感器伪影。这种退化引入不可靠感知证据，误导跨模态融合并显著降低检测性能，因此需要显式建模模态可靠性。

Method: 提出RA-SOD框架，包含三个组件：(1) 可靠性条件表示，自适应补偿退化模态特征同时保留结构线索；(2) 不确定性引导的双流精炼策略，逐步校正跨模态表示并抑制不可靠证据；(3) 像素级模态竞争机制，依据空间可靠性动态选择模态线索实现细粒度融合。

Result: 在VT821、VT1000、VT5000和VT-IMAG四个基准上的大量实验表明，RA-SOD达到state-of-the-art性能，并在严重模态退化下表现出强鲁棒性。

Conclusion: 通过显式建模并利用模态可靠性，RA-SOD有效缓解了不可靠感知证据对跨模态融合的干扰，验证了可靠性感知建模在RGB-T显著目标检测中的有效性与鲁棒性。代码与模型已开源。
...
TL;DR: 构建高保真深伪数据以研究伪造对生理信号与面部行为的双重干扰，提出双向共注意力融合检测器联合建模rPPG与面部行为token，在多项评测中优于单模态与特征级融合基线。


<details>
  <summary>Details</summary>
Motivation: 现有深伪检测基准缺乏生理真值，且检测器对“面部特征—生理动态”的跨层级关系探索不足，多采用晚期融合或仅依赖rPPG特征。

Method: 在真实rPPG数据集（COHFACE、UBFC-rPPG）上构造高保真深伪操纵；提出双向共注意力融合检测器，将rPPG与面部行为表示为token并联合建模，显式捕捉脉搏动态与面部运动的跨层级依赖，学习联合真实性表征。

Result: 主体不相交5折评测：换脸构造数据AUC 92.80%，动作迁移96.78%，优于rPPG单模态基线与最优特征级融合方法；在Celeb-DF-v2上固定双特征提取器迁移学习，准确率91.20%、AUC 86.08%。

Conclusion: 跨层级联合建模rPPG与面部行为可提升深伪检测鲁棒性，且融合检测器在目标域适配场景下具备可迁移性与适用性。
...
TL;DR: 提出NVKE-CEI统一系统，用于虚假新闻视频检测与解释（FNVDE），通过关键帧提取（NVKE）与内容-证据融合（CEI）克服现有方法的帧选择与多模态/证据检索局限，实验优于SOTA并生成高质量解释。


<details>
  <summary>Details</summary>
Motivation: 短视频平台已成为公众主要新闻来源，也导致虚假新闻视频广泛传播。现有FNVDE方法存在两大局限：一是常用帧选择策略可能遗漏与真实性相关的线索，或无法为理解新闻视频提供足够的时序上下文；二是先前方法要么忽视多模态理解，要么忽视证据检索。

Method: 提出统一系统NVKE-CEI，包含两部分：NVKE（新闻视频关键帧提取），基于视觉与OCR文本相似度联合值的时序变化来选择关键帧；CEI（内容与证据信息融合的FNVDE框架），采用两个专用LLM事实核查器（基于内容与基于证据），其输出由轻量级judge模型融合。

Result: 大量实验表明，NVKE-CEI优于最先进的基线方法，同时能够生成高质量的、有内容依据的解释。

Conclusion: NVKE-CEI通过联合关键帧提取与内容-证据双路LLM核查融合，有效提升了虚假新闻视频检测性能与解释质量，为FNVDE任务提供了统一解决思路。
...
TL;DR: NOVA-GS：噪声感知的低光3D高斯泼溅统一框架，用VGGT前馈估计替代SfM，结合增强、盲点自监督去噪与一致性优化，实现无配对监督、无需正常光照参考的低光三维重建。


<details>
  <summary>Details</summary>
Motivation: 真实低光场景存在严重传感器噪声、低信噪比与光度一致性退化，导致几何估计与新视角合成不稳定。现有方法要么依赖良好光照参考数据完成SfM初始化，要么逐视角增强而引入跨视角不一致。

Method: 提出NOVA-GS统一噪声感知框架，将增强、去噪与几何优化纳入单一流程：基于VGGT前馈估计从退化输入直接获得鲁棒相机位姿与几何，免除SfM；在此基础上集成三个耦合模块——结构感知曝光校正增强、带盲点掩码的自监督去噪（生成伪监督）、一致性驱动的高斯泼溅优化以强化跨视角几何连贯；并引入噪声引导的球谐正则化抑制噪声区域的视角相关伪影。

Result: 在多个真实低光数据集上验证，几何保真度、颜色一致性与鲁棒性均有提升。

Conclusion: 该方法无需配对监督或良好光照参考，可在退化输入下稳定完成低光3D重建与新视角合成。
...
TL;DR: IABEdit在训练阶段嵌入可微语义验证：冻结VLM从GT编辑提取空间感知描述子，可训练对齐器从生成输出重建之，残差梯度教生成器"改什么、改哪里"，推理无VLM开销。


<details>
  <summary>Details</summary>
Motivation: 指令引导图像编辑存在训练盲区：生成模型从不被要求语义验证输出是否真正满足指令，监督止于重建与输入文本级条件，导致编辑不完整、空间溢出、定位差。

Method: 模型无关框架：冻结VLM从真值编辑中抽取空间感知描述子；可训练对齐器从生成输出复现该描述子；两者残差作为梯度回传生成器，同时传递"编辑什么"与"编辑何处"的信号。推理阶段无VLM开销，兼容U-Net（Stable Diffusion）与MMDiT（FLUX）等骨干且不改变其推理管线。

Result: MagicBrush上结构保真度较最佳扩散基线提升+3.49 DINO-I，较最佳总体基线提升+1.26，指令对齐保持竞争力；RealEdit与EMU Edit上基于嵌入指标达SOTA指令遵循；D-LORD监控基准重度遮挡下超Gemini agent +5.13 DINO-P；人类与GPT-4o评估确认编辑感知精确、定位良好。

Conclusion: 梯度对齐的VLM蒸馏可在类真实监控与遮挡条件下保持有效，证明训练时语义验证是解决编辑盲区的可行路径。
...
TL;DR: 本文提出 ExpertHTR，一个统一视觉-语言框架，通过互补监督与条件化模型容量，实现异构手写文本识别资源的联合页面级训练。


<details>
  <summary>Details</summary>
Motivation: 手写文本识别资源规模小，且分散在语言、书写系统、文档结构和标注格式各异的集合中，导致联合页面级训练困难。

Method: 先用统一的 Page-Region-Line 表示组织异构标注，构建完整转录、物理行覆盖、文本定位、局部识别四类相关任务，无需额外人工标注；在联合训练的稠密模型基础上引入稀疏混合专家架构，包含常激活共享分支与条件路由的全 MLP 专家；Sparsegen 使激活路由专家数随隐表示变化，路由正则化抑制对少量专家的持续集中。

Result: 在七个异构手写基准上，互补监督相较仅页面转录训练持续提升，联合多源训练在多数数据集上进一步增益；稀疏专家模型在七个来源中的六个上优于稠密基线；最终统一模型在多数基准上显著优于所评估的通用 OCR 与视觉-语言系统，并在 IAM 段落级基准达到 SOTA，但在若干困难集合上专用 HTR 系统仍更强。

Conclusion: 互补监督、联合多源训练与稀疏专家条件容量可有效提升异构手写文本识别，统一模型具备较强通用性，但在部分困难场景仍不及专用系统。
...
TL;DR: 提出多模态平面图编码器（MMFE），将多种2D室内表示映射到共享的稠密隐网格，实现跨模态几何任务。


<details>
  <summary>Details</summary>
Motivation: 平面图形式多样（矢量CAD、栅格渲染、传感器密度图），异质性导致难以构建跨模态迁移的学习系统，并支持对齐、检索等几何中心任务。

Method: MMFE结合冻结的DINOv3骨干与可训练的DPT头；采用逐单元InfoNCE目标对齐跨模态空间对应区域，并以其他单元为负样本；引入受控相似变换与特征网格扭曲以增强几何畸变鲁棒性并施加几何一致性约束。

Result: 在Structured3D这一域外留出数据集上，MMFE提升了跨模态稠密匹配，借助RANSAC实现稳健的相似对齐，并与学习式聚合结合时取得较强检索性能。

Conclusion: MMFE为异构2D室内表示提供了统一的稠密潜在表示，有效支持跨模态匹配、对齐与检索等几何任务。
...
TL;DR: 提出 GRACE 框架，通过子空间约束与动态门控实现精准、局部化的 T2I 扩散模型概念擦除。


<details>
  <summary>Details</summary>
Motivation: 现有 T2I 扩散模型概念擦除方法缺乏对参数更新的显式约束，导致过度干预与语义漂移；同时依赖人工构造的反事实监督（如替代提示词），数据构建成本高，难以扩展到新概念。

Method: 1) 语义加权敏感子空间估计，精确锁定干预方向；2) 轻量级子空间约束适配器，避免全局语义扰动；3) 自动解耦安全锚点机制，去除人工提示工程；4) 能量驱动动态门控机制，在推理时自适应控制干预时机与强度。

Result: 与五种 SOTA 概念擦除方法的平均表现相比，细粒度 NSFW 削减率提升 17.86%，宏平均目标 CLIP Score 降低 4.75%，面向保真度的 FID 降低 50.58%。

Conclusion: GRACE 在擦除有效性与生成保真度之间取得更优平衡，实现更强概念抑制的同时显著保留原模型的生成能力。
...
TL;DR: 提出AquaCubeAI，一种面向Φsat-2多光谱影像的轻量机器学习方法，在星上直接估算近岸水体浊度，以避免下传与地面处理带来的时延，并在Intel Myriad VPU上验证了嵌入式部署可行性。


<details>
  <summary>Details</summary>
Motivation: 传统卫星水质监测依赖数据下传与地面处理，存在明显时延，难以及时响应快速变化的近岸浊度事件；同时星载平台算力与带宽受限，因此需要将推理从地面段前移到卫星端。

Method: 构建可扩展数据集生成流程，将模拟Φsat-2多光谱影像块与CMEMS HR-OC高分辨率浊度产品在四个欧洲海洋大区的局地近岸站点进行空间对齐；采用空间分块划分协议以避免训练与评估间的数据泄漏；训练紧凑的MLP浊度回归器；通过参数共享重构实现稠密空间预测，并支持阈值化异常掩膜用于星上决策；在Intel Myriad VPU上进行嵌入式部署验证。

Result: 方法在考虑空间相关性的泛化评估下进行验证，所得紧凑MLP模型满足嵌入式约束；参数共享重述可实现浊度制图与简单阈值异常掩膜；在Intel Myriad VPU上的部署证实了低功耗硬件与低时延多光谱推理的可行性。

Conclusion: 将浊度反演从地面段迁移至星上，在严格算力与带宽约束下可实现更低时延、更及时且更具业务价值的近岸水质监测，星载轻量模型与嵌入式VPU方案具备实际可行性。
...
TL;DR: 提出CP-BG-Bench配对视图评估框架，通过固定中心细胞、改变周围像素，揭示Cell Painting编码器在四种评估协议下排名系统性不一致，分歧可沿三条轴解释。


<details>
  <summary>Details</summary>
Motivation: 当前Cell Painting视觉编码器通常仅依赖单一评估指标（多为replicate mAP）进行排名，缺乏对"细胞本身与背景"贡献的受控分析，可能导致排名偏颇且不可迁移。

Method: 构建CP-BG-Bench配对视图框架：固定中心细胞，生成四种匹配视图（原始裁剪C、分割S、密度增强变体CD与SD），将背景像素的剔除或增强作为受控干预。在JUMP-CP、RxRx1、RxRx3-core三个数据集与DINOv3 ViT-B/16、OpenPhenom、SubCell三个编码器上，采用四种社区标准协议（replicate mAP、scIB批次整合、CellProfiler特征预测、跨批次扰动召回）进行评估。

Result: 四种协议对同一编码器给出系统性不同的排名，分歧沿三条轴分解：细胞vs背景、形态vs语境、研究内vs跨批次。最大效应：RxRx3-core上SubCell采用分割输入保留94%裁剪replicate mAP，但仅保留32%裁剪R@10；密度增强恢复84%研究内C-to-S差距，但仅恢复8%跨批次差距。分割视图在两个数据集上预测CellProfiler特征不劣于甚至优于裁剪，逆转replicate-mAP排名；C-to-S差距跨数据集相差一个数量级，而跨编码器保持相近，表明背景驱动的收益由实验设计而非编码器决定。

Conclusion: Cell Painting编码器的单指标排名对所用协议高度敏感；协议分歧可被解释为配对视图设计所揭示的三条轴上的投影。将发布配对视图数据集、重建流程、36个训练检查点、聚合嵌入及完整评估套件。
...
TL;DR: 本文提出轻量级置信度引导的偏振图像融合框架LG-PF，将偏振融合建模为选择性残差迁移过程，并构建多场景数据集MSP，在精度与效率上均取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 偏振图像融合需结合S0的稳定亮度与结构信息以及DoLP的材料敏感细节，但DoLP的可靠性存在空间差异，盲目迁移偏振信息会放大不稳定响应或破坏由S0锚定的结构外观。因此需要一种能够选择性、置信度引导地迁移偏振信息的方法。

Method: LG-PF将偏振融合表述为选择性残差迁移过程，包含三个核心组件：偏振置信度先验（估计空间上可靠的偏振响应）、掩膜引导的多尺度融合模块（在三个特征尺度上调控偏振信息迁移）、轻量级上下文感知有界校正头（稳定局部光度与结构过渡）。置信度引导同时被引入优化目标，以保留可靠偏振细节并抑制无支撑响应。此外构建了MSP数据集，含17类室内外场景、1000对像素对齐图像。

Result: 在MSP数据集上，LG-PF在全部六项评估指标上取得最佳结果；在PIF和GAND上的子集评估显示出良好的迁移性且无需微调。模型仅含0.2936 M参数，单张图像推理时间21.712 ms，在低计算成本下实现有竞争力的融合质量。

Conclusion: LG-PF通过置信度引导的选择性残差迁移实现了高效、高质量的偏振图像融合，并配套发布MSP数据集与官方数据划分，代码和数据集将在发表后公开。
...
TL;DR: 提出LGFN轻量门控RGB-偏振融合框架，通过模态路由器按偏振可用性选择RGB-only或偏振辅助配置，在PCOD_1200上取得最优性能并大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 伪装目标检测在目标与背景高度相似时极具挑战；偏振成像提供互补物理线索，但现有方法通常假设固定的多模态输入配置，并将偏振内部协调与RGB-偏振交互纠缠在一起，缺乏灵活性且冗余度高。

Method: LGFN包含四个组件：(1)确定性模态路由器，依据偏振可用性选择配置；(2)可用性条件模态门，校准可用的偏振分支；(3)门控偏振枢纽，协调学习的DoLP与AoP表示及显式偏振线索；(4)RGB-偏振交叉融合，通过受控残差交互将协调表示注入RGB层级。推理时无需样本相关统计或手工质量描述子。

Result: 在完整230张图像的PCOD_1200测试集上，RGB-only配置取得MAE 0.0090、Dice 0.8806、IoU 0.8144，在所评估的RGB方法中六项指标全部最优；在统一局部重评估协议下，多模态配置在六项指标上均优于PolarNet和IPNet，相比IPNet参数减少53.1%、FLOPs减少73.6%、延迟降低63.0%。

Conclusion: LGFN以轻量门控方式解耦偏振内部协调与跨模态交互，兼顾RGB-only与偏振辅助两种配置，在保证检测精度的同时显著降低参数量、计算量和延迟，验证了灵活多模态融合在伪装目标检测中的有效性。
...
TL;DR: 提出纯在线视频智能体框架VideoXAgent，通过任务规划、按需调用异构专家工具并聚合多模态证据，以约50k token的小上下文实现长视频理解，性能媲美前沿LMM与视频智能体。


<details>
  <summary>Details</summary>
Motivation: 长视频理解类似"大海捞针"：查询相关证据稀疏分布于长时间跨度中，而将密集帧塞进单个VLM上下文会导致上下文退化（context rot）和高成本。现有视频智能体依赖与查询无关的离线预处理或临时工具集，容易遗漏查询相关细节并浪费算力。

Method: VideoXAgent为纯在线框架：从视频文件和用户查询出发，进行规划与任务分解，按需调用专用专家工具，聚合多模态证据生成答案并解决观测冲突。工具集基于数据驱动的原子能力分类法构建，涵盖脚本、VLM及领域模型（检测、OCR、ASR、人脸识别等）等异构专家工具；同时引入客观证据提示与预算感知控制，抑制幻觉与不终止。

Result: 在Video-MME-Long、LongVideoBench-Long、LVBench与MINERVA上，以每样本约50k token的智能体上下文（即便处理小时级视频）媲美前沿LMM与视频智能体；在MINERVA等复杂推理基准上，仅用1,024帧密集打包基线约15%的上下文达到相当水平。

Conclusion: 该框架在视觉能力弱甚至纯文本编排器下仍有效，表明强大的长视频理解可源于渐进式的智能体证据搜寻，而非将整段视频打包进单一上下文。
...
TL;DR: 提出空间上下文差分Mamba（SCDM），通过非对称双分支架构实现表征解耦，在RSNA肺炎数据集上以更少参数和计算量取得竞争性分类性能并实现精准病灶定位。


<details>
  <summary>Details</summary>
Motivation: 现有SSM（尤其是VMamba）在医学图像分析中虽能高效建模长程依赖，但难以从视觉相似的健康解剖背景中区分细微病理特征。其表征往往是纠缠的，缺乏将疾病特异性信号与正常解剖结构显式分离的机制。

Method: 提出SCDM，一种用于选择性表征解耦的非对称双分支架构。其中正分支（Positive Branch）提取判别性特征，负分支（Negative Branch）主动建模并抑制正常解剖上下文。二者通过相似度驱动的排斥门（similarity-driven repulsion gate）与差分推理规则（differential inference rule）实现分离，从而促进竞争性特征学习，且无需额外的分支标签，也不增加模型容量。

Result: 在RSNA Pneumonia数据集上，SCDM取得有竞争力的分类性能（AUC为0.858），同时参数量（29.4M）与FLOPs（1.44G）显著少于标准VMamba和视觉Transformer基线。激活分析表明，该差分机制可实现高精度定位，通过抑制无关解剖干扰有效地分离病灶。

Conclusion: SCDM通过差分解耦机制在医学图像分析中兼顾了效率与性能，验证了显式分离病理与正常解剖表征对于细微病变识别和定位的有效性。
...
TL;DR: 本文提出 CoralscapesV2，一个面向珊瑚礁通用视觉场景理解的数据集，将类别从39扩展到95个细粒度类别，并提供6.5万条完整的鱼类实例掩码标注，成为首个珊瑚礁全景分割数据集与具有挑战性的基准。


<details>
  <summary>Details</summary>
Motivation: 应对全球珊瑚礁退化需要大幅扩大生态监测规模，计算机视觉方法被广泛用于处理海量数据。随着珊瑚礁数据采集范式从高度标准化、受限的调查图像转向可扩展平台上的非受限影像，亟需设计机器学习方法，以便从通用目的的珊瑚礁影像中获得对礁体的细粒度理解。

Method: 构建 CoralscapesV2，作为 Coralscapes 数据集的扩展：扩大数据集规模、范围、标签完整性与质量以支持语义分割；将类别数从39提升至95个细粒度视觉类别；提供6.5万条详尽的鱼类实例掩码标注，并借助视频实现标注完整性（表明仅基于静态图像标注鱼类是不充分的）。

Result: CoralscapesV2 是首个用于珊瑚礁全景分割的数据集，涵盖野外多种场景，为当代语义分割与实例分割模型构成了具有挑战性的基准。

Conclusion: CoralscapesV2 是迈向珊瑚礁通用全景分割的重要一步，对扩大珊瑚礁监测规模意义重大，可应用于从机器人或手持视频的底栖覆盖制图，到设计鱼类行为及鱼礁相互作用自动化量化与理解方法等广泛场景。
...
TL;DR: 该论文提出一种用于宫颈上皮内瘤变（CIN）分级与Swede评分预测的双流交叉注意力深度学习框架，并发布多中心阴道镜数据集BUET Multi-Center Colposcopy Dataset，在CIN三分类上达到71.85%准确率与86.23% AUC-ROC，Swede评分预测AUC-ROC为75.7%–88.4%，总分MAE为1.489。


<details>
  <summary>Details</summary>
Motivation: 宫颈癌疾病负担在低收入和中等收入国家尤为突出，原因在于缺乏受训专科医生，且基于阴道镜的筛查具有主观性。现有方法难以在资源受限环境中实现客观、可扩展的风险分层，因此需要自动化的CIN分级与Swede评分预测工具。

Method: 1) 提出双流交叉注意力架构，通过显式融合成对的多模态宫颈图像（cervigrams）来模拟专家阴道镜医师的对比性组织反应视觉推理；2) 设计复合损失函数，以应对五个Swede评分组件间的严重类别不平衡与评分不一致问题；3) 构建并标注多中心队列数据集BUET Multi-Center Colposcopy Dataset，用于Swede评分预测与CIN分级。

Result: CIN三分类任务：准确率71.85%，AUC-ROC 86.23%，优于现有方法。Swede评分各组件预测：AUC-ROC为75.7%–88.4%，复合损失函数带来一致的F1-score提升。Swede总分（0–10）预测：MAE为1.489。

Conclusion: 所提方法为开发AI辅助阴道镜筛查工具、在资源受限医疗环境中支持基于风险的分诊提供了可行路径。数据集与源代码已公开（https://github.com/mHealthBuet/BUET-colposcopy）。
...
TL;DR: 论文提出PA-CDM——一种位置感知的手写数学表达式识别评价指标，结合字符检测匹配、位置森林编码与散度级加权，并配套StructPerturb v2.0基准与跨指标一致性协议，在与人类判断的相关性上优于现有七个自动指标。


<details>
  <summary>Details</summary>
Motivation: 现有HMER评价指标存在位置盲区：精确匹配率与字符串相似度指标对错误发生位置不敏感，下标错位与分数操作数交换的惩罚与普通token错误无异；基于渲染的字符检测匹配（CDM）虽能稳健对齐字形，但同样位置不敏感（在受控的分数操作数交换上得0.8595，而位置感知评分仅为0.6253）；树编辑指标则存在互补盲点，将解析器归一化范围之外的改写判为结构错误（0.8552，而基于渲染的指标为1.0）。

Method: 提出PA-CDM位置感知指标，将字符检测匹配与位置森林编码及散度级加权相结合；构建StructPerturb v2.0冻结基准，含15个类型—强度单元、共1,340个受控扰动对；设计跨指标一致性协议，由敏感度矩阵、人类研究和LLM评判校准三部分构成。

Result: 在六名标注者的研究中，PA-CDM在七个自动指标里与人类判断的相关性最高（Spearman rho=0.9535，n=990）。前沿LLM评判相关性略高（rho=0.9613），但成本高、非确定性且依赖API；PA-CDM以零边际成本逼近该水平，且行为确定、可诊断。

Conclusion: PA-CDM弥补了现有指标的位置盲区，在与人类判断一致性上优于传统自动指标，并在保持确定性、可诊断与零边际成本的前提下接近LLM评判的表现，可作为HMER评价的实用替代方案。
...
TL;DR: 提出锚点正则化的 Flow-GRPO 框架，在流匹配文生图模型中实现连续效价-唤醒度情感控制，兼顾情感对齐与语义一致性。


<details>
  <summary>Details</summary>
Motivation: 连续情感控制需在不改变提示词所描述的物体、布局与场景的前提下提升情感对齐。现有监督式情感注入方法多优化特征空间代理目标，易出现"情感-语义漂移"：情感条件增强的同时伴随非预期的内容改变。因此需要一种在强化学习层面直接优化情感对齐、同时显式约束语义保持的方法。

Method: 基于流匹配（flow matching）图像生成框架，融合三项要素：(1) 连续效价-唤醒度（VA）条件；(2) 组相对策略优化（GRPO）；(3) 中性语义锚点。将确定性的概率流 ODE 转换为保持边缘分布的 SDE，从而获得非退化的转移密度，支持轨迹采样与策略比估计。以冻结的 CLIP 情感回归器提供终端奖励，度量预测 VA 与目标 VA 坐标的距离；同时用同一提示词在零 VA 条件下生成的图像作为特征空间参考，约束语义保持。在线 RL 采样采用缩减的去噪步数调度，推理时恢复原始调度。

Result: 在 3,300 个提示词-情感组合上，效价与唤醒度误差显著低于 VA 条件基线，CLIPScore 优于 EmotiCrafter，但在无参考图像质量指标上存在可测量的折衷。

Conclusion: 结果表明，锚点正则化的 Flow-GRPO 是连续情感图像合成中平衡情感对齐与语义一致性的可行方案。
...
TL;DR: 该研究面向WSLP 2026共享任务的姿态到文本手语翻译，比较T5规模与显式运动特征的影响，发现T5-small加运动特征在BLEU上表现最佳，提交系统官方排名第5。


<details>
  <summary>Details</summary>
Motivation: 探究预训练T5模型规模以及显式运动特征对姿态到文本印度手语翻译性能的影响。

Method: 通过轻量级姿态编码器将姿态序列投影到T5嵌入空间并整体微调生成英文；对比T5-small、T5-base、T5-large，并提出加入帧间姿态差分的运动增强变体T5-small + Motion。

Result: 仅空间特征模型中T5-small的BLEU和ROUGE最高，T5-large的chrF最高；T5-small + Motion带来最大单项提升，BLEU显著优于空间基线并成为该指标最强模型；提交系统官方排行榜第5名。

Conclusion: 适度规模的T5配合显式运动特征可有效提升姿态到文本手语翻译，运动信息比单纯增大模型规模更有价值。
...
TL;DR: 该研究系统性比较了不同自监督预训练策略在纵向疾病进展建模中的效果，以AMD为案例，发现冻结编码器少标签下预训练表征优势明显，且迁移能力主要取决于自监督目标而非语料规模或域匹配度，最终提出"冻结自监督编码器+轻量生存头"的实用方案。


<details>
  <summary>Details</summary>
Motivation: 纵向影像队列稀缺且样本量小，限制了疾病进展建模；而横断面数据（每人一张图像）丰富。自监督预训练有望弥合这一差距，但何种预训练策略最适合进展建模、以及该结论如何随标注纵向数据量变化尚不明确。

Method: 以年龄相关性黄斑变性（AMD）为研究对象，在大型横断面NAKO队列上预训练编码器，在纵向AREDS数据集上预测进展至晚期AMD的时间。对比自研自监督编码器与通用基础模型（DINOv2）、领域专用基础模型（RETFound）；覆盖对比学习、掩码自编码、自蒸馏等目标；在冻结与微调两种协议下评估；标注训练集规模从100到32,250例不等。

Result: 模型表现取决于编码器的使用方式：在冻结且标签稀缺（纵向队列的典型情形）时，预训练表征仅用数百标注样本即可达到临床可接受的判别性能，而从零训练模型无法做到；该优势在微调下消失。迁移能力由自监督目标而非语料规模或域匹配度主导，因此在中等规模横断面队列上预训练的编码器可匹配甚至超越规模大得多的域内基础模型。

Conclusion: 在纵向数据稀缺时，构建进展模型的实用方案是采用冻结的自监督编码器配合轻量生存头。
...
TL;DR: 本文提出首个预训练低秩张量分解（PLTD）框架，将预训练大视觉模型（DINOv3）融入经典张量分解，兼顾高恢复保真度、少可学习参数与低碳足迹，并在多维图像恢复任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于张量分解的多维图像表示方法从零学习每张图像的实例特定结构，忽略了不同图像间的共性结构，导致语义建模能力受限、计算开销高、可学习参数多。因此需要一种能同时利用共性结构与实例特定结构的分解框架。

Method: 提出 PLTD 框架：将目标张量分解为潜在张量与可学习的变换（将潜在张量映射回原始数据域）。潜在张量由两部分互补组成——固定的预训练潜在张量（从预训练大视觉模型 DINOv3 蒸馏得到，用于刻画目标张量的共性结构）和可学习的低秩潜在张量（刻画实例特定结构）。基于该框架构建多维图像恢复模型，并从理论上论证其优势，同时讨论其与经典张量分解框架的联系。

Result: 在多维图像恢复任务上的大量实验表明，PLTD 相比当前最先进方法持续取得更优性能，并在恢复保真度、可学习参数量与碳足迹之间实现前所未有的平衡。

Conclusion: PLTD 是首个将预训练大视觉模型与经典张量分解有机结合的框架，突破了浅层、未训练深度张量分解的局限，为多维图像表示与恢复提供了更具语义建模能力且高效的新范式。
...
TL;DR: MGAvatar 提出高斯-网格混合表示，通过两种高斯-网格绑定模式联合建模头部几何与外观，实现高保真头部化身。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯（Gaussian）的头部化身通常依赖参数化模板（如 FLAME）进行高斯初始化与变形，但此类模板缺乏个性化先验，且难以表示头发、衣物等结构。为此需要一种既稳定又富有表现力的几何表示。

Method: 1) 提出高斯-网格混合表示 MGAvatar，采用两种高斯-网格绑定模式；2) 引入顶点绑定高斯（vertex-bound Gaussians）并约束其可学习参数，支持渐进式网格变形以表示复杂头部几何，同时用姿态相关偏移模块处理非刚性变形；3) 几何稳定后切换为面绑定高斯（face-bound Gaussians）进行外观建模；4) 引入视角条件神经颜色场（view-conditioned neural color field）以缓解独立优化高斯颜色导致的伪影，提升新姿态与新视角下的外观一致性；5) 设计高斯偏移网络在观测空间预测高斯偏移图，增强面绑定高斯捕捉动态面部纹理的灵活性。

Result: 在多视角与单目视频上的大量实验表明，MGAvatar 在渲染质量上优于现有方法，能够生成具有丰富纹理细节的高保真头部化身。

Conclusion: 通过高斯-网格混合表示与分阶段绑定策略（先几何后外观），结合视角条件颜色场与高斯偏移网络，MGAvatar 有效克服了参数化模板的个性化先验不足与头发/衣物建模困难问题，提升了头部化身的渲染质量与纹理细节表现。
...
TL;DR: 提出基于潜在布朗桥扩散（BBDM）的3D CT到PET转换框架，两阶段训练（VAE+对比学习、潜空间BBDM），在两个公开数据集上提升PET信号保真度与病灶级指标。


<details>
  <summary>Details</summary>
Motivation: PET在肿瘤诊断与治疗计划中提供功能信息，但受限于辐射暴露高、成本高、可及性差；用深度学习从CT合成PET信息是可行思路，然而CT与PET之间存在较大的跨模态差异，精确建模困难。

Method: 两阶段框架：1）在成对的CT-PET图像块上训练VAE，并引入对比学习以增强解剖表征与代谢表征之间的潜在对齐；2）在潜空间中训练布朗桥扩散模型（BBDM），将CT潜表示转换为对应的PET潜表示；解码转换后的PET潜变量并拼接重建最终3D PET体数据。

Result: 在两个公开数据集上评估，基于图像保真度和病灶级PET专用指标的定量结果显示优于对比方法；具体表现为提升PET信号保真度、更好保留临床相关的摄取模式，并在保留小病灶代谢激活方面表现更优。

Conclusion: 该潜空间布朗桥扩散框架能有效缓解CT-PET跨模态差异，改善合成PET的质量与临床相关病灶信息保留，为虚拟成像应用提供了可行路径。
...
TL;DR: 提出4D感知视频分词器VideoTok4D，通过时空解耦、轨迹感知动态注意力及Co4DGen扩散先验，实现紧凑4D世界表征与高效生成，存储比稠密4D表示少至多4个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有视频分词器基本停留在2D视觉域，将视频视为图像序列而非动态3D世界的观测，导致学到的token继承"以观测为中心"的偏差，难以紧凑地表征真实4D场景。

Method: 三项关键设计：1）时空解耦策略，将视频分解为静态token与动态token以实现整体世界建模；2）轨迹感知的动态注意力机制，聚合轨迹对齐线索以提升跨视角运动一致性；3）Co4DGen，在VideoTok4D token空间上学习的扩散先验，用于高效4D场景生成。

Result: 实验表明方法达到最先进性能，同时所需存储比稠密4D表示少至多4个数量级；紧凑token空间显著缩短扩散序列，实现高效生成。

Conclusion: 将视频分词从2D观测范式推进到4D感知的世界表征范式，可在极低存储开销下兼顾表征紧凑性与生成效率，为4D场景建模提供新思路。
...
TL;DR: 该论文将SCE与GCE鲁棒损失应用于手语识别中的单标签分类器，在ISLR上验证其抗噪增益，但在CSLR上未获增益，并指出辅助损失权重混淆了结论。


<details>
  <summary>Details</summary>
Motivation: ISLR的单标签分类损失以及CSLR中基于伪标签的帧级辅助分类器都以单一标签为真值：ISLR受风格差异导致标注模糊，CSLR缺乏时间边界被迫使用伪标签，二者均存在标签噪声。作者希望借用图像分类中CE的鲁棒替代（SCE、GCE）来缓解这一问题。

Method: 不将SCE/GCE用于CTC，而是用于其前置的单标签分类器：ISLR分类损失，以及CSLR(PHOENIX-2014)中帧级伪标签辅助分类器（lambda_aux=25）。在ASL Citizen上注入对称噪声，于三个骨干（ST-GCN三个种子）上对比CE、SCE、GCE，并考察q的选取与迁移性。

Result: ISLR：干净标签下鲁棒损失最多损失2.5 pt；噪声率0.2时全部六种条件下超越CE达2.9-10.0 pt（其中一种需在dev上重选q）。GCE增益更大但最优q无法跨骨干迁移，而单一SCE设置在全部九种条件下均有效；两者跨运行波动比CE大2-11倍。CSLR：无增益，帧级目标存在系统性分配偏差，仅为单一配置的诊断。lambda_aux=25时伪标签CE辅助在VAC、CorrNet、SlowFastSign上使WER高于无辅助基线，GCE/SCE较CE提升1.7-3.2 pt（六种中三种回到基线以下）。

Conclusion: 三种损失在相同lambda_aux下有效梯度相差一个数量级以上；匹配初始梯度后差距缩至0.4-0.9 pt，且单独降低CE权重即可超越基线，因此退化与提升均无法与权重效应分离。研究仅使用对称噪声，多种子评估仅覆盖ST-GCN与VAC isolated，结论受限于上述设定。
...
TL;DR: 提出RABR-Net两阶段框架，先由UNet++/EfficientNet-B4生成基预测，再以边界感知可靠性表示驱动门控残差精修器选择性修正不确定边界像素，在血涂片显微分割中显著提升边界相关指标。


<details>
  <summary>Details</summary>
Motivation: 生物医学图像分割对临床有意义的边界描绘要求高，但深度模型在模棱两可的边界区域常表现出不确定或过度自信，即使Dice分数已很高。血涂片显微中的细胞质与细胞核轮廓是下游形态学分析的结构基础，因此需要面向可靠性的边界精修方法。

Method: 两阶段框架RABR-Net：第一阶段用UNet++ EfficientNet-B4基分割器输出初始类别概率与logits；随后融合预测熵、测试时增强方差、margin不确定性、概率梯度与软边界线索，构建边界感知的可靠性表示；该表示驱动门控残差精修器，选择性修正不确定边界像素并保留基预测中自信区域。

Result: 在留出测试集上，相比缓存的基预测，Dice由0.9602提升至0.9614，Boundary Dice由0.3448提升至0.3611，HD95由3.0354降至2.8274；配对统计检验证实Dice、Boundary Dice与HD95提升显著。定性结果显示学到的门控集中于不确定的细胞质与细胞核边界，修正图验证了局部化边界精修。但精修后校准性能未自动改善。

Conclusion: 该框架为边界敏感的生物医学图像分割提供了可解释、以可靠性为中心的策略，能在保持高重叠精度的同时改善边界描绘；局限在于校准未随精修自动提升。
...
TL;DR: 本文提出 UniPart，一种文本条件的前馈跨模态 3D Transformer，用于开放词汇的 3D 部件分割，并构建了大规模数据集 LangPart-1M 与高质量标注子集 LangPart-4K，实现了零样本分割与真实世界语言条件抓取。


<details>
  <summary>Details</summary>
Motivation: 细粒度机器人操作需要理解部件而非仅整体对象，但现有 3D 基础模型存在两难：泛化性强但仅关注整体对象，或关注部件却局限于封闭类别体系，导致零样本迁移能力弱。

Method: 研究文本条件的 3D 部件分割任务：以自由形式短语在点云上选择功能部件。提出 UniPart——前馈跨模态 3D Transformer，以 CLIP 文本嵌入为条件。为扩大监督规模，利用多视图一致的部件生成构建 LangPart-1M，包含 16 万+ Objaverse 资产与 800 万文本-部件对；并人工标注高质量子集 LangPart-4K 用于微调与评测。

Result: UniPart 在开放词汇部件基准上取得强零样本结果，并成功迁移至真实世界的语言条件部件抓取。

Conclusion: 文本条件的前馈跨模态架构结合大规模自动生成监督，可有效实现开放词汇 3D 部件分割，并具备向真实机器人操作任务迁移的实用价值。
...
TL;DR: 提出基于图像分解与定位的混合CNN-DNN网络，支持并行高效训练深度学习模型，用于COVID-19 CT影像诊断。


<details>
  <summary>Details</summary>
Motivation: AI辅助影像诊断前景广阔，但大规模或动态增长的数据集（如疫情期间）要求深度学习模型能够高效训练，传统训练方式效率不足。

Method: 回顾性研究，收集德国三个中心的300例COVID-19与非COVID-19肺炎患者CT扫描；构建基于图像分解和定位的混合CNN-DNN网络，天然支持并行高效训练。训练了三种架构共156个模型以捕捉不同层次特征，最终形成12个患者级COVID-19诊断模型，并测量诊断性能与训练时间节省。

Result: DenseNet121与3D CNN并行CNN-DNN方法表现最佳：DenseNet121（4×4×1子域）训练/验证/测试准确率为88.78%/76.67%/76.03%；3D CNN（4×4×1子域）为87.72%/76.82%/74.86%。3D CNN配合4×4×2子域的并行训练时间最长缩短31倍。

Conclusion: 并行训练方法同时提升了训练效率与模型性能，可实现快速模型开发，对疫情应对准备等具有重要意义。
...
TL;DR: 该论文将端到端检测总延迟建模为预处理、推理、后处理三阶段分布（在独立性近似下）的卷积，并令各阶段参数随源图像分辨率变化，用YOLOv11n在Jetson Orin NX上按多分辨率验证该分辨率感知参数化对延迟分布拟合的改进。


<details>
  <summary>Details</summary>
Motivation: 理解目标检测流水线的端到端延迟分布对实时系统设计至关重要，但总延迟受多阶段与输入分辨率影响，现有建模常采用固定参数，难以刻画分辨率变化带来的分布差异。

Method: 在独立性近似下假设总延迟为预处理、推理、后处理三个阶段分布的卷积，其CDF给出端到端检测时间分布；每阶段用参数化分布（如Exponential、Erlang、Normal、Gamma）建模，并将参数表达为源图像分辨率的函数。以YOLOv11n在NVIDIA Jetson Orin NX上、使用COCO2017多分辨率图像进行实验，采用Kolmogorov–Smirnov、Anderson–Darling、Cramér–von Mises统计量，与固定参数基线对比。

Result: 在所测设置中，分辨率感知参数化能改善分布近似，尤其对更灵活的Normal与Gamma模型效果更明显；但拟合质量仍依赖于具体分布选择。

Conclusion: 论文提出了一种有理论依据且轻量的建模框架，用于研究实测目标检测流水线中随分辨率变化的延迟分布。
...
TL;DR: 该论文提出DementiaCare-Bench——首个评估视频语言模型（VLM）理解痴呆症行为与心理症状（BPSD）触发因素能力的基准，包含56个专业护理培训视频、94个剪辑、9类症状及2023个基于转录文本定位的问题，并在四种视觉条件下测量问题的视觉依赖度。评测显示12个现有VLM的时序理解能力普遍不足，而轻量LoRA微调模型DemCare-VLM可显著改善视频依赖度。


<details>
  <summary>Details</summary>
Motivation: 痴呆症影响约5700万人，护理中最困难的是BPSD（激越、游走、抗拒护理、日落综合征等）而非记忆丧失。理解这些症状不仅需识别行为本身，还需了解其发生前的触发因素，同一行为因触发因素不同需采取不同应对。VLM有望支持护理人员，但现有基准均未评估这一能力，因此存在空白。

Method: 构建DementiaCare-Bench：56个专业制作的护理培训视频，切分为94个片段，覆盖9类BPSD，通过多智能体流水线生成2023个问题，每条临床论断均锚定于逐字转录文本片段。每个问题在四种视觉条件下探测，并按所需视觉信息量打标签，从而测量而非假定其视觉需求。另通过轻量LoRA微调得到DemCare-VLM。

Result: 测量结果与设计意图相悖：作者原以为77.7%的问题需要有序帧，实际仅34.8%需要。在12个现有VLM上模式一致：最佳模型总体准确率达85%，但该均值主要由仅凭临床知识的语言模型即可回答的问题支撑；在需要有序片段的题目上准确率平均下降17个百分点，某领先开源模型在判断护理人员应对是否恰当时仅为随机水平。LoRA微调后DemCare-VLM将视频依赖度从-3.3提升至+4.5个百分点。

Conclusion: 现有VLM在BPSD护理场景中的时序视频理解能力被总体高分掩盖，真实视觉依赖需求下性能显著退化。基准所暴露的缺陷可通过微调修复，说明该基准不仅能测量问题，还能指导改进。
...
TL;DR: 该论文提出一种有原则的测量条件速度场参数化方法，用于求解成像逆问题，在流匹配框架内显式引入前向模型与数据一致性，实现端到端训练且推理时无需额外引导。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配求解逆问题主要有两类做法：基于条件的方法将测量信息作为网络输入（如拼接），推理引导方法则将无条件速度场与独立的数据一致性更新结合。这两类常见形式都未在学习到的条件速度场内部显式地强制前向模型，因而缺乏原理性的约束。

Method: 在线性插值下，将条件速度 v(x_t,t,y) 用后验均值 E[x_1 | x_t,y] 表示，并证明该均值是某个变分目标的唯一极小值点，该目标含显式的数据一致性项；进一步证明该速度场定义了从源分布到测量条件后验的概率流。随后对变分目标进行拆分，得到一种含算子相关数据一致性更新的条件速度参数化，并在流匹配目标下端到端训练，推理时无需额外引导。

Result: 该方法取得最先进的 PSNR，且函数评估次数比最强流匹配基线少 50 倍；通过调整采样步数可在不重新训练的情况下于测试时控制失真—感知权衡。

Conclusion: 通过将前向模型显式嵌入条件速度场的参数化，论文为流匹配求解逆问题提供了原理性框架，兼顾高精度与高采样效率，并具备灵活的测试时可控性。
...
TL;DR: 提出无监督文本行人搜索框架GTR+（生成-检索两阶段），并构建大规模数据集LargeFine-Person。


<details>
  <summary>Details</summary>
Motivation: 现有文本行人搜索方法多依赖人工标注的图像-文本对进行监督学习，标注成本高、扩展性受限；本文探索仅用无标签图像的无监督TBPS。

Method: 两阶段"先生成后检索"框架。生成阶段采用三层级描述生成：基础层用自动问答机制生成基本视觉属性描述；中间层用样本间对比机制增强细粒度细节；高级层用风格化扩展机制提升文本多样性。检索阶段提出自适应置信度加权检索学习框架，用高斯混合模型将图像-文本对建模为干净或噪声，并以实时图文相似度和静态文本生成概率进行校准，产生自适应样本权重。

Result: 在多个TBPS基准上验证了GTR+的有效性与泛化能力，同时贡献了大规模高质量数据集LargeFine-Person。

Conclusion: GTR+与LargeFine-Person为无监督文本行人搜索提供了实用且可泛化的预训练基准。
...
TL;DR: 提出面向单心室生理（SVP）的CMR分割框架：用SDF4CHD生成合成心脏网格与MRI做数据增强，并以诊断条件化基础模型SV-Cine（基于CineMA+FiLM）提升右心室等分割性能。


<details>
  <summary>Details</summary>
Motivation: SVP是罕见先天性心脏病亚型，仅有单一功能心室且解剖构型非典型，常规图像分割方法难以适用；临床数据稀缺、各亚型形态差异大，导致鲁棒分割方法开发困难。

Method: 1) 数据增强流程：基于SDF4CHD生成合成3D心脏网格，并通过生成建模得到对应的合成心脏MRI；2) SV-Cine：对基础模型CineMA进行诊断条件化改造，通过Feature-wise Linear Modulation（FiLM）层引入患者级诊断信息，实现分割中的诊断感知特征自适应。

Result: 在包含不同SVP亚型的内部队列上评估：左心室中位Dice 0.89（IQR 0.80–0.91），右心室中位Dice 0.72（IQR 0.54–0.84），右心室较最强基线nnU-Net提升0.39 Dice；主导心室中位射血分数误差5.55个百分点（IQR 3.41–7.69）。与内部队列相比，外部队列的LV和心肌分割性能更低，而RV Dice在两个队列间相当。

Conclusion: 预训练基础模型可借助诊断先验适配到高度专门化的下游任务，同时利用大规模MRI预训练中获得的解剖知识，从而改善SVP等罕见复杂解剖条件下的分割表现。
...
TL;DR: 本文提出PhysPlan，一种无需训练的视频扩散模型物理感知引导框架，通过VLM认知模拟与对象中心测试时优化，在PhyGenBench和Physics-IQ上显著优于基线与可控VDM方法。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型（VDM）能生成高保真视频，但缺乏对物理规律的内在理解，常产生结构幻觉与物理上不合理的动力学序列。现有免训练测试时优化方法依赖全局梯度更新和刚性调度启发式，会破坏被动背景且难以建模复杂动态状态变化。

Method: 提出PhysPlan免训练引导框架，将范式从随机视觉插值转为智能体物理模拟。首先VLM作为迭代认知模拟器，将多模态输入分解为视觉思维链，构建运动学轨迹与3D深度几何的多模态表示；其次以这些信号驱动对象中心测试时优化，引入Object-Centric Gradient Routing隔离运动学修改并锁定被动环境，同时用Kinetic Intensity Profiling动态参数化超参数以适应不同程度的物理形变。

Result: 在PhyGenBench和Physics-IQ基准上的广泛评估表明，PhysPlan显著优于基础VDM与可控VDM基线。

Conclusion: PhysPlan为提升视频生成的物理理解提供了一种有前景的免训练引导途径。
...
TL;DR: 视觉语言模型的视觉KV缓存中保留了可像素解码的视觉内容，但该保留量与计算是否因果依赖该单元无关（任务惰性）。像素可解码保留性是与功能性正交的信息轴，不能作为KV压缩信号。


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型的视觉KV缓存中保留的可解码视觉内容是否与任务计算相关，即该保留量能否追踪计算对某单元的因果依赖。

Method: 用学习到的像素反演解码器度量保留量，用单超块KV消融和教师强制下金答案对数概率下降度量因果使用；在预注册、符号校准、留出设计下在图像内关联二者。并检验保留量与注意力的关系，比较编码器自由与基于编码器架构的保留差异，以及在超块/词元粒度下作为KV驱逐信号的排序能力。

Result: 保留量与因果利用解耦（well-powered零结果），也与注意力解耦；注意力弱但显著地追踪因果利用，是唯一信号（设计的阳性对照）。编码器自由模型保留量是基于编码器模型的2.7倍。超块粒度下，去混杂的像素可解码保留量排序KV驱逐不优于随机；词元粒度下仅在大预算时获得弱反重要性信号，且在每个预算下都被注意力幅度支配。

Conclusion: 像素可解码保留性是视觉KV缓存中与功能性正交的信息轴；在测试的所有粒度下，它都不是有竞争力的KV压缩信号（受控负面结果）。
...
TL;DR: 本文提出一种面向平屋顶缺陷检测的实时UAS深度学习框架，采用双高度飞行采集图像并结合轻量化CNN-SVM分类器实现低延迟机载推理，模型准确率达94.4%，优于GoogLeNet与AlexNet。


<details>
  <summary>Details</summary>
Motivation: 平屋顶是建筑围护结构中最具影响力的部件之一，同时决定结构性能与热效率，直接影响家庭能耗、碳排放和长期环境可持续性。及时检测屋顶缺陷有助于减少供暖与制冷损失、防止湿气导致的霉变等劣化，并支持国家气候变化缓解目标。然而，嵌入式UAS硬件的计算与功耗约束限制了复杂模型的应用，因此需要一种兼顾精度与实时性的检测方案。

Method: 提出基于UAS的实时深度学习框架，通过双高度航拍采集实时图像，自主检测屋顶缺陷。多分辨率飞行策略用于同时识别细小缺陷和较大结构性缺陷。为满足嵌入式硬件的计算与功耗限制，框架集成基于切片的（tile-based）架构与轻量化CNN-SVM分类器，实现低延迟机载推理。最终模型包含五个卷积层和四个全连接层，最后一层为线性SVM头。在照片级划分的数据集上评估（43,383张训练、3,869张验证、2,540张测试的切片与增强图像），并与GoogLeNet和AlexNet进行对比。

Result: 模型平均测试准确率为94.4%（三个随机种子下95%置信区间±0.4%），优于GoogLeNet（89.2%）和AlexNet（79.8%）。使用DJI Matrice 350 RTK无人机现场采集的真实UAS图像进行实验评估，结果表明系统支持快速、可重复且安全的屋顶巡检。

Conclusion: 该框架能够降低人员风险、减少运营成本，并促进更可持续的建筑维护，为平屋顶缺陷的实时自动化检测提供了有效方案。
...
TL;DR: 提出标签引导知识蒸馏（LGKD），用真值标签引导3D-CNN学生模型的特征蒸馏，包含样本级与类别级两个分量，在UCF101与HMDB51上取得有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频分析中的特征蒸馏方法多为图像分析方法的简单移植，忽略了视频特征在时间维度上的差异，因此需要针对3D-CNN设计更适配的特征蒸馏方案。

Method: LGKD包含两部分：1）样本级蒸馏，利用标签信息与教师概率分布引导学生学习对时间准确性影响显著的特征并抑制噪声；2）类别级蒸馏，采用原型网络捕获同类样本间的关系知识，提升高维语义学习能力与泛化性。

Result: 在UCF101和HMDB51两个动作识别基准数据集上进行全面实验，取得有竞争力的结果，验证了方法的有效性与优越性。

Conclusion: 通过标签引导的样本级与类别级特征蒸馏，LGKD弥补了视频特征时间维度建模不足的问题，是一种有效的3D-CNN知识蒸馏方法。
...
TL;DR: 该研究构建统一3D胰腺分割框架，通过域对抗学习对4604例CT/MRI扫描学习跨模态解剖表征，实现跨模态全胰腺分割与标签高效的下游亚区分割迁移。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，不同成像模态在外观和强度分布上差异显著，单模态模型应用于未见域时性能大幅下降，因此需要学习跨模态鲁棒且可迁移的统一解剖表征。

Method: 采用域对抗学习，在4,604例异质CT与MRI扫描上训练共享nnU-Net编码器-解码器用于全胰腺分割，引入潜在域判别器促进CT-MRI特征对齐；随后将学习到的编码器迁移至胰头-胰体-胰尾亚区分割，仅使用有限的MRI亚区标注。

Result: 全胰腺分割在分布内测试集上平均Dice为87.31%，在多个外部OOD数据集上Dice为84.20%~88.09%；下游亚区分割在未使用CT亚区标注的情况下，MRI Dice为80.53%、CT Dice为83.05%。

Conclusion: 统一的解剖表征可同时支持跨模态胰腺分割和标签高效的下游任务迁移，验证了域对抗学习在多模态医学图像分割中的有效性。
...
### [73] [R2VC: Modular Fact-Checking with Retrieval, Verification, and Confidence Calibration](https://arxiv.org/abs/2609.11955)
*Dhruv Dixit,Paritosh Pandey*

Main category: cs.CL

TL;DR: 提出模块化事实核查架构 R2VC（检索-推理-验证-校准），在 FEVER 上比基线提升 13.74% 准确率，验证器候选选择与置信度校准是主要增益来源，检索失败仍为主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 端到端提示将证据检索、推理与不确定性估计纠缠在一起，导致失败难以诊断、置信度难以信任，因此需要解耦的模块化流程来提升可解释性与可靠性。

Method: R2VC 包含四个模块：1) 在 Wikipedia 上的稀疏+稠密混合检索；2) 经监督微调与 DPO 对齐的生成器，产出多样化的结构化判定候选；3) 外部 NLI 交叉编码器进行基于证据的候选选择；4) 轻量级序列级校准器用于置信度估计与选择性弃答。骨干模型规模为 8B。

Result: FEVER 上准确率较基线提升 13.74%；消融显示验证器候选选择与置信度校准贡献最大——移除候选选择后准确率降至 76.24%，移除校准后 Brier 分数近乎翻倍至 0.161；对 250 个错误的人工分析表明检索失败（尤其错误实体证据）是主导瓶颈。

Conclusion: 模块化事实核查流程可在开放域验证中同时显著提升预测准确率与置信度可靠性，并具备可诊断性与支持引用、弃答的能力。
...
TL;DR: 该文报告了100例诵读录音的人工标注（348个评分单元、162个定位事件、10个组合标签），并用可执行评测器同时评估标签与词位置，比较朴素diff与生产级清洗/对齐基线，以及8次LLM编码代理试点运行的表现。


<details>
  <summary>Details</summary>
Motivation: 从ASR转写中核查诵读需要区分未解决的错误与重复、自我修正、开场套语和可接受的拼写差异，而这一区分任务的标注规范与评测方法此前缺乏系统研究。

Method: 完成100例录音的人工标注，定义10个组合标签，构建同时评分标签与词位置的可执行评测器；对比朴素diff与适配的清洗/对齐基线；再由3个编码代理、8个模型各运行单次20分钟，共8次试点。

Result: 朴素diff标签感知F1为0.525、定位F1为0.826；适配基线为0.518与0.786，两者精确跨度F1均为0.505。修正适配器词坐标后可召回全部5个重复事件。试点运行标签感知F1跨0.143–0.892，其中7次远超所有基线，1次因缺少归一化步骤低于朴素diff；972个金标事件实例中有970个获得重叠预测；162个事件中7个击败全部同日运行，其中5个属同一正字法规则，最强运行亦未命中。

Conclusion: 剩余难点并非事件检测，而是约定问题：跨度边界，以及由裁定而非文本可见所确定的标签；所有运行均在建构之前标注，因而试点仅衡量了任务的算法部分。
...
TL;DR: 提出弱监督框架，将通用文献上训练的轻量模型与前沿LLM结合，实现强迫流离失所与FCV领域文档的数据集抽取，无需大规模人工标注，在1706段金标准基准上达到74.1%精确率与70.5%召回率。


<details>
  <summary>Details</summary>
Motivation: 发展与人道组织产出大量数据资源，但其被引用情况散落在研究论文、项目文件、人道报告中，难以系统识别，既阻碍数据使用追溯，也难以发现数据供给或传播的缺口。而领域内缺乏大规模人工标注语料，直接训练抽取模型成本高。

Method: 弱监督标注框架：先用通用研究文献训练的轻量模型在无标注领域文档上生成候选数据集提及；再由前沿LLM在上下文中审核，验证或否决候选并修正抽取边界；将所得标注补充针对性合成样本与对比样本，用于微调轻量模型，实现大规模抽取。模型在涵盖研究、人道与运营文档的1706段独立金标准基准上评估。

Result: 整体基准上，提及级别精确率74.1%、召回率70.5%；在含数据集引用的段落中，精确率达89.5%。段落级别上，区分含与不含数据集引用段落的准确率为88.2%，特异度为88.6%。

Conclusion: 该框架为标注数据有限时构建领域专用监督提供了可行路径，并为更大规模分析流离失所数据格局中的数据使用及潜在缺口奠定技术基础。
...
TL;DR: 论文提出覆盖—压缩模型，将事实幻觉分解为未观测事实的覆盖缺失与已观测事实的有损压缩两个可分离误差源，并证明统一的误差下界。


<details>
  <summary>Details</summary>
Motivation: 主流观点将闭卷问答中的事实幻觉仅视为覆盖问题（相关事实不在模型内部记忆中）。作者指出这忽略了第二类误差来源：即便事实被观测过，有限记忆也迫使其只能被近似存储，即已观测事实的有损回忆。

Method: 构建简单的覆盖—压缩事实回忆模型：无结构问答任务含N个可能查询、K个可能答案；学习器观测M个训练事实，压缩进至多B比特，并在无检索条件下回答均匀抽取的测试查询。对均匀随机真值映射，推导误差下界并与理论驱动的仿真及现代语言模型上的受控事实注入探针相结合，变化事实负载与有效可训练记忆。

Result: 证得误差下界 E ≥ (M/N)δ*(B/M) + (1−M/N)(1−1/K)，其中δ*(r)为均匀K元信源在0-1损失下的率失真函数逆函数。第一项刻画已观测事实的压缩失真，第二项刻画未观测事实的覆盖缺失。仿真与受控事实注入探针在现代语言模型中复现了理论预测的特征。

Conclusion: 该工作并非幻觉的完整理论，而是对一种可分离失效模式的信息论刻画：有限记忆下已观测事实的有损回忆。下界为选择性记忆、强迫压缩、结构、检索、弃答与长上下文组织等提供了简洁的分析框架。
...
TL;DR: RIPPLE 通过将“编辑位置”与“组合后安全性”解耦，诊断失败轨迹并将修正限制在对应策略片段，同时经重放验证隔离增益与组合效应，仅保留组合后仍安全的编辑；在 Flow-HO 基准上验证成功率最高提升 23.1%。


<details>
  <summary>Details</summary>
Motivation: 提示策略编辑可在不更新底层模型的前提下改进可执行工作流合成智能体，但持久化编辑存在两个耦合特性：一是编辑局部性不等于效果局部性，局限于某策略片段的编辑会沿下游执行传播，改变该片段之外的行为；二是编辑效果具有组合敏感性，单独有效的编辑在组合后可能相互干扰，失去收益甚至变得有害。因此持久化自适应需支持两个独立决策：从执行反馈中定位策略应改动之处，以及判断该编辑在组合后是否仍安全可持久化。

Method: 提出 RIPPLE（Replay-Informed Persistent Policy Localization and Editing），将“编辑在哪里进行”与“组合后是否仍安全”分离。具体地：诊断失败轨迹，将每个可操作失败映射到预定义的策略片段，并把修正限制在该片段内；随后以同一迭代起始策略评估候选编辑，比较其孤立增益；再将有前景的编辑在先前已接受的更新之后重放，以暴露下游效应与交互作用；仅保留在组合条件下仍安全的编辑。

Result: 在可执行工作流合成的合成留出基准 Flow-HO 上进行评测，RIPPLE 将验证成功率提升最高达 23.1%，并在另外两个冻结语言模型骨干上取得正向增益，同时保持编辑效率与较低执行成本。针对性交互分析进一步验证了上述两个特性：一个片段局部的工具使用编辑改变了下游资源解析与验证，而一个孤立条件下有益��编辑在组合后变得有害。

Conclusion: 持久化提示策略编辑必须同时处理效果非局部性与组合敏感性；RIPPLE 通过定位—隔离评估—组合重放的两阶段决策流程，在保持编辑效率与低执行成本的同时显著提升工作流合成智能体的验证成功率，并具备跨骨干模型的泛化性。
...
TL;DR: 利用2.5万条Google Maps杂货店评论，通过无监督主题建模与零样本分类量化五维食品可及性，揭示地理邻近无法捕捉的障碍。


<details>
  <summary>Details</summary>
Motivation: 食品可及性是多维度的，但群体层面的测量仍严重依赖地理指标，因为感知维度难以大规模测量，需要可扩展的补充手段。

Method: 收集北卡罗来纳州罗利市49家杂货店的25,125条Google Maps评论；用无监督主题建模识别评论主题，再用零样本分类将主题归入食品可及性五个维度（可得性、可达性、可负担性、适配性、可接受性），与人工编码的一致率为85.4%；并与地理邻近、同连锁邻近门店、社会经济人口特征进行对比分析。

Result: 门店层面的测量捕捉到食品可及性的不同方面，揭示地理邻近无法反映的障碍；同一连锁的邻近门店即便政策相同，感知也差异显著；感知可及性呈现系统的社会经济与人口格局，与地理可及性大体平行但不完全重合。

Conclusion: 在线杂货评论可作为地理测量方法的可扩展补充，用于刻画感知层面的食品可及性。
...
TL;DR: 提出GAUGE离线评测协议，检验persona驱动的LLM用户模拟器+LLM评判的门控排名是否与可验证奖励一致，发现满意度与任务成功脱钩、近能力对排名分辨率丧失，建议"先校准后信任"并以无评判的完成位作为低成本哨兵。


<details>
  <summary>Details</summary>
Motivation: 面向任务的LLM智能体选型日益依赖低成本离线评测门控（persona驱动用户模拟器对话+LLM-as-a-judge打分，分高者胜出），但该门控的排名是否与真实可验证奖励一致、评测效度是否成立尚不明确，且实践中混淆了排名效度与构念效度两类评测有效性。

Method: GAUGE作为可复用离线协议：在τ²-bench与SimulatorArena两个基准上，对来自六家提供商的25个智能体，将门控排名与grounded可验证奖励进行对比；区分排名效度与构念效度；采用盲评小组对满意度与任务成功做交叉检验，覆盖五种评分者群体与全部主观维度，并统计宽奖励对与接近对上的决策分歧率。

Result: （1）满意度-成功缺口：满意度几乎不含任务成功信息，被判满意的对话与真实成功去相关，其中57.5%未完成客户任务，该模式在五种评分者群体、两个基准及所有主观维度上一致；（2）门控在宽能力跨度上排名稳健，但在实力接近的强智能体间丧失分辨率，决策分歧率从宽奖励对的<1%升至接近对的31%。

Conclusion: 该离线门控虽经人类验证却锚定错误（human-validated yet mis-anchored）。补救方案是"calibrate-then-trust"节奏：以无评判的完成位（judge-free completion bit）作为零成本的截断回归哨兵，先校准再信任排名。
...
TL;DR: 提出上下文增强训练框架，用于知识图谱多跳问答，通过附加支持三元组构建上下文图，并结合修复式SFT与强化学习提升多跳推理性能。


<details>
  <summary>Details</summary>
Motivation: 仅用孤立的知识图谱头-关系-尾三元组训练大语言模型，难以让模型学到多跳推理所需的上下文信息，限制了多跳问答能力。

Method: 提出上下文增强训练框架，并在胃轻瘫和糖尿病疾病知识图谱上验证。用GraphMERT抽取KG，为每个主三元组附加同一文本块中的支持三元组，形成上下文图（CG），构建KG-grounded与CG-grounded两种监督设置。使用Qwen3-14B进行SFT，得到KGModel与CGModel。引入LLM评判、历史感知的自适应修复流水线，识别未解决的一跳失败、持续微调修复样本并移除或隔离噪声三元组。最后使用低跳问答数据进行强化学习，并在3/4/5跳任务上评估泛化。

Result: 在两种疾病上，上下文增强监督相比仅KG监督持续提升多跳性能；修复后模型在清理保留的一跳验证集上达到100%准确率；从修复后的SFT检查点初始化RL，可获得更大且更稳定的增益。

Conclusion: 上下文增强监督能有效提升多跳问答表现，低跳修复与强化学习结合可进一步带来更稳定、更显著的多跳泛化收益。
...
TL;DR: 本文提出 Chopthin-Consensus Power Sampling (CCPS)，将 Chopthin 重采样器引入 LLM 推理的序贯蒙特卡洛 (SMC) 解码，并结合语义多数选择机制，在不训练的前提下提升推理准确率与搜索多样性。


<details>
  <summary>Details</summary>
Motivation: 现有基于 SMC 的推理时功率采样多采用等权重采样，会激进地剪除低权重轨迹，丢弃潜在正确的推理路径，并降低搜索空间的谱系多样性。

Method: 方法包含两部分：(1) 将 Chopthin 重采样器应用于 LLM 解码，不对权重做等化处理、避免不必要的粒子复制，而是对最大与最小权重之比施加上界，并保留非等权权重继续传播；(2) 采用语义多数选择机制，合并 token 完全相同的最终轨迹，对语义等价的答案进行聚类，返回被最多不同轨迹支持的答案。

Result: 在 3 个开源权重模型和 5 个推理基准上评测：Chopthin 在 15 个设置中有 13 个提升了 oracle 覆盖率；与语义多数选择结合后，CCPS 在 15 个设置中有 14 个达到或超过 Power-SMC 基线的最终答案准确率，绝对增益最高达 10.6 个百分点。理论上，该方法保持加权 SMC 近似在条件期望上不变，并保证重采样后有效样本量 (ESS) 有下界。

Conclusion: 保多样性的重采样与多样性感知的选择是互补机制，可共同支撑无需训练的 LLM 推理改进；代码已开源。
...
TL;DR: 本文提出一个基于Transformer的开放模块化框架，可在段落级别检测并结构化"环境社会临界点"证据，包含分词、检测、重写、评级与语义检索五个组件，并已部署为可视化系统。


<details>
  <summary>Details</summary>
Motivation: 气候文献增长速度超过综述团队的阅读能力，而环境社会临界点（small change triggers rapid, self-reinforcing change）的证据通常仅隐藏在长文档的一两段中。现有文本挖掘工具以整篇文档主题分类或孤立主张标注为主，缺乏对这类证据的系统性发现与组织方法。

Method: 构建模块化Transformer工作流：DistilBERT边界切分器用于段落分割；迭代增强的RoBERTa分类器用于检测；Mistral 7B重写检测到的段落以提升可读性；LLaMA 3.2 3B依据五项已发表的社会临界点标准进行评级；Milvus向量库实现语义检索；前端采用Streamlit，后端由MinIO对象存储支撑。

Result: 在GPT-4.1标注的163段基准测试上，切分器以9指标综合得分6.137超过三种对比方法；调优后的RoBERTa在完整基准上准确率71.4%、Cohen's kappa 0.337；在有标签段落上准确率87.5%、kappa 0.742，优于气候领域专用模型和未调优语言模型；另在专家复核的51段数据集上进行了评估。

Conclusion: 该段落级、多组件框架为气候文献中社会临界点证据的规模化发现与结构化提供了可部署方案；检测性能在完整基准与有标签子集间存在差距，提示标注覆盖度是关键因素。
...
TL;DR: 该研究构建统一生化知识图谱与550条酶源-罕见病终点路径基准，用六种大模型在四种信息条件下生成13,200条假设并按五准则评分，发现"极简信息"产出高分但证据根基弱，而"完整生物路径"驱动证据约束性推理。


<details>
  <summary>Details</summary>
Motivation: 大模型能生成生物医学假设，但是否真正从科学证据推理，还是仅生成听起来可信的观点，尚不明确，需要可控实验加以区分。

Method: 整合KEGG、Rhea、UniProt为统一生化知识图谱；构建550条连接酶源与罕见病终点的路径基准；六种LLM在四种信息条件下（仅源酶、完整生物路径、仅源与疾病终点等）生成13,200条假设；由专家制定五准则1-5分量表评分；并通过固定端点、打乱中间步骤的扰动实验验证因果性。

Result: 仅给出源与疾病终点时假设评分最高，但证据根基较弱；给出完整生物路径时假设与已知机制关系更一致。打乱中间路径步骤后证据根基显著下降（delta=-0.793, p<0.001），表明模型确实利用了路径结构。

Conclusion: 知识图谱以两种方式支持假设生成：一是识别文献中缺失的生物终点对，二是以其机制路径引导大模型在两端点间进行"证据约束性推理"。
...
TL;DR: 论文提出EAR（实体感知分块方法），以规范化表层锚点作为检索单元，在MMLU风格子集上以更少检索词量（减少37.5-40.2%）实现可比的准确率，贡献定位为方法学层面。


<details>
  <summary>Details</summary>
Motivation: RAG可提升知识密集型问答，但语料如何切分为可检索单元这一首要设计选择常被忽视；固定长度分块常返回与问题仅有隐含关联的长段落。

Method: EAR从问题、选项与语料中抽取规范化表层锚点，检索语料锚点周围的局部窗口，并可通过抽取式摘要附带更大的父级段落；在153道经自动语料支持启发式筛选的MMLU风格子集上，使用去污染公开教材文本，以Mistral、Gemma、DeepSeek在top-k=3与8下进行同协议评测。

Result: 实体窗口相比固定分块减少37.5-40.2%的检索词量；top-k=3时准确率变化为+5.2、+1.3、-3.9个点，top-k=8时为+5.9、-3.3、-4.6个点，均不具统计显著性。

Conclusion: EAR提供了紧凑且可检视的检索单元，属方法学贡献；其基于规则的锚点抽取器仍具领域特定性，迁移前需单独验证。
...
TL;DR: 本文首次大规模对比了蒸馏式字节模型与Token模型在扩展规律上的差异，提出两种Token logits转Byte logits的方法（近似Marginalize-It与精确End-Of-Token），发现在低FLOP阶段Token-1B占优但会饱和，而字节模型随算力增加反超并达到更高的下游性能上限，渐近情况下蒸馏End-Of-Token-1B最高可超蒸馏Token-1B 4%，且数据效率与存储效率显著更优。


<details>
  <summary>Details</summary>
Motivation: 蒸馏通常要求师生模型共享分词方案，但字节模型与Token模型在算力和数据增加时的扩展趋势是否一致尚不清楚。作者希望在同一框架下系统比较两种分词方案（Tokens、Bytes、Bytes w/ eot）与两种训练目标（蒸馏 vs. 交叉熵）的扩展行为。

Method: 引入两种将Token logits高效转换为Byte logits的变体：1）近似方法Marginalize-It；2）精确方法End-Of-Token。开展首个大规模过训练研究，使用decoder-only稠密Transformer，在层数与参数量匹配（约10亿参数）条件下，同时变化分词方案与训练目标两个维度，训练数据规模最高达1万亿字节。在涵盖多选题问答、语言生成、机器翻译三大类的八个基准上评估，并拟合平均top-1误差与验证BPB的扩展律进行外推。

Result: Token-1B模型在低FLOP区间优于字节模型（End-Of-Token-1B与Bytes-1B），但随后趋于饱和；字节模型起点较低，却随算力增加反超Token-1B并达到更高的下游任务性能上限。外推扩展律预测，渐近情况下蒸馏End-Of-Token-1B最高超出蒸馏Token-1B约4%。字节模型数据效率极高，仅用约六分之一训练数据即可匹配蒸馏Token-1B的性能。此外，因仅需256字节的小词表（而非约10万token），无需在logit转储时做top-k截断，logit存储开销降至约五分之一。下游性能扩展律预测，蒸馏End-Of-Token-1B模型渐近情况下在平均下游任务上分别超过Llama 3.2-1B、Gemma-3-1B-pt和Gemma 2B最高约6.5%、8.1%和2.1%。

Conclusion: 字节级蒸馏模型在足够算力下优于Token级蒸馏模型，具备更高的性能上限、更强的数据效率与更低的logit存储成本，其扩展律预测可超越同量级乃至更大规模的主流开源模型。这表明字节分词方案在大规模蒸馏场景中具有显著潜力。
...
TL;DR: 本文介绍了ESTS团队参加WMT26无约束模型压缩共享任务的六个提交系统，针对英译简中与英译埃及阿拉伯语方向，基于GPT-OSS-20B通过专家剪枝、恢复微调与MXFP4量化构建不同压缩操作点，并配套鲁棒推理系统。


<details>
  <summary>Details</summary>
Motivation: 在WMT26模型压缩共享任务中，需在保持翻译质量的前提下大幅压缩大规模模型，探索专家路由信息指导的剪枝与量化策略在不同翻译方向上的有效压缩配置。

Method: 以GPT-OSS-20B为基础模型，利用任务特定路由质量对专家排序，并用跨语言路由散度在层间分配保留容量，物理移除低重要性专家；对所得专家模型使用GPT-5.1生成的合成翻译数据进行恢复微调，并对保留的专家投影权重施加MXFP4量化。同时实现面向指令条件设定的鲁棒推理系统，包括类别推断、输出验证、重试、分段回退与源端JSON重构。

Result: 六个提交模型参数量在4.186B至7.770B之间，打包产物规模为4.55至6.33 GiB；使用GPT-5.1伪参考的内部xCOMET-XL评估给出了各压缩操作点之间的比较。

Conclusion: 结合路由引导专家剪枝、恢复微调与MXFP4量化的多操作点压缩方案，可在不同压缩率下为两个翻译方向提供可比较的模型配置，并配合鲁棒推理流程满足指令条件翻译任务需求。
...
TL;DR: 本文提出一种风格感知、提示驱动的大语言模型改写式匿名化方法，用于抑制文本中的作者身份可识别风格指纹，在博客与评论数据集上将作者归属F1降低60–70%，同时保持内容质量与可读性，优于差分隐私及非差分隐私基线。


<details>
  <summary>Details</summary>
Motivation: 作者归属模型可借助稳定的风格指纹，在显式标识符被移除后仍能重新识别用户，对文本发布与分析构成日益严重的隐私风险。该风险同样存在于语音转写文本（如会议与呼叫中心对话的ASR转写）中，即使经过声学匿名化，文体计量泄露依然可能残留。而基于差分隐私的匿名化方法往往严重损害文本质量与实用性，亟需一种能在隐私保护与文本效用之间取得更好平衡的方案。

Method: 提出一种风格感知、提示驱动（prompt-driven）的匿名化方法：利用预训练大语言模型，从少量样本中构建紧凑的风格画像（stylistic profile），并据此对文本进行改写，以抑制可识别的风格标记，同时保留原始语义。

Result: 在博客与评论数据集上，该方法使作者归属F1下降60–70%，同时保持内容质量与可读性，显著优于基于差分隐私（DP）的基线及非DP基线。

Conclusion: 利用大语言模型的风格感知提示驱动改写，可在不显著牺牲文本质量的前提下有效削弱文体计量指纹，为文本发布、文本分析以及声学匿名化后的ASR转写等场景提供了一种优于差分隐私基线的实用匿名化途径。
...
TL;DR: 提出 SynthSentry：一种语料级、模型无关的合成数据污染检测信号，无需生成模型访问、生成历史或合成标签，用于训练前筛查未知来源语料。


<details>
  <summary>Details</summary>
Motivation: 大模型递归训练自身或其他模型输出会导致模型崩溃（分布尾部与事实准确性退化但流畅度保留）。既有工作多在训练后诊断崩溃，而真正可操作的问题是：在训练前对来源未知的语料进行筛查。

Method: SynthSentry 为语料级、模型无关的污染信号，不需要生成模型、生成历史与合成标签。得分基于三类统计量的分布散度：词汇多样性塌缩、n-gram 尾部截断、跨参考模型的困惑度方差。评估采用留一生成器协议，在由小型开源生成器与一个指令微调开源模型污染的语料上测试；并以领域分层研究衡量自然重复性人类文本（法律、临床、源代码）上的假阳性。

Result: 该得分可按严重程度对语料排序，且在整类生成器家族被留出时性能损失很小。使用协方差收缩与自助法阈值替代朴素分位数后，各领域校准接近名义假阳性预算（朴素分位数超标约四倍）。下游微调检查在本实验规模下未发现由污染导致的准确率缺口，因此剪枝能否恢复准确率仍待确定；同一次运行显示当剪枝比例超过真实污染比例时存在过度剪枝风险。

Conclusion: 将筛查定位为数据治理防御手段而非事后诊断，并开源评分工具包。所有结果均为小规模；范围限于英语、批处理式语料筛查。污染来源为单代生成或人工撰写，而非递归生成，故结论适用于一般合成污染，不涉及递归深度。
...
TL;DR: CueMem：将记忆记录视为检索线索而非自足证据，通过线索定位源对话轮次并在对话图上扩展，重建紧凑的证据上下文以支持长期对话问答。


<details>
  <summary>Details</summary>
Motivation: 长期对话智能体需从冗长历史中回忆信息作答，但直接用全量历史成本高且不可靠，而压缩后的记忆单元又可能丢失问答所需的细粒度证据。受自传体记忆的"重构观"启发，作者希望兼顾低开销与证据保真度。

Method: 构建阶段：从对话轮次中抽取细粒度记忆线索（cue），并将每条线索与其源轮次建立链接。查询阶段：检索与查询相关的线索，映射到源轮次锚点，并在同时刻画时间邻近性与语义相关性的对话轮图上从锚点扩展，从而从原始对话中重建紧凑的证据上下文，供LLM生成答案。

Result: 在 LoCoMo 与 LongMemEval 两个基准上，CueMem 持续优于代表性长期记忆基线。进一步分析表明，基于图的上下文重建有助于找回支撑性对话证据，同时相比全量历史的LLM设置减少了查询时的输入 token 数与延迟。

Conclusion: 检索线索可作为自足记忆证据的有效替代方案，用于长期对话问答，兼顾证据可恢复性与推理效率。
...
TL;DR: ORQA 将 O*NET 职业与可信的行业专属网站相连，自动加人工审核生成可溯源问答对，评测 15 个模型的职业级知识。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么通过任务定义把抽象的 LLM 技能映射到职业，要么依赖难以规模化获取且昂贵的专家知识，均存在局限，因此需要一种可扩展、可溯源的职业级知识评测方法。

Method: 连接 O*NET 职业与可信的行业专属网站（监管机构、执照颁发机构、专业组织、政府出版物），将其转化为可溯源的问答对；采用自动化流水线加人工审核生成高质量职业问题；据此评测 15 个前沿与开放权重模型，并考察开放式问题与按工资总额加权的影响。

Result: 问题集覆盖 SOC 全部 21 个大类的 116 个职业，480 道题来自 187 个网站。Claude Opus 4.6、GPT-5.4、Claude Sonnet 4.6 表现最佳，约 58–62%；较小的开放权重模型约 33–41%。职业间差异显著：医疗健康类最高（78%），办公与行政支持约 40%；个别职业（如钣金工、渔猎监督员）几乎为零。开放式问题与按工资总额加权均未显著改变模型排名。

Conclusion: 利用现有可信的行业专属信息评测 LLM 在专业领域的知识，可能是未来评估职业级 AI 表现的一种可扩展且有价值的方法；结果与数据见 orqabench.org。
...
TL;DR: GraphProfiler 将用户历史帖子构建为可溯源的个人知识图谱，使 LLM 属性推断可审计、可引用证据，在保持高攻击成功率的同时支持精准隐私缓解。


<details>
  <summary>Details</summary>
Motivation: 年龄、收入、职业等敏感属性可通过聚合大量普通帖子中的间接线索被推断，LLM 画像器能高精度自动化完成这一过程，构成大规模隐私威胁。但现有 LLM 画像器难以说明是哪些具体帖子、概念和关系导致了推断，而这正是精准隐私缓解（只删改真正泄露属性的少数帖子，而非扰动整个历史）的关键。

Method: 提出 GraphProfiler——一种可审计的 LLM 画像器：将每个用户的帖子历史表示为源头可追溯的个人知识图谱，节点与边均可回溯到原始帖子，并将属性预测归结为有引用的图谱记录与源文本。

Result: 在八属性 SynthPAI 基准上攻击成功率达 86.7%，与强纯文本基线相差不到两个百分点；在 PANDORA 上为 84.6%；对超过 98% 的预测给出支撑证据。受控消融实验显示，删除被引用的帖子比删除等量随机帖子使攻击成功率下降显著更多，说明这些帖子确实对攻击成功有贡献。

Conclusion: GraphProfiler 在几乎不损失推断性能的前提下实现可审计、可溯源，为针对性的隐私缓解提供了细粒度证据基础，并验证了被引用帖子与推断结果之间的因果性关联。
...
TL;DR: 提出ASCIL——一种后ASR自适应自校正推理层，通过融合声学、语言、设备上下文和历史误判模式，对唤醒意图进行二次判定，在线更新无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 误唤醒是会话AI的持续难题：与唤醒词语音相近的语句可生成语法语义合理的ASR文本，导致助手错误执行。现有系统多为孤立单次意图决策，缺乏从重复错误中学习及个性化适应的机制。

Method: ASCIL作为后ASR校正框架，在响应生成前重新评估唤醒意图，融合声学嵌入、语言线索、设备上下文与历史误分类模式；将犹豫、脱离、静默等隐式信号以及取消、重复等显式信号作为自动推断的噪声行为指标，驱动在线模式更新（无需人工标注；离线评估用的有意/无意参考标签由人工标注），在推理时施加校正并与自然语言执行并行持续更新。

Result: 在包含3,667条交互、14类声学与上下文条件、带人工标注参考标签的私有数据集上：在由基线失败构建的会话不相交子集上实现54.27%的相对错误率降低；在问题标记评估切片上，阈值0.90处相对错误率降低最高达24.39%；同时提升有意唤醒接受率，报告中位附加延迟低于60 ms。

Conclusion: ASCIL以低成本、低延迟的方式显著降低误唤醒错误并提升正确唤醒接受率，验证了反馈驱动、在线自校正与个性化学习在唤醒意图判定中的有效性。
...
TL;DR: 提出AMDKernelVault开源语料库与智能体流水线，面向AMD CDNA GPU，通过SFT与执行感知强化学习训练Qwen3-8B，在PyTorch-to-HIP等基准上取得最高正确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的内核智能体大多以CUDA/NVIDIA为中心，且常依赖反复调用前沿大模型进行生成、反思与优化，缺少面向AMD ROCm生态的语料与训练框架，存在明显空白。

Method: 构建AMDKernelVault开源HIP与Triton内核语料库及训练框架；开发HIPKernelGen与TritonKernelGen两条智能体驱动流水线，将PyTorch参考实现转换为HIP或Triton内核，在ROCm下编译验证，并在AMD硬件上进行延迟剖析；随后用监督微调与执行感知强化学习训练Qwen3-8B作为语料库可用性验证。

Result: 语料库包含62,153个经执行验证的HIP内核样本、2,377条基于生产环境的ROCm Libraries问答条目及39,893个Triton内核。在固定评测预算下，PyTorch-to-HIP达34.0% Pass@1，TritonBench-G达33.2% Corr@3，ROCmBench达41.94% Corr@3，均为对比模型中最高正确率；但在编译或速度指标上并非全面领先。

Conclusion: 该工作填补了AMD平台内核智能体语料与训练资源的空白，验证了所构建语料库在提升PyTorch到HIP/Triton内核生成正确性方面的效用；语料与文档已开源，训练及内核生成代码同步公开。
...
TL;DR: 本文提出ZipBench，一种低成本基准压缩方法，通过少量锚点LLM与伪评估结果合成选出代表性样本子集，并提供误差与排序一致性理论保证；据此构建覆盖100+基准的ZipBench Zoo，与完整基准的平均绝对误差仅0.002–0.02、Spearman相关约0.98。


<details>
  <summary>Details</summary>
Motivation: 现有基准套件冗余导致LLM评估成本高昂；虽已有基准压缩方法（BCM）可缓解，但许多强方法依赖大量LLM在全部样本上的逐样本评估结果，构建这种集合本身代价很大，除非已公开，因此难以扩展到新发布的基准。

Method: ZipBench仅评估少量锚点LLM，合成伪评估结果以扩大覆盖范围，学习紧凑的样本表示，并据此选出小而具有代表性的样本子集；方法附带误差与排序一致性理论保证。基于该方法构建ZipBench Zoo，覆盖文本、多模态与智能体任务的100+基准的紧凑版本。

Result: ZipBench Zoo中的紧凑基准与完整基准相比，平均绝对误差为0.002–0.02，平均Spearman相关系数约为0.98。

Conclusion: ZipBench同时降低了LLM评估与紧凑基准构建的成本，降低了算力受限研究者开展广泛LLM研究的门槛，代码已开源。
...
TL;DR: 该论文提出基于LLM的交互式小说系统HWAMW，通过让用户讲故事并生成不同文学风格的"窗口"重述，帮助用户探索个人故事的多重意义，实证表明其能提升叙事认同感。


<details>
  <summary>Details</summary>
Motivation: AI辅助写作容易使人类叙事意义扁平化，产生同质化输出，缺乏写作应有的意图性与意义建构努力。为应对这一挑战，作者希望探索LLM帮助用户挖掘个人故事广度与深度的可能。

Method: 作者设计了HWAMW（The House with a Million Windows），一个基于LLM的交互式小说系统，借鉴心理学中的"restorying（叙事重构）"干预范式。用户在文本叙事中讲述一个故事，随后会遇到一组LLM生成的"窗口"，按不同文学风格对其故事进行重新框定。

Result: 实证证据显示HWAMW提升了用户的叙事认同感（narrative identity）；专家评审进一步探讨了这一效果的实现机制。

Conclusion: HWAMW促进了restorying，并为AI辅助写作提供了一个有价值的范式：LLM并非替我们讲故事，而是帮助我们在自己所讲的故事中看到更大的可能性。
...
TL;DR: 提出 Agent as Policy（AGP）框架，让通用智能体无需任务或环境特定训练，直接驱动物理机器人完成操作任务。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作通常依赖任务特定或环境特定的训练，泛化能力受限。作者希望验证通用智能体能否凭借推理与编程能力直接充当机器人策略，将自主性扩展到物理操作领域。

Method: 将任务规划与执行交由智能体控制：在给定任务和机器人接口的条件下，智能体解析视觉证据、编写可执行程序、下发运动指令，并根据物理反馈修正动作，使推理与编程能力与物理世界持续交互。在多种真实操作任务上验证，涵盖精密操作、动态运动与可变形物体，包括从人类视频学习装配、依据目标图像搭建积木、骰子重新定向、定向投掷及双臂毛巾折叠。

Result: 在三种积木搭建配置下分别取得 100%、100% 和 80% 的成功率。

Conclusion: 研究确立了通用智能体作为机器人策略的可行路径，通过运行时推理、编程与交互将其自主性延伸至物理操作。
...
TL;DR: 本文提出Meddies-PII-Dataset，一个覆盖17种语言、9类PII标签的百万级合成临床文档语料库，配套BIOES模型Meddies-PII-Model在15个外部基准上平均F1达0.827，优于最强基线0.658。


<details>
  <summary>Details</summary>
Motivation: 临床去标识化依赖对个人可识别信息（PII）的准确识别，但人工标注数据构建成本高，现有合成替代方案对生成过程的描述有限，且合成策略较为简单。

Method: 构建Meddies-PII-Dataset：通过属性条件化提示（attribute-conditioned prompts）生成100万条合成临床文档，覆盖17种语言与9类PII标签，并经13道确定性门控（deterministic gates）校验结构与标注一致性；随后训练BIOES序列标注模型Meddies-PII-Model，并采用实体级精确匹配F1与现有PII抽取系统对比。

Result: Meddies-PII-Model在所有报告基准上取得最优性能，在15个外部基准上平均F1为0.827，而最强基线为0.658。

Conclusion: 该工作将公开数据集、基准套件、模型、生成框架与评测代码，以推动多语言临床去标识化研究。
...
TL;DR: 该论文提出"校准歧义"概念，基于Dixit桌游构建任务与编码量表，比较人类与多模态语言模型生成的线索，发现模型存在"歧义坍缩"与"文化扁平化"。


<details>
  <summary>Details</summary>
Motivation: 歧义通常被视为AI系统需要消除的缺陷，但在人类交流与文化中（如幽默、政治、艺术），歧义是一种生成性资源：表达既足够开放以容纳多种解读，又足够受约束以保持可解释性。作者希望将这一"校准歧义"能力操作化，检验AI是否具备此类人类式的模糊表达能力。

Method: 借鉴派对游戏Dixit设计任务，并构建一套用于衡量"校准歧义"的新型编码量表；在此框架下对比人类与多模态语言模型所生成线索的差异，同时考察在提示使用典故和比喻性语言时模型的表现。

Result: 模型一致表现出"歧义坍缩"，即输出过度明确、未给多种合理解读留下空间；与人类线索不同，AI生成线索还表现出"文化扁平化"，即便被提示使用典故与比喻，也几乎不引用具有文化情境的知识。

Conclusion: 校准歧义是当前多模态语言模型的系统性短板：模型倾向于消解而非利用歧义，并缺乏文化情境化的表达能力。这提示未来研究需在评估与训练中重视校准歧义及文化嵌入的语言生成能力。
...
TL;DR: 提出SWARM数据集（2183条、9种语言、多站点类型的搜索结果，人工标注是否支持俄宣传叙事），并对比黑名单、监督分类器与零样本LLM，发现内容级检测更有效，LLM最优（正类F1=0.73）。


<details>
  <summary>Details</summary>
Motivation: 现有计算研究多局限于单一社交平台、一两种语言，且侧重信息源而非内容本身，难以刻画跨语言、跨站点的宣传传播。

Method: 构建SWARM数据集：2,183条搜索引擎结果，覆盖9种语言与新闻、博客、政府站点等多元域名，由受训标注者逐条判定是否支持反复出现的俄宣传叙事；以该标注为基准，评测基于来源的黑名单、监督分类器与零样本LLM。

Result: 黑名单漏检大量支持宣传的文档，因其亦出现于主流媒体；内容级分析有效但依赖模型：最强LLM正类F1为0.73，监督分类器约0.5，较小LLM过度预测支持，将话题相关性误判为立场认同。

Conclusion: 检测搜索渠道传播的宣传须进行分语言、内容级评估，SWARM与配套评测代码可支撑此类研究。
...
TL;DR: 提出LifeMem，一种面向LLM智能体的终身学习框架，通过聚类交互轨迹提取可复用技能，在10个环境、13k+任务上实现跨环境经验迁移并缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有基于记忆的LLM智能体难以将可复用经验跨环境迁移，且随着经验积累会出现灾难性遗忘，无法持续适应新任务与环境。

Method: LifeMem在终身学习过程中，基于底层工作流对累积的交互轨迹进行聚类，从中提取可复用技能；推理求解新任务时，召回相关技能与轨迹以指导动作。

Result: 在10个环境、超过13k任务及2k条新标注交互轨迹上开展实验，LifeMem实现了有效的经验复用，在学习过的任务上遗忘更少，并具备更优的跨任务迁移能力。进一步分析表明，任务流式输入会影响学习效果，而在记忆中整合结构相似的轨迹可提升性能。

Conclusion: LifeMem通过轨迹聚类与技能提取、技能召回机制，有效支持LLM智能体的终身学习，兼顾抗遗忘与跨任务迁移，且记忆中的结构相似轨迹整合是关键增益因素。
...
TL;DR: 本文提出固定范围切分（FRC），通过动态规划将文档切分为预设长度区间内的块，并在训练与推理中一致应用，以缓解训练-推理长度不匹配问题，提升长上下文LLM的文档级机器翻译质量。


<details>
  <summary>Details</summary>
Motivation: 长上下文LLM虽可减少文档级机器翻译的输入截断，但直接Doc2Doc翻译易出现n-gram重复与质量渐进退化。细粒度分块是常见补救手段，但传统规则化分块无法处理训练与推理之间的长度分布不匹配。

Method: 提出Fixed-Range Chunking（FRC），利用动态规划将文档切分为落在预定义长度区间内的块，使任意长度输入文档映射到相同长度分布；在训练和推理中一致应用FRC。以FRC为核心，提出轻量级双边界匹配算法用于块对齐，并设计四种训练策略。

Result: 实验表明，基于FRC的微调显著优于直接Doc2Doc微调，并在IWSLT2017上超越现有DocMT方法；构建了涵盖10种语言、独立于主流DocMT训练来源的GlobVDoc测试集，证明FRC可改善分布外文档翻译。

Conclusion: FRC通过统一训练与推理的块长度分布，有效缓解长度不匹配，配合块对齐与训练策略，可显著提升7B LLM的文档级翻译表现及分布外泛化能力。
...
TL;DR: 提出CoG，一种受认知启发的无训练框架，通过计划-探索-反思循环实现图与文本深度双向协同的自适应知识探索，在七个多跳QA基准上显著超越SOTA并提升探索效率。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法在面向全局异构知识库（大规模知识图谱与文本语料）进行复杂推理时，多采用反应式、图驱动的探索策略，盲目遵循图拓扑，无法适应问题上下文与探索进展，且图与文本之间缺乏深度双向协同。

Method: CoG受人类问题求解启发，采用无训练的连续"计划-探索-反思"循环：主动制定调查计划、执行双源检索、动态反思进展并调整策略；并建立结构化图与非结构化文本之间的深度双向协同，由文本抽取的实体动态引导图探索以弥补知识缺口。

Result: 在七个多跳问答基准上，CoG显著优于现有最先进方法，同时实现了更优的探索效率。代码与数据集已开源。

Conclusion: CoG以认知启发、免训练的方式实现自适应知识探索，验证了主动规划与图-文本双向协同在复杂推理中的有效性。
...
TL;DR: 本文提出首个同时覆盖完整人设与场景、三种全双工行为、人格对齐情感语音表达以及剧本感知音效四个维度的合成口语对话数据集 DuplexDrama，通过4阶段流水线构建超2000小时音频，并将发布6400段双语对话子集。


<details>
  <summary>Details</summary>
Motivation: 现有全双工口语对话研究缺乏同时具备人设场景、全双工行为、情感表达和音效事件的多维度数据支撑，制约了全双工对话模型的训练与研究进展。

Method: 采用4阶段构建流水线生成 DuplexDrama 数据集，覆盖四大维度：(i) 完整人设与场景设定；(ii) 三种全双工行为（打断、反馈、不完整话语）；(iii) 与人设情感标签对齐的表达性语音；(iv) 剧本感知的音效事件，并对脚本与合成音频进行质量验证。

Result: 生成超过2000小时音频，包含64种音色池，覆盖13种人设与5个年龄段；3.8%的对话轮次至少含一种全双工行为；数据已通过内部全双工模型训练验证；将发布6400段双语对话子集（800小时，中文约500小时、英文约300小时）。

Conclusion: DuplexDrama 是首个多维度合成口语对话数据集，经脚本、音频及模型训练三重验证，将为全双工口语对话模型研究提供重要数据基础，样本与 LLM 评估提示将随数据集一并公开。
...
TL;DR: 提出三阶段训练流程（跨语言对齐、关系知识蒸馏、对比微调），无需人工相关性标注，仅依赖强嵌入模型与重排序器作为教师生成监督信号，开发出支持8192 token的波兰语检索器家族PolDense（17M–1B）和多语言检索器EuroDense（435M，9种欧洲语言），在41个波兰语和150个多语言检索任务上取得优异的质量-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 稠密检索系统日益依赖数十亿参数的语言模型，其内存与计算开销使大规模索引、频繁语料更新和低延迟服务成本高昂。因此需要开发紧凑高效、同时能与大得多的模型竞争的检索器。

Method: 三阶段训练流程：1）跨语言对齐；2）关系知识蒸馏；3）对比微调。整个流程不使用原始真值相关性标签，完全依赖强嵌入模型和重排序器作为教师生成的监督信号。基于该流程训练PolDense（6个波兰语检索器，17M–1B参数）与EuroDense（435M参数，支持9种欧洲语言），上下文长度均达8192 token。

Result: 在41个波兰语任务和150个多语言检索任务上评测：PolDense-1B优于所评测的参数量最高达9B的检索器；PolDense家族在各模型规模上构成帕累托前沿；在参数量低于1B的评测模型中，EuroDense在任务平均与语言平均性能上均排名第一，并在9种语言中的7种上领先。

Conclusion: 该三阶段、无真值标签的蒸馏式训练流程能够产出紧凑且高效的稠密检索器，实现显著优于其参数规模的质量-效率权衡，并支持长上下文与多语言场景；所有模型已公开释放。
...
TL;DR: 提出 DualMLC 双分支框架，用自回归解码器语言模型与双向编码器分别建模同一文档、各自在共享标签空间上打分，再通过后融合（late logit fusion）整合，在三个大规模多标签文本分类基准上取得 SOTA。


<details>
  <summary>Details</summary>
Motivation: 大规模多标签文本分类需从数千至数万候选标签中为每篇文档分配少量相关标签。尽管预训练语言模型提升了语义表示，但现有基于表示的方法多将预测流程集中于单一主编码器，或将辅助特征合并进单一排序器，异质语言模型之间的互补性未被充分挖掘。

Method: DualMLC 采用双分支结构：将同一文档分别输入（1）自回归 decoder-only 语言模型与（2）双向编码器。每个分支保持独立的表示通路，并在共享标签空间上独立估计相关性分数；随后通过 late logit fusion 将两个分数向量融合，使共享证据强化相关标签，而分支特有证据补偿另一分支表示的不足。

Result: 在三个广泛使用的大规模多标签文本分类基准上达到 SOTA。消融实验进一步证实，融合异质预测器所得到的排序效果优于任一单分支。

Conclusion: 异质语言模型（自回归解码器与双向编码器）在标签层面具有互补性，双分支独立表示加后融合是一种有效的大规模多标签文本分类范式；代码已开源。
...
TL;DR: 该文考察梵语在子词分词下的信息密度是否真的"逐词更高"。以"命题"为单位而非词为单位度量后发现：与匹配的英语对照分词器相比，梵语在32k、64k词表下每命题token数均高于英语，优势随词表增大而收敛；真正稳健的结论是部署分词器下梵语每命题耗费1.831–2.899个英语token。


<details>
  <summary>Details</summary>
Motivation: 梵语通过词尾融合格、数、人称、时态，并用复合词串联分句，因而"每词"信息密度高。但这一密度能否在子词分词（subword tokenization）后保留，是一个独立问题，且应按"意义单位"而非"词"来考察。已有对比常以部署的英语分词器为基准，可能高估梵语的token效率，故需要匹配算法与词表的对照实验来厘清密度差异的真正来源。

Method: 在FLORES-200 devtest 相同内容上比较梵语与英/印地语的token数（词表≥200,019 id）。设置匹配英语对照：相同BPE算法与相同词表规模、在相同语料英语侧训练，词表规模取32k、64k、128k，构成8个配对臂，每臂均与配对匹配对照和字节匹配对照比较，报告95%区间。以"每命题token数"为度量，并将比率分解为字符长度比与每字符token数比两个因子；同时比较SLP1转写下梵语散文与诗体的字符长度。

Result: 梵语在FLORES上耗英语token的1.774–2.187倍，但仅为印地语token的1.325–1.353倍。对部署英语分词器，梵语训练的BPE在当代散文上每命题看似比英语便宜（0.887）。但与匹配英语对照相比该反转消失：32k与64k下全部8个配对（每臂对配对匹配与字节匹配两种对照）在散文上均高于1.0，且95%区间排除1.0。词表增大后差距收窄：128k时BPE配对在域内为0.983，域外仍高于平价（1.025），FLORES上为1.116。比率可分解为字符长度比与每字符token数比，后者全程接近1，说明匹配分词下真正保留的是字符级长度——梵语散文在SLP1下相对英语并无优势（1.028），诗体才有（0.596）。稳健结论针对部署实践：当代散文与FLORES上，SLP1梵语对部署o200k英语枢轴，每命题耗1.831–2.899个英语token。

Conclusion: 所谓梵语"token更省"多源于与未经匹配的英语分词器比较的假象；在算法与词表匹配后该优势消失或反转，随词表增大而收敛。存活下来的并非语素融合带来的语义密度，而是字符级长度差异，且仅在诗体中显著。对实际发布的分词器而言，梵语每命题成本显著高于英语。
...
TL;DR: 二元选择真值基准存在表层特征泄漏问题：模型可绕过推理、仅凭答案的表面特征超随机水平作答。作者证明该泄漏可被检测并利用，提出Audit-Prune机制清洗数据，发布泄漏降至接近随机的TruthfulQA版本。


<details>
  <summary>Details</summary>
Motivation: 二元选择真值基准要求模型在正确与错误答案间二选一，但若两者在表层特征上系统性差异，模型无需进行目标推理即可超越随机猜测，导致评估结果虚高、无法真实衡量推理能力。作者希望揭示并解决这一失效模式。

Method: 1) 在TruthfulQA上训练一个仅用六个特征逻辑回归分类器，验证其区分正确答案与错误答案的能力，从而检测表层特征泄漏；2) 检查其他基准中是否存在类似的表层伪影；3) 提出通用清洗机制Audit-Prune，通过移除最强化泄漏的答案对来净化数据。

Result: 简单的六特征逻辑回归分类器在TruthfulQA上能以较高准确率区分正确与错误答案，表明泄漏可被下游分类器检测并利用；其他基准中也存在类似的表层特征伪影。

Conclusion: 表层特征泄漏是二元选择真值基准中的普遍且可利用的缺陷。Audit-Prune机制能有效降低泄漏，作者发布了泄漏接近随机水平的TruthfulQA版本，并建议数据集在发布前应先行清洗。
...
TL;DR: 本文提出TAM基准，用于评估长程程序性推理；在ICD-10-CM编码与联邦量刑任务上，GPT-5最佳精确匹配率仅为1%和15.5%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM多跳推理基准多为短程，仅需少量检索或推理步骤，缺乏对真实长文档、复杂互依规则任务可靠性的证据。

Method: 构建TAM基准，从ICD-10-CM临床编码和美国联邦量刑两领域精选真实任务，配合人工验证标签；每项任务需依据含数万条规则的权威手册，跨章节执行一系列相互依赖的步骤以得出精确答案。评估通用提示方法，包括检索增强生成、ReAct式提示及智能体框架基线，在GPT-5上进行测试。

Result: 最佳精确匹配表现极低：ICD-10-CM编码1%，量刑任务15.5%。

Conclusion: 当前基准可能高估LLM推理能力，并忽略了关键挑战——可靠遵循长程、基于规则的流程。TAM数据与代码已公开。
...
TL;DR: 提出Kraken语音到语音翻译模型，通过低比特率单层VQ token、SSL特征重建与源语音条件化解码器Autowave-X，基于Qwen3-8B训练，在翻译质量与非语言信息迁移上优于SeamlessM4T-Large v2和Qwen2.5-Omni。


<details>
  <summary>Details</summary>
Motivation: 现有基于语音LLM的S2ST面临两大问题：一是在LLM中预测高比特率语音token困难；二是依赖说话人身份与韵律理想对齐的S2ST训练数据，数据约束强。

Method: 1) 采用基于单层矢量量化（VQ）的低比特率token，并训练其重建自监督学习（SSL）特征；2) 使用独立的token到波形解码器Autowave-X，并以源语音为条件以增强非语言信息迁移，从而放宽训练数据约束；3) 在此基础上构建Kraken模型：在预训练LLM上增加语音特征输入与低比特率token输出，后接Autowave-X声码器。模型基于Qwen3-8B，使用15万小时多语言、多任务语音数据训练。

Result: Kraken的翻译质量优于SeamlessM4T-Large v2与Qwen2.5-Omni，同时说话人迁移与韵律迁移能力均有提升。

Conclusion: 低比特率VQ token结合源语音条件化解码器可有效缓解语音LLM在S2ST中的token预测与数据对齐难题，实现更优的翻译质量与非语言信息保持。
...
TL;DR: 本文提出ESRL框架，将MoE模型的专家路由作为强化学习中的额外探索维度，通过受限扰动的路由探索与路径重放提升LLM后训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有MoE强化学习研究主要集中于优化稳定性与训练效率，并将专家选择视为固定组件。但路由决定稀疏计算路径进而影响输出分布，因此专家选择本身可作为 rollout 多样性的额外来源。作者通过实证发现：扰动专家路由能改变模型输出、增加 rollout 多样性（类似于提高解码温度），然而直接扰动会激活不合适的专家并显著降低 rollout 质量。因此需要一种既能利用路由多样性又保持可靠性的探索方法。

Method: 提出了Expert-Space Exploration Reinforcement Learning（ESRL），一个架构感知框架，显式探索MoE模型的专家路由空间。具体做法：1）保留高置信度专家作为锚点，将随机路由限制在合理候选池内，以维持可靠计算路径；2）根据路由器熵自适应调整扰动强度，避免过度扰动；3）记录 rollout 期间使用的专家路径，并在策略优化阶段重放，以缓解扰动带来的路由不匹配。

Result: ESRL在top-K、top-1和共享专家路由的多种MoE骨干上，以及数学、科学、代码任务上均取得最佳性能，且无额外采样或计算开销。在Qwen3-30B-A3B上，ESRL优于所有对比方法，平均Pass@1和Pass@8分别比GRPO提升3.2和4.5个百分点。对专家利用率和训练动力学的进一步分析揭示了利用MoE特有路由结构对RL训练的益处。

Conclusion: 将MoE专家路由空间作为RL的显式探索维度是有效且高效的，通过锚点保留、候选池约束、熵自适应扰动和路径重放，可在不增加采样与计算成本的情况下提升多个MoE架构与任务上的性能，并从专家利用与训练动力学角度提供了机理洞察。
...
TL;DR: 提出Duplex Cue评测框架，度量全双工语音智能体在话轮内的适应能力，发现PersonaPlex在协作性线索下的适应率显著低于人类。


<details>
  <summary>Details</summary>
Motivation: 现有全双工评测多采用二元判断（继续说话或停止），无法表达人类常用的第三种反应：在继续说话的同时吸收听者刚提供的贡献（补词、纠正或澄清）。因此需要新的评测维度来刻画这种"话轮内适应"。

Method: Duplex Cue将听者意图（附和、协作、打断）与说话者行为（不变继续、话轮内适应、让出话轮）解耦，其中适应涵盖确认与内容修订。用300条来自无脚本英语对话、经人工确认的线索做单模型案例研究，将人类真实回应与在回放听者音频条件下生成的PersonaPlex续说进行对比；保留208对满足线索起始时说话者活跃且两种条件均可评分的样本。

Result: 在66对协作性样本上，人类说话者有68.2%做出适应，PersonaPlex为34.8%；模型其余情况为不变继续（42.4%）或让出话轮（22.7%）。

Conclusion: 评测自然语音交互不仅需测量智能体是否继续说话，还需测量其如何回应听者的贡献；Duplex Cue为此提供了可操作的评测方式，并揭示当前模型在话轮内适应上的明显不足。
...
TL;DR: 提出SAS（Simple Attention Sparsification），一种门控稀疏注意力机制，通过将选择器的连续分数注入注意力logits，用语言建模损失端到端优化上下文排序，在多种任务上优于可训练稀疏注意力基线，紧预算下增益尤为显著。


<details>
  <summary>Details</summary>
Motivation: 现有训练后注意力稀疏化方法使用轻量选择器打分并配合硬Top-K选择，但硬选择阻断了语言建模损失的梯度，因此这些方法通常只能蒸馏逐层的稠密注意力分布。这使得选择器按原模型中的稠密注意力权重排序，而该排序与固定注意力预算下各上下文单元对预测的实际影响并不直接对齐，可能把有限预算浪费在用处较小的单元上。

Method: 提出SAS（Simple Attention Sparsification），一种门控稀疏注意力机制，用语言建模损失端到端优化上下文排序。核心思想是在训练时将选择器的连续分数注入注意力logits，使损失能通过标准反向传播更新选择器。关键设计选择包括：将门控以log形式置于注意力softmax内部；使用归一化的softmax门控，以将历史上下文与始终保留的当前块进行校准；保留连续的选

Result: 在推理、长上下文理解与智能体任务上，SAS在各种注意力预算下均持续优于可训练的稀疏注意力基线，在紧预算条件下增益尤其大，表明其能更有效地为下游任务进行上下文排序。

Conclusion: SAS通过门控软化的设计，将选择器的连续分数纳入注意力计算，使排序直接对齐下游预测目标，从而实现更有效的上下文排序；配套的高效Triton内核支持长序列训练，验证了该简单设计的实用性与优越性。
...
TL;DR: 本文将组合泛化划分为词汇泛化与结构泛化，提出结构泛化更难并非Transformer固有属性，而是由数据集类型多样性差异所致，并通过在COGS与SLOG上构建类型多样性可控的变体验证了这一假设。


<details>
  <summary>Details</summary>
Motivation: 先前工作发现Transformer在结构泛化上比词汇泛化更难。作者认为这一差异并非源于模型架构本身，而是由于既有数据集中词汇类型多样性高、结构类型多样性低（类型多样性指该类型构造子的数量，而非具体词组合）。因此需要在控制类型多样性的条件下重新检验该差异。

Method: 在已发表数据集中改变词汇类型与结构类型的多样性程度；使用Grammatical Framework构建语言上更多样的COGS与SLOG数据集变体；进一步考察其他数据集属性（如非新测试结构的其他类型多样性、逻辑语义格式的表层属性）对组合泛化的影响。

Result: 类型多样性与组合泛化在词汇测试和结构测试中呈现出同等程度的相关性，支持了作者的假设。这一发现与先前工作中"复合散度（compound divergence）解释组合泛化任务难度"的论断相矛盾。

Conclusion: 结构泛化与词汇泛化的难度差异并非Transformer固有特性，而取决于类型多样性；类型多样性对两类泛化具有一致影响，复合散度并非难度差异的充分解释。
...
TL;DR: 提出 Occamy-1.0，一个基于后训练 Qwen3.6-35B-A3B 检查点进一步训练得到的高性价比协同工作（co-work）模型，通过执行接地数据、可重放长程轨迹与分阶段后训练，在多个基准上达到同级最强并处于成本—性能帕累托前沿的低成本拐点。


<details>
  <summary>Details</summary>
Motivation: 协同工作智能体需在多次模型调用中完成信息收集、工具使用、编码与文件操作等复合工作流，成本与延迟在整个回合中累积。因此其实用价值不仅取决于峰值能力，还取决于能力交付的效率。而日常工作中的许多环节侧重状态跟踪、协调、恢复与任务跟进，而非前沿级推理，故需要一类"够用且高效"的专用模型。

Method: 以已后训练的 Qwen3.6-35B-A3B 检查点为起点继续训练；构建执行接地（execution-grounded）的数据与环境；在多种 harness 下采集可重放的长程轨迹；采用分阶段后训练（staged post-training）逐步培养并巩固互补的执行能力。

Result: 在广泛的 co-work 基准套件上，Occamy-1.0 在同类规模模型中持续位居最强之列，并在若干任务上与规模显著更大的前沿系统保持竞争力。在既定评测与定价协议下，其在四个代表性基准上的综合表现落在观测到的成本—性能帕累托前沿的低成本拐点处。工具调用、编码与指令遵循的补充评测表明该特化未损害广泛的智能体能力。

Conclusion: 面向实用协同工作智能体，能力交付效率与峰值能力同等关键；通过执行接地的数据构造、长程轨迹采集与分阶段后训练，中小规模模型可在保持通用智能体能力的同时取得优异的成本—性能折中。作者开源模型权重与部分训练数据，以支持协同工作智能体与智能体后训练研究。
...
TL;DR: LAMAE是一种多模态、结构感知的掩码自编码器，在自监督预训练阶段通过共享潜在注意力模块在潜空间中直接交换多模态信息，学习患者级表征，并在MIMIC-IV上验证其在多模态任务上的优越性。


<details>
  <summary>Details</summary>
Motivation: 心血管诊断依赖ECG、超声心动图、胸片和临床变量等多种互补模态的整合。然而现有医学基础模型多为单模态，仅在微调或后训练阶段融合模态，这既丢弃了临床医生自然整合的跨模态证据，也忽视了各模态内部的结构信息。

Method: 提出LAMAE（Latent-Attention Masked Autoencoders），一种多模态、结构感知的掩码自编码器。它不是后验融合，而是通过共享的潜在注意力模块在潜空间中直接交换信息，该模块作用于study-view-entity层级结构之上，从而支持变量观测的聚合，并能优雅处理缺失模态。

Result: 在超过120万次MIMIC-IV住院数据上预训练后，LAMAE在住院死亡率、ICD-10与DRG编码、住院时长等多模态任务上优于单模态预训练及强对比学习和视觉-语言基线，在单模态任务上也保持竞争力。即使测试时仅有单一模态可用，这些增益依然存在。

Conclusion: 同时建模模态内和模态间结构能产生更鲁棒、更具可迁移性的表征。
...
### [116] [Language Is an Insufficient Substrate for Quantitative Reasoning, and Consequential Domains Need Large Quantitative Models](https://arxiv.org/abs/2609.12105)
*Reuben Vandeventer,David Imrem,David J. Wild*

Main category: cs.AI

TL;DR: 论文指出，语言描述是对定量记录的有损编码，且损失不可逆，因此LLM无法胜任定价风险、资本配置、患者分诊、网络入侵检测等关键定量决策。作者将此归因于训练表征的属性而非模型容量，并提出关键场景所需的四项性质：可复现性、从输出回溯至源记录的谱系、校准的不确定性。作者据此定义了一类新的模型类别——大定量模型（LQM）。


<details>
  <summary>Details</summary>
Motivation: 主流观点假设关键定量决策的进展将源自LLM的进展。作者质疑该假设：语言模型训练于人类描述的产物，而描述是定量记录的有损编码，且损失不可逆——无论下游模型规模多大，都无法从描述中恢复描述未编码的信息。因此需要重新界定此类任务的模型类别。

Method: 这是一篇立场/概念性论文，不含实验。方法为形式化论证：(1) 将信息损失问题形式化为训练所用表征的属性，而非模型容量的属性；(2) 识别关键场景对模型提出的四项要求——不可逆信息保全、可复现性、输出到源记录的谱系、校准不确定性；(3) 论证语言基底在构造上无法满足这些要求，据此界定新模型类别LQM。

Result: 无数值结果。核心推论为：语言基底在构造上无法提供可复现性、谱系与校准不确定性；无论模型规模如何扩大，都无法弥补描述阶段的不可逆信息损失。

Conclusion: 关键定量决策不应寄望于LLM的能力扩展。应以表征属性为判据，将此类模型划为独立类别——大定量模型（LQM），其要求包括直接作用于定量记录、可复现、具备全链路谱系与校准不确定性。
...
TL;DR: DU-NO是一种多尺度U型谱算子，仅在编码器/解码器最浅两层接入轻量卷积U-Net分支，以3.64M参数在FUNWAVE-TVD基准上取得最优自回归预报精度，较U-FNO误差降低14.9%且参数少10.8倍。


<details>
  <summary>Details</summary>
Motivation: 相位解析波浪模型（如FUNWAVE-TVD）是近岸动力学的精度标准，但对集合预报、不确定性量化和实时预警而言计算成本过高。神经算子有望以低得多的成本达到求解器级精度，但在波浪主导场中高精度算子（如混合谱-卷积的U-FNO）参数量达数千万，规模过大，亟需兼顾精度与轻量化的算子设计。

Method: 提出DU-NO（双U型神经算子）：一种多尺度U型谱算子，仅在编码器和解码器的最浅两层附加轻量卷积U-Net分支。该放置基于采样论证——高波数内容只存在于细网格上，因此局部全带通路置于高波数内容所在处，而粗的带限层级保持纯谱结构。采用随深度衰减的模态调度将参数量控制在3.64M。

Result: 在公开的FUNWAVE-TVD基准上，DU-NO在六个同等训练架构中取得最佳自回归滚动误差，较U-FNO改善14.9%且参数量少10.8倍；频带分析显示增益在所有频带均成立，包括截断谱算子失效的高波数带。参数匹配对照表明增益源于架构本身：在相同3.6M预算下最佳基线仍落后DU-NO 28.6%。该优势可迁移至近岸波浪之外：在二维Navier-Stokes上匹配最强基线，在PDEBench浅水滚动任务上明显胜出。

Conclusion: 仅在浅层引入局部全带卷积通路的U型谱算子设计，能在极低参数预算下显著提升波浪主导场的预报精度并保持跨频带稳健性，且泛化到更广泛的PDE场景，为作业化近岸预报提供了高精度轻量替代方案。代码、训练模型与评估产物已开源。
...
### [118] [Mined from Scientific Literature: Process Schemas for Atomic Layer Deposition and Etching in Materials Science](https://arxiv.org/abs/2609.12139)
*Sameer Sadruddin,Eleni Poupaki,Alex Watkins,Bora Karasulu,Adriaan J. M. Mackus,Erwin Kessels,Sören Auer,Jennifer D'Souza*

Main category: cs.AI

TL;DR: 该文提出四套经领域专家评审的JSON Schema，用于规范描述ALD与ALE的实验与模拟过程，并借助schema-miner及QUDT实现语义对齐，支持文献抽取与ORKG结构化发布。


<details>
  <summary>Details</summary>
Motivation: ALD与ALE的实验及模拟文献在材料科学领域异质性强，缺乏统一描述规范，导致结果难以比较、机器难以复用。

Method: 使用schema-miner进行数据策管，并基于schema-miner pro以QUDT本体为语义基础，构建覆盖材料、工艺条件、配置及测量/预测结果的四套JSON Schema；随后比较其范围、结构与语义基础，并演示schema引导的文献抽取及通过ORKG模板发布结构化记录。

Result: 获得四套经领域专家评审的JSON Schema，明确了其在范围、结构与语义基础方面的差异，并验证了其在文献抽取与ORKG结构化出版中的可用性。

Conclusion: 标准化Schema可提升ALD/ALE文献的互操作性与机器可复用性，为领域数据的比较与重用提供可扩展的语义化框架。
...
### [119] [Can LLMs in Draft-Verify-Revise Pipelines Resolve Deictic Ambiguity?](https://arxiv.org/abs/2609.12162)
*Obinna I. Ekekezie*

Main category: cs.AI

TL;DR: 论文研究draft-verify-revise流水线中的"指示性偏移"（deictic shift）现象，发现不同阶段LLM对"previous"等指代性表达的理解可能不一致，进而影响meta-evaluator的判定准确性，并建议在第各阶段显式指定指代对象。


<details>
  <summary>Details</summary>
Motivation: Draft-verify-revise是扩展推理期计算的常见LLM编排模式，但上下文在阶段间级联传递，不同阶段的LLM可能对"previous"这类依赖上下文的表达产生不同解析，导致指代发生"指示性偏移"。该现象此前缺乏系统研究。

Method: 构建合成数据集：10个基础样例，每个在三种条件下渲染（共享组件保持不变），条件变化包括：draft阶段（assistant）或verify阶段（grader）是否正确解析该表达，以及revise阶段（meta-evaluator）判断哪种解读正确所需独立推理的程度。测试3家供应商的6个模型、21种推理努力配置，采用e-values进行序贯检验；包含主实验与移除grader反馈中错误分类标签的消融实验；另用独立LLM分析meta-evaluator每次错误判定的理由。

Result: 平衡准确率（灵敏度与特异度的非加权平均）跨度从0.156（低于随机）到接近完美。GPT-5.2从无推理时的0.156升至最高推理努力下的0.942；Gemini 3 Pro在每一推理级别都保持0.94以上。Gemini 3 Pro低推理努力的表现超过GPT-5.2的xhigh水平，且每次试验成本约为其5%。meta-evaluator出错时倾向依赖表面线索而非操作性推理。

Conclusion: 指示性偏移会显著影响draft-verify-revise流水线的可靠性，且模型表现高度依赖推理努力与模型选择（存在成本效益更优的选择）。实施此类流水线的上下文工程师应警惕指示性偏移，并在每个阶段显式声明预期指代对象。
...
### [120] [GLARE: Generative Learning via Adversarial Reward Estimation For Social Dynamics Forecasting](https://arxiv.org/abs/2609.12165)
*Tenghao Huang,Zhaoxuan Tan,Muhao Chen,Jonathan May,Mengting Wan,Longqi Yang,Pei Zhou,Sihao Chen*

Main category: cs.AI

TL;DR: 本文提出会议动态预测基准MDFB（含2207场真实会议、24794条前瞻性查询）及对抗模仿学习方法GLARE，用于评估模型生成多轮会议续写的能力。


<details>
  <summary>Details</summary>
Motivation: 会议续写需要追踪议程、发言者角色、参与者意图以及长程多方讨论中的分歧，现有工作缺乏针对性基准与方法。

Method: 构建MDFB基准：给定转录前缀与活跃问题，模型一次调用生成合理的多轮续写；评估效用（对问题的推进）与拟人性（对话流畅度与角色一致性），不要求精确复现真实未来。提出GLARE，将对抗模仿学习适配到条件语言生成：判别器将观测续写排在当前actor采样之上，其得分作为KL正则化的策略奖励；用当前策略负样本重训使奖励景观随actor演化。

Result: GLARE在效用上取得平均0.66的人类评估胜率，拟人性上0.70，优于SFT与SPIN，但仍低于真实人类续写。MDFB亦可作为社会推理竞技场，通过参考辅助判断比较通用模型（含闭源系统）。

Conclusion: MDFB兼具任务特定学习与基于输出的会议行为评估双重用途。
...
### [121] [WinSyn: An Automated Pipeline for Realistic Enterprise Question-Answering Evaluation](https://arxiv.org/abs/2609.12171)
*Amey Varhade,Ananya Sutradhar,Ravishankar Krishnaswamy,Navin Goyal*

Main category: cs.AI

TL;DR: 本文提出一条自动化流水线，生成反映真实职场场景的合成邮件数据集，用于评测企业级问答智能体，实验显示现有前沿模型表现不足。


<details>
  <summary>Details</summary>
Motivation: 企业场景中信息分散在不断演变甚至相互冲突的邮件、聊天、文档中，检索增强生成（RAG）与深度研究（DR）等智能体面临严峻挑战。现有基准缺乏真实复杂度、多为短答案且查询不自然，无法刻画企业环境的困难。

Method: 构建自动化流水线：模拟持续数月、最多涉及25名不同角色员工的企业项目，生成带有歧义性、信息分散特征的合成邮件数据，并配套长/短文本问题及有数据支撑的标准答案。

Result: 在生成的数据集上，用最新前沿模型评测多个标准智能体基线，各数据集的整体平均得分均低于80%，仍有较大提升空间。

Conclusion: 企业级部署仍需更多研究，构建真实高复杂度评测数据对发展更强的现实企业DR系统至关重要。
...
### [122] [Soft Symbol Grounding for Prototypical Concepts](https://arxiv.org/abs/2609.12247)
*Marcos Galván-López,Nijesh Upreti,Hiram Calvo,Carlos Aguilar-Ibáñez,Vaishak Belle*

Main category: cs.AI

TL;DR: Soft-PNet将概念接地重构为在预计算可行符号解缓存上的Metropolis游走，用每概念单个标注锚点构建的原型分布引导，并以统一的KL目标训练，消除了任务特定的可微损失工程。


<details>
  <summary>Details</summary>
Motivation: 神经符号模型通常仅在最终标签上监督，中间概念不可观测；同一标签可由多种概念赋值产生，导致模型预测标签正确却恢复错误概念的"推理捷径"。原型网络通过将每个概念锚定到少量标注样本来缓解该问题，但现有方法仍依赖手工设计、任务特定的可微损失来耦合感知与推理，每换一个任务都需重新设计。

Method: 提出Soft-PNet：去除该手工损失，把概念接地重新表述为在预计算的可行符号解缓存上进行Metropolis游走，由每个概念仅需一个标注锚点构建的原型分布引导；训练时使用原型加权缓存与网络概念预测之间的单一KL目标。该目标跨任务一致，且在解空间无法枚举时仍适用。

Result: 在MNIST-EvenOdd、Visual Sudoku和Kand-Logic的稀缺监督设定下，Soft-PNet在概念级和标签级指标上均与经损失工程设计的原型网络相当，并能恢复软接地基线遗漏的概念，同时无需损失工程且训练时间更短。

Conclusion: Soft-PNet以统一的KL目标替代任务特定的手工可微损失，在保持与损失工程原型网络相当性能的同时，减少了对任务定制的依赖、恢复了更多正确概念并降低了训练时间，为神经符号学习中的推理捷径问题提供了一种通用且易迁移的解决方案。
...
### [123] [GTA: Graph Theory Agent and Benchmark for Algorithmic Graph Reasoning with LLMs](https://arxiv.org/abs/2609.12265)
*Zixiang Xu,Yanbo Wang,Chenxi Wang,Lang Gao,Zirui Song,Yue Huang,Zhaorun Chen,Xiangliang Zhang,Xiuying Chen*

Main category: cs.AI

TL;DR: 本文提出GT Bench基准（覆盖24个图问题、44种任务-结构设置、10万+样例、4种表示），揭示LLM图推理准确率强烈依赖输入表示；并提出GTA智能体（偏好训练的表示选择器+计划分解框架），将Phi-4在easy/hard划分上分别从53.5%/33.0%提升至69.1%/41.5%。


<details>
  <summary>Details</summary>
Motivation: LLM被越来越多地要求对图等结构化数据进行推理，但其在语言中执行多步图算法的可靠性尚不明确。现有评测存在三类局限：任务简单且图规模小、只评估代码生成而非图本身的推理、固定单一输入格式，难以反映真实能力。

Method: 构建GT Bench：涵盖24个经典图问题、44种任务-结构设置、超10万样例，提供自然语言、结构化语言、邻接表、邻接矩阵四种表示，并评测8个LLM。在此基础上提出GTA：由偏好训练的表示选择器与plan-and-decompose脚手架组成，围绕冻结的执行LLM协同工作。

Result: 评测显示准确率与输入表示强相关，最优表示随图密度、规模、拓扑及模型而变化，该敏感性在最强推理模型中依然存在但有所减弱。GTA将Phi-4在easy split上从53.5%提升至69.1%，在hard split上从33.0%提升至41.5%，优于8个提示与智能体基线，并可无重训练迁移至GraCoRe和NLGraph。

Conclusion: 输入表示是LLM图推理的关键变量，且难以用单一固定格式覆盖；通过表示选择与计划分解的组合，GTA能显著提升冻结LLM的多步图算法能力并具备跨基准泛化性。
...
### [124] [Learning Symbolic Constraint Representations from Examples: A Neuro-Symbolic Approach](https://arxiv.org/abs/2609.12267)
*Nassim Belmecheri,Arnaud Gotlieb,Nadjib Lazaar,Helge Spieker*

Main category: cs.AI

TL;DR: 本文提出一种神经-符号框架，用神经Oracle Transformer模型模拟用户回答并泛化概念知识，替代传统约束获取中昂贵的人工交互，通过与FastCA引擎协作，从数据中恢复可靠、一致且可解释的约束网络。


<details>
  <summary>Details</summary>
Motivation: 传统约束获取方法依赖与人类oracle的大量交互，学习过程在时间和查询次数上代价高昂，需要一种能显著降低用户参与度的自动化方案。

Method: 提出神经-符号自动约束获取框架：引入经历史样本训练的Oracle Transformer模型来模拟用户响应并泛化概念知识；该学习型oracle与专用约束获取引擎FastCA交互，FastCA将oracle的回应系统性地精炼为可靠、一致且可解释的约束网络。

Result: 实验表明该神经-符号交互能有效将数据驱动的模式识别与符号推理对齐，无需先验领域知识即可从数据中恢复结构化符号模型。

Conclusion: 神经-符号协同为组合领域中的模型构建自动化提供了一种稳健途径，显著减少了人工介入并实现了可解释的约束网络学习。
...
### [125] [T-GADE: Thermodynamical Generative-AI-Driven Evolution of LLM Artifacts](https://arxiv.org/abs/2609.12286)
*Kyoko Ogawa,Naoki Mori*

Main category: cs.AI

TL;DR: 提出 T-GADE，将热力学遗传算法扩展到 LLM 生成的"结构化产物"（如描述+代码），用自由能目标统一世代与稳态更新，并在在线装箱任务上验证其优于 EoH。


<details>
  <summary>Details</summary>
Motivation: 将进化计算与 LLM 结合需要同时控制种群多样性与生成能力；LLM 输出的结构化产物（描述+代码）需要一种能在产物层面评估多样性并约束重复基因型的进化框架。

Method: 扩展热力学遗传算法：引入基于 LLM 的遗传算子和产物级多样性评估；用统一的自由能目标支持世代更新与稳态更新；采用费米型占据排除重复基因型，玻色型占据允许重复；建立精确的单成员移除机制及恢复 EoH 零温生存规则的条件。

Result: 在在线装箱任务上，以相对体积下界的箱数超出率（excess）为指标；世代玻色型 T-GADE 在 T=0.003 时，20 次运行的中位训练 excess 从 1.152% 降至 0.815%，约降 29%（双侧 Mann-Whitney p=0.042，Cliff's δ=0.378）；对其两个最高排名最终候选做验证选择，迁移 excess 中位数与 EoH 持平，均为 0.496%。
...
### [126] [Robust Prototypical Networks for Few-Shot Sensor Fault Diagnosis](https://arxiv.org/abs/2609.12287)
*Mohammed Ayalew Belay,Amirshayan Haghipour,Pierluigi Salvo Rossi*

Main category: cs.AI

TL;DR: 提出多片段原型网络（MEPN），通过聚合多个不相交支持片段的原型均值来降低原型方差，在DeFACTO传感器故障数据集五分类任务上显著优于单片段基线，且与等支持预算下的ProtoNet性能相当。


<details>
  <summary>Details</summary>
Motivation: 工业故障诊断中标注样本极少，少样本学习适用，但标准原型网络在极低样本（very-low-shot）场景下，因每次决策仅依赖小规模支持集，导致类原型不稳定、方差大。

Method: 提出多片段原型网络（MEPN）：从多个不相交的支持片段（episode）中分别计算原型，以其均值作为最终类代表，在不改变编码器架构的前提下降低原型方差。在DeFACTO传感器数据集上评估，采用五路故障分类，并向真实工业测量中注入合成偏置、漂移、尖峰和噪声故障。

Result: 100次独立运行中，MEPN在每片段单样本设置（K=1，聚合Nag=10个支持片段）下达到[SensorOneShotGcpn]%，大幅超过单片段基线；在同等10样本支持预算下，MEPN与K=10的ProtoNet统计上无法区分。

Conclusion: 原型累积（prototype accumulation）而非固定预算下的更优学习是MEPN性能提升的机制；MEPN通过多片段聚合有效稳定了极低样本下的类原型，且不改变编码器结构。
...
### [127] [Hybrid Physics-AI Framework of Body Center of Mass Dynamics from Wrist-Worn Sensors](https://arxiv.org/abs/2609.12304)
*Shuhao Que,Valentina Breschi,Ying Wang*

Main category: cs.AI

TL;DR: 提出简化运动学模型(KM)与三种混合AI建模方法(HKM-NN)，将腕部IMU映射到全身质心(COM)加速度，误差显著降低且具备噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 腕部IMU广泛用于日常健康监测，但无法充分反映全身动力学；全身质心(COM)被视为生理参考标准。因此需要一种仅依赖腕部IMU即可估计COM加速度的方法。

Method: 提出简化运动学模型KM，基于若干简化假设使动态方程仅凭腕部IMU测量即可求解；并在此基础上提出三类混合AI建模方法HKM-NN：串行学习(ser-)与两种同时学习(sim1-、sim2-)，融合灰箱与黑箱建模。使用10名健康受试者在六种步态活动及坐-站(SS)过渡运动中的腕部IMU与真实COM数据训练测试。

Result: KM模型误差为步态活动6.7%–12.5%、SS为5.6%；HKM-NN显著提升，步态误差降至5.3%–9.3%，SS最佳误差3.9%。噪声条件下鲁棒性各异：sim1-/sim2-在高斯扰动下通常更稳健，KM在椒盐噪声下相对更稳健。

Conclusion: 从腕部IMU估计COM加速度可行；将生物力学结构与数据驱动学习相结合，对不完美、含噪测量条件下的可穿戴传感应用具有重要意义。
...
### [128] [Do Influence-Derived Data Perturbations Enable Machine Unlearning? A Controlled Study of Three Plausible Roles](https://arxiv.org/abs/2609.12313)
*Chenkai Wu,Chrispine Kambimbi,Qinyang Zeng,Jun Yan*

Main category: cs.AI

TL;DR: 该论文重新评估了Deep Perturbation Learning（DPL）在机器遗忘中的三种角色，发现其公共实现存在两处正确性缺陷；修正图像扰动流程后，DPL在CIFAR-10/ResNet-18的直接删除判据上于三个配对种子中均失败，效用影响符号不一致，计入方向计算时间后也不及简单热启动基线；Tiny ImageNet单种子检验同样不支持DPL，且代码预处理不一致使该对比不确定。


<details>
  <summary>Details</summary>
Motivation: 先前工作将DPL分别定位为直接删除信号、效用保持正则化器以及对抗遗忘的热启动，而较弱角色的证据被用来支持较强的直接删除主张。作者希望在同一匹配协议和精确种子重训练基线下分别检验这三种角色，并审计公开实现是否存在正确性问题。

Method: 在匹配协议下分别测试DPL的三种角色，并采用精确种子的重训练基线；审计公开实现，发现图像方向在增强、归一化后的张量上计算却应用于原始图像，且标签扰动低于float32分辨率导致标签实际未改变；修正图像扰动流程后在CIFAR-10/ResNet-18上进行三个配对种子实验，并做Tiny ImageNet单种子检验，同时统计方向计算时间。

Result: 修正图像扰动流程后，DPL在CIFAR-10/ResNet-18上于全部三个配对种子中均未通过直接删除判据；其效用效应在不同种子间符号不一致，计入方向计算时间后表现低于简单热启动基线；Tiny ImageNet单种子检验也不支持DPL作为正则化器或热启动，且发布代码中的预处理不一致使该处的直接对比无法定论。

Conclusion: 结论仅适用于随机实例删除场景，不能排除基于影响函数的方法在其他删除机制下的有效性。作者发布了角色匹配的评估协议以及针对基于扰动的删除主张的审计清单。
...
### [129] [AIM: A Privacy-Aware Interoperable Memory Framework for Multi-Agent Multi-User LLM Systems](https://arxiv.org/abs/2609.12320)
...
TL;DR: 提出AIM（Agentic Interoperable Memory）多智能体、多用户记忆框架，实现私有/公共记忆的动态分类与索引级访问控制，并发布首个多用户记忆评测数据集MUMBench。


<details>
  <summary>Details</summary>
Motivation: 传统LLM局限于单用户会话，知识仅限单次对话，无法学习随时间演进的用户偏好；现有智能体记忆系统多停留在单用户层面，限制了可在用户间共享、提升下游响应的公共知识。

Method: AIM是一个统一的、隐私感知的记忆框架，支持多智能体、多用户LLM系统持久管理私有与共享记忆；动态将信息分类为私有（仅限单一用户、他人不可访问）或公共（所有用户可访问），并实施索引级访问控制，使私有记忆仅能由拥有者检索。同时构建MUMBench，包含四个领域的多用户交互数据，覆盖私有与可共享信息。

Result: 在MUMBench上三次独立运行，AIM取得96.0%的可见性分类准确率、58.8%的严格操作准确率和70.5%的状态感知操作准确率。

Conclusion: AIM在保护隐私的前提下实现了跨用户的知识共享与协调一致性，MUMBench为首个面向多用户环境、评估检索、创建、更新、删除等多种记忆操作的公开数据集。
...
### [130] [Affective Agent: On-Device Personalized Intervention Reasoning for Wearable Systems](https://arxiv.org/abs/2609.12322)
*Reina Mun,Zishen Wan,Vijay Janapa Reddi*

Main category: cs.AI

TL;DR: 该论文提出 Affective Agent，一种面向可穿戴硬件的三层参考架构，用于在不确定性下进行个性化干预推理，并通过记忆驱动的个性化与两遍结构化推理提升干预决策。


<details>
  <summary>Details</summary>
Motivation: 可穿戴情感计算虽已推进状态推断，但设备端"是否、何时、如何干预"的推理仍具挑战，且需要摆脱云端依赖和逐用户重训练。

Method: 提出三层交互架构（感知层、个性化层、推理层），结合亚十亿参数紧凑语言模型与生理证据、情境及用户历史；通过主机管理的结构化记忆演化实现个性化，而非逐用户权重更新；并在室内环境质量控制场景中实例化，用模拟器生成纵向留出场景进行评估。

Result: 在涵盖生理变化、情境、信号质量与干预历史的合成纵向评估中，记忆驱动个性化与两遍结构化推理改善了干预决策。

Conclusion: 将决策层迁移至设备端，展示了从可穿戴状态推断走向可穿戴级硬件上闭环、个性化干预的路径。
...
### [131] [LoRA-RC: Reservoir Computing with Low-Rank Adaptation](https://arxiv.org/abs/2609.12327)
*Wenbin Wan*

Main category: cs.AI

TL;DR: 提出LoRA-RC：以流式预测误差驱动低秩校正自适应递归矩阵，并投影至谱范数球，保证收缩性与增量ISS稳定性；在Lorenz漂移任务上后漂移误差较固定RC降56%、较仅读出适配降51%，消融显示去掉投影误差放大40倍以上。


<details>
  <summary>Details</summary>
Motivation: 储备池计算（RC）仅训练固定递归层上的线性读出，具备快速、数据高效的在线预测优势。但静态储备池在系统漂移下性能退化：仅适配读出不足，而无约束的储备池自适应又会破坏回响状态性质与增量稳定性等RC可靠性基础。因此需要一种在保持稳定性的前提下实现储备池自适应的机制。

Method: LoRA-RC通过由流式预测误差驱动的低秩校正来适配递归矩阵。基础储备池与自适应基离线固定，仅在线适配一个小型核心矩阵；每步将其投影到谱范数球上并进行低通滤波。该投影保证所施加的每个递归矩阵均位于经认证的收缩集内，并沿每条在线自适应路径为储备池建立增量输入-状态稳定性（ISS）界，且速率与增益与路径无关。

Result: 在具有突发参数漂移的Lorenz系统上，LoRA-RC将后漂移预测误差相比固定RC降低56%，相比仅读出适配降低51%。在20个随机种子上的消融实验表明，去除投影会使该误差扩大40倍以上。

Conclusion: 低秩、带谱范数投影与低通滤波的储备池自适应可在在线过程中维持收缩性与增量ISS，从而在系统漂移下显著提升预测精度；稳定性约束（投影）对性能至关重要。
...
### [132] [Toward Robust Personalized Alignment for LLMs: Mitigating Persona Drift in Multi-Turn Dialogue](https://arxiv.org/abs/2609.12373)
*Youyuan Zhang,Siyuan Li,Fangming Liu,Jing Li*

Main category: cs.AI

TL;DR: 该论文针对个性化语言模型中的"人格漂移"问题，提出CORE方法，通过区分单轮局部证据与持久人格状态修订，并借助不确定性感知的信念修正，选择性地更新有依据的用户偏好；同时引入PERSIST基准，在歧义、冲突和受控社会影响的序列交互压力下评估人格状态鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 用户画像会随长期交互而演变，而非永久固定，因此个性化模型必须"人格漂移"（persona drift）成为核心挑战：当偏好真实改变时应修订持久人格状态，但同时要避免因瞬时、模糊或未解决的观察而触发错误更新。现有方法难以平衡这一对矛盾。

Method: 提出CORE方法：(1) 将单轮局部证据与持久人格状态修订分离；(2) 通过不确定性感知的信念修正（uncertainty-aware belief revision），仅选择性地更新有依据的用户偏好。此外构建PERSIST——一个留出的锚点后（post-anchor）基准，用于在序列交互压力下评估人格状态鲁棒性，覆盖歧义、冲突与受控社会影响三类情形。

Result: 在ALOE、PersonaChat与PERSIST三个数据集/基准上，CORE提升了个性化对齐度与鲁棒性，并在归一化闭槽（closed-slot）状态保真度上取得互补增益。人工评估与机制性控制实验进一步表明，其显式更新控制能力超越单纯更强的生成能力或持久记忆机制。

Conclusion: CORE通过显式、不确定性感知的人格状态更新控制，有效缓解人格漂移，提升个性化对齐与鲁棒性；其收益不能仅由更强生成或持久记忆解释，说明显式更新控制机制是关键。
...
### [133] [BlueLM-GUI Technical Report: A Real-Device-Centric Flywheel for Self-Improving Mobile GUI Agents](https://arxiv.org/abs/2609.12394)
...
TL;DR: 本文提出BlueLM-GUI，一个35B-A3B的原生端到端手机GUI智能体，以"真实设备为中心"的飞轮架构（Every Sample Matters、Every Rollout Is Real、Every Query Evolves 三大原则）解决沙箱训练与生产环境分布不匹配、真机失败数据未被利用、固定基准饱和三大工业落地痛点，在MobileGUI-VBench上取得87.4分、超越最佳闭源模型5.1分，在AndroidWorld上取得84.9分。


<details>
  <summary>Details</summary>
Motivation: 手机GUI智能体正从多模块框架转向端到端训练的原生模型，但工业落地存在三个持续缺口：(1) 沙箱训练与生产环境存在分布不匹配；(2) 昂贵的真机失败代价未被充分利用为训练信号；(3) 固定基准随模型提升而饱和，丧失指导迭代的能力。作者希望以真实设备为核心构建闭环飞轮来同时弥合这些缺口。

Method: 提出BlueLM-GUI（35B-A3B手机GUI智能体），围绕三条原则构建：(1) Every Sample Matters——双轨数据管线，采用异构三系统共识（Heterogeneous Triple-System Consensus）评估与错误纠正与衍生模块（Error Correction & Derivation Module），将每条轨迹挽救为可用监督信号；(2) Every Rollout Is Real——三阶段训练配方，即持续预训练、有监督微调、以及在数百台真实手机上的智能体强化学习，使每次rollout扎根于真实生产环境，从而保证能力可直接迁移到部署场景；(3) Every Query Evolves——配额驱动的基准方法，含三个正交维度，实现精确归因并支持基准随模型提升而系统性升级。

Result: BlueLM-GUI在MobileGUI-VBench上达到87.4分，超越最佳闭源模型5.1分；在AndroidWorld上达到84.9分，为开源模型中的最佳结果，并与闭源模型相当。

Conclusion: 将模型训练与迭代改进同时扎根于真实设备与"三个Every"原则，能够获得强大、鲁棒且可迁移的手机GUI能力，验证了以真机为中心的闭环飞轮范式在工业部署中的有效性。
...
### [134] [Is Gaussian Splatting Becoming Neural Again? A Taxonomy and Controlled Study of Learned Parameterization](https://arxiv.org/abs/2609.12395)
*YuanHang Wang,Xin Cao,Yi Zhang*

Main category: cs.AI

TL;DR: 本文分析3DGS中"神经化"参数生成趋势，沿五个维度对19种代表性方法进行梳理，并通过受控实验得出"选择性神经化"结论。


<details>
  <summary>Details</summary>
Motivation: 3DGS本身采用显式基元与高效光栅化，但近期系统越来越多使用神经网络生成或共享高斯参数。作者希望厘清这一趋势，判断哪些神经化选择真正带来收益，而非简单以"是否神经化"二元划分。

Method: 沿五个维度（属性解码、空间共享、视角条件解码、拓扑生成、摊销推理）对19种代表性方法进行归纳分析；并在受控的mip-NeRF 360实验中隔离三种神经参数化形式进行对比。
...
### [135] [Niching Agents in The Core](https://arxiv.org/abs/2609.12398)
*Gary B. Parker,Jim O'Connor,John Asaro*

Main category: cs.AI

TL;DR: 本文扩展了竞争性协同进化算法 The Core，将智能体进行生态位划分（niching），使其适应原始环境的特定子环境。结果表明，生态位化智能体在同子环境中的表现优于针对整个环境或不同子环境进化的智能体。


<details>
  <summary>Details</summary>
Motivation: The Core 此前已证明能在 Xpilot 游戏中进化出具备战斗与导航能力的智能体，但其智能体是在整个环境中演化的。作者希望探究将智能体限制在环境特定子集中演化是否能够提升其局部适应性与性能。

Method: 沿用 The Core 的无传统适应度函数机制，智能体通过局部交互进行锦标赛选择、交叉与变异以产生后代并进化出更优控制器。在此基础上引入 niching 策略，将智能体分别针对原始环境的特定子环境进行进化，并与针对整个系统及不同子环境进化的智能体进行对比。

Result: 实验显示，生态位化到特定子环境的智能体在同子环境中取得成功的能力，优于生态位化到整个系统的智能体，也优于生态位化到其他不同子环境的智能体。

Conclusion: 将智能体生态位化到特定子环境可有效提升其在对应子环境中的表现，说明 niche 划分是增强 The Core 局部适应能力的有效手段，为无适应度函数的协同进化控制提供了改进方向。
...
### [136] [OneLA: Scaling Linear-Attention Decoding to Large Beams in Generative Recommendation](https://arxiv.org/abs/2609.12399)
...
TL;DR: OneLA 通过共享 prompt 状态与仅追加的分歧转移记录来表示所有 beam 状态，避免为每个 beam 重建完整循环状态，在生成式推荐解码中实现 1.54-2.46 倍加速并显著降低内存与数据搬运开销。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐依赖大 beam 解码生成数百个候选项，给循环线性注意力带来新的扩展性挑战。现有线性注意力服务系统要么为每个 beam 物化完整循环状态，要么重复重放共享历史，导致显著的内存与流量开销。

Method: OneLA 利用 GR 负载中共享 prompt 与较短分歧后缀的特点：用单一共享 prompt 派生状态加紧凑的、仅追加的分歧转移记录表示所有 beam 状态；每步仅计算所需状态信息，不重建每个 beam 的完整循环状态；使用轻量级祖先索引追踪构成各 beam 历史的转移记录，使 beam 更新无需移动或复制已有记录；并设计融合 GPU kernel 在 beam 间复用共享状态。

Result: 端到端解码取得 1.54-2.46 倍加速，同时大幅降低循环状态内存占用与数据搬运量。

Conclusion: OneLA 通过共享状态表示、仅追加记录与祖先索引、以及融合 GPU kernel，有效解决了大 beam 生成式推荐下线性注意力解码的内存与流量瓶颈，并取得显著性能提升。
...
### [137] [Decentralized Evolution of Hexapod Gaits with Independent Leg Controllers](https://arxiv.org/abs/2609.12400)
*Gary B. Parker,John Asaro,Jim O'Connor*

Main category: cs.AI

TL;DR: 本文提出一种去中心化进化算法，使六足机器人各腿步态独立进化，无需集中协调，即可涌现出高效稳定的协调运动，效果优于协同协同进化。


<details>
  <summary>Details</summary>
Motivation: 传统六足机器人步态优化依赖集中式协调，优化复杂度高、可扩展性差，作者希望降低优化复杂度并提升机器人的适应性与可扩展性。

Method: 在 Webots 仿真器中以 Mantis 六足机器人为平台，对每条腿的控制器进行独立进化，不设集中协调机制，让协调行为自然涌现；并与协同协同进化（cooperative coevolution）进行对比基准测试。

Result: 去中心化方法在生成稳定、自适应步态方面优于协同协同进化，并展现出有趣的涌现式协调行为。

Conclusion: 腿控制器的独立进化降低了步态优化复杂度，凸显了去中心化策略在构建可扩展、自适应机器人系统方面的潜力。
...
### [138] [Beyond ID Embeddings: Process-Grounded Language Modeling for Cognitive Diagnosis](https://arxiv.org/abs/2609.12403)
*Minghang Liu,Yuanzhuo Wang,Qiang Qiu,Huawei Shen,Xueqi Cheng*

Main category: cs.AI

TL;DR: 提出过程感知的语言认知诊断框架PLCD，用语言派生结构作认知先验、用作答记录校准学生后验状态，在预测性能与认知迁移上均超越传统基线。


<details>
  <summary>Details</summary>
Motivation: 传统认知诊断模型依赖离散的、基于ID的嵌入来表示学生、习题和概念，这与学习者认知的本质不符——知识并非以孤立符号形式存储和提取。因此，当出现新习题或新概念时，CDM存在语义局限（冷启动问题）。

Method: PLCD框架：利用大语言模型构建概念图式（concept schemas）与认知过程图（cognitive process graphs）；采用目标条件语义记忆（target-conditioned semantic memory）检索与每个目标习题相关的历史作答；再通过带DA-MoE专家和过程级对比学习的过程接地Language-to-Cognition Mapper，将文本证据映射到统一认知空间。

Result: 实验结果表明，PLCD不仅在学生表现预测上优于传统基线，还展现出较强的认知迁移能力，并提升了冷启动鲁棒性与认知接地性。

Conclusion: 该工作将LLM的计算能力与测量潜在知识状态的心理测量目标相连接，表明经作答记录校准的结构化语言先验可改善冷启动鲁棒性与认知接地。
...
### [139] [VRL-Bench: Benchmarking agents on computer control tasks under finite trial budgets](https://arxiv.org/abs/2609.12404)
*Yu Bai,Yukai Miao,Dawei Wang,Li Chen,Yanyu Ren,Yuqian Shi,Dan Li,Ying Xiong,Chengqiu Tan,Run Zhou,Li Li*

Main category: cs.AI

TL;DR: 本文提出VRL-Bench评估框架与VEX²调度方法，研究语言智能体在有限试错预算下"言语强化学习"的探索-利用权衡。


<details>
  <summary>Details</summary>
Motivation: Reflexion等工作将失败经验转化为文本以指导后续尝试，无需更新模型参数，被视为提升语言智能体完成复杂任务（如计算机控制）的有效途径。然而，这些言语记忆方法缺乏在有限试错预算下的公平评估，且其实际效果是否稳定一致尚不明确。

Method: 1) 提出VRL-Bench，一个在有限试错预算下公平评估试错学习的测试框架；2) 在MiniWoB与WebShop两个环境、三种模型上，评估Reflexion及后续多种言语记忆方法的更新效果，并与无记忆重试（memory-free retry）基线比较；3) 通过重放（replay）实验分析反思机制的影响；4) 提出VEX²——一种言语探索-利用调度器，用语言模型联合选择策略并分配剩余试错预算。

Result: 各言语记忆方法在部分设置中相对无记忆重试提升了观测成功率，但在其他设置中反而降低。重放实验表明，使用反思可能降低成功率，揭示出利用已有经验与持续探索之间的权衡。VEX²是唯一在全部六个设置中相对重试均取得正向观测成功率增益的方法。

Conclusion: 试错学习中的言语记忆方法效果并不稳健，存在探索-利用权衡；显式地调度探索与利用（如VEX²）比单纯复用反思文本更可靠，可实现一致的成功率提升。
...
### [140] [SoK: Rethinking Jailbreaking in the Era of Agentic AI: Attacks, Defenses, and Practical Consideration](https://arxiv.org/abs/2609.12413)
*Md Jueal Mia,Yanzhao Wu,Selcuk Uluagac,M. Hadi Amini*

Main category: cs.AI

TL;DR: 本文是一篇SoK，围绕LLM agent完整执行流水线重构越狱安全研究，提出攻击与防御的统一分类法和安全-效用-效率评估框架，并通过受控实证研究揭示现代LLM及agentic AI时代的三个关键安全缺口。


<details>
  <summary>Details</summary>
Motivation: LLM正从对话助手演变为具备推理、规划、工具调用、持久记忆、多智能体通信与多步任务执行能力的agentic系统；同时现代模型的原生安全对齐显著强于早期模型，而大量越狱攻击与防御研究却建立在旧代模型之上。由此产生核心问题：哪些既有越狱安全结论在现代LLM与agentic AI时代仍然成立。

Method: 采用系统化知识综述（SoK）方法，将越狱安全重新锚定于完整agent执行流水线；构建覆盖用户交互、规划与推理、记忆、工具使用、智能体间通信的攻击与防御统一分类法；提出分离原生有害提示安全、对抗越狱鲁棒性与agent级安全结果的安全-效用-效率评估框架；并在统一agent框架内对代表性攻击与防御进行受控实证研究。

Result: 实证揭示三个缺口：（1）强原生对齐并不意味着对抗越狱鲁棒性；（2）防御有效性高度依赖模型、攻击类型与组件，且可能以过度拒答、效用损失与延迟为显著代价；（3）最终响应层面的低攻击成功率可能掩盖严重的中间环节失陷——即使最终响应被成功过滤，规划、记忆与工具交互仍可能不安全。

Conclusion: 研究结果促使越狱防御从以响应为中心转向跨层、执行感知的安全范式，在保护agent状态、组件间转换与外部动作的同时保持实际效用与效率。
...
### [141] [Hierarchical Belief Modeling for Zero-Shot Opponent Adaptation in Partially Observable Multi-Agent Navigation](https://arxiv.org/abs/2609.12422)
*Kowei Shih,Lu Cheng,Zeyu Wang,Yeyun Xu,Kejian Tong*

Main category: cs.AI

TL;DR: 提出分层智能体HORIZON，用于Lux AI Season 3竞赛，在部分可观测与随机化环境下结合对称感知、双记忆信念跟踪、圣物图注意力、信息增益探索与对手条件策略混合，通过PPO训练后在胜率与联赛评分上超越强基线。


<details>
  <summary>Details</summary>
Motivation: Lux AI Season 3要求智能体在部分可观测、回合级动态随机化以及五局三胜的赛制下行动，既需战术执行也需快速适应，现有循环或前馈基线难以兼顾短程控制与跨对局元推理。

Method: HORIZON为分层智能体，融合对称感知的空间表征、双记忆信念跟踪、以圣物为中心的图注意力、信息增益驱动的探索以及对手条件化的策略混合；将短程控制与跨对局元推理分离，并引入辅助的信念与世界模型目标稳定学习；在大规模JAX模拟器中用PPO训练。

Result: 实验表明，在比赛胜率、单局胜率、适应增益与联赛评分等指标上，HORIZON相对强循环与前馈基线均取得一致提升。

Conclusion: 该智能体能够显式推断隐藏游戏参数与对手风格，验证了分层结构结合信念跟踪、图注意力与对手建模在部分可观测竞赛环境中的有效性。
...
### [142] [LifeFuse-Mem: Lifecycle-Aware State Fusion Against Temporary Overwriting for Long-Term Memory](https://arxiv.org/abs/2609.12436)
*Hanyu Zhao,Yuqian Feng,Zhenyu Song,Yuanchao Cheng,Yance Jiao,Tengfei Pan,Li Du*

Main category: cs.AI

TL;DR: 该论文研究长时运行的LLM智能体记忆机制，提出生命周期标注记忆设定及LifeFuse-Mem框架，通过分离稳定与临时知识来缓解临时信息覆盖持久知识的“覆写”问题。


<details>
  <summary>Details</summary>
Motivation: 长时交互的LLM智能体需要保持内部状态连贯，但现有记忆机制难以区分应当长期影响未来交互的信息与仅影响当前上下文的信息。这种生命周期错配会导致临时信息覆盖持久知识，引发持久智能体的行为漂移。

Method: 论文设定“生命周期标注记忆”场景：训练时写入片段提供生命周期元数据，评估时采用阶段感知读出。在此设定下提出LifeFuse-Mem——一种生命周期感知的神经记忆框架，依据信息的时间承诺进行分离，使用专用记忆组件与生命周期感知更新，使稳定知识和临时知识各自局部演化，避免将临时上下文转化为持久状态。

Result: 在受控的反覆写基准上，LifeFuse-Mem提升了受采集控制的保持能力并减少了临时覆写；在两个公开长记忆基准上，其总体表现保持竞争力。

Conclusion: 结果表明，显式的生命周期信号有助于诊断并缓解紧凑在线记忆中的覆写问题。
...
### [143] [Do LLMs Trust the Accuser or the Accusation? Measuring Belief Shifts in Werewolf](https://arxiv.org/abs/2609.12446)
*Yu-Yu Yang,Ti-Rong Wu,Hung Guei,Hsing-Yu Chen,I-Chen Wu*

Main category: cs.AI

TL;DR: 提出狼人杀"信念转移"评测基准，通过标注怀疑/指控消息并测量观察者模型信念变化，评估40个开源权重LLM配置，发现模型能区分狼人与村民，但仍易受指控影响，难以整合指控内容与来源可信度。


<details>
  <summary>Details</summary>
Motivation: 现有社交推理游戏（如狼人杀）对LLM智能体的评估多依赖最终游戏结果，无法细粒度分析其沟通能力。作者希望从"信念更新"角度构建更细致的评测方法，以揭示模型在策略性沟通中的真实表现。

Method: 构建狼人杀信念转移评测基准：使用LLM进行对局，对怀疑与指控类消息进行标注，测量村庄阵营的观察者模型在每条消息后信念的变化。在1,224条标注消息上评估40个开源权重LLM配置，考察模型区分狼人与村民的能力及指控对信念的影响。

Result: 更大模型能更好地基于对局历史区分真狼人与村民；但指控仍强烈影响其信念——模型对被指控者更怀疑、对指控者更不怀疑，尤其当指控者被信任时，即便指控者属于狼人阵营；更大模型对来自其原本不信任者的指控有更强抵抗力。

Conclusion: 当前120B参数以内的开源权重LLM仍难以在策略性沟通中整合"指控内容"与"来源可信度"。作者公开了基准与代码。
...
### [144] [EvoRS: On-Policy Self-Evolution of Reward Systems for Open-Ended Reinforcement Learning](https://arxiv.org/abs/2609.12459)
...
TL;DR: 提出 EvoRS 自演化强化学习框架，将奖励系统表示为可执行的 Reward-DAG，并借助智能体式设计器根据在线轨迹持续更新奖励系统，以维持训练期可靠性。


<details>
  <summary>Details</summary>
Motivation: 开放式任务缺乏可直接验证的答案，常依赖基于评分标准的奖励，但策略与奖励系统构成动态反馈回路：随着策略优化当前奖励，原本有效的奖励系统可能因奖励黑客或响应区分度下降而失效。现有动态评分标准方法只调整评价准则，而奖励失效也可能源于打分机制或信号组合，因此奖励系统需要整体演化而非固定不变。

Method: EvoRS 是一种自演化强化学习框架，从在线经验中演化奖励系统，并将其表示为可执行的 Reward-DAG。具体地，一个智能体式设计器基于在线 rollout 与奖励轨迹对该系统进行更新，从而在训练过程中保持奖励的可靠性。

Result: 在写作与角色扮演任务上，EvoRS 在三个评判者下均取得最佳质量，分别超过策略 2.107 分与 4.767 分，同时减少了奖励黑客与覆盖失败，并保持奖励信息量。消融实验证实，一个全面的固定奖励系统在开放式任务中无法持续可靠，必须在整个训练过程中演化。

Conclusion: 奖励系统应作为随训练共同演化的动态组件；通过以 Reward-DAG 形式的可执行表示并利用在线经验持续更新，可有效缓解奖励失效问题，提升开放式任务表现。
...
### [145] [Beyond Vector Similarity: Hierarchical Context-Aware Graph RAG vs Standard RAG in Enterprise Code Migration](https://arxiv.org/abs/2609.12464)
*Nilesh Jaiswal,Aniket Agrawal,Arjit Shukla,Divya Malhotra,Saurabh Garg,Suchit Puri,Suddhasatwa Bhaumik*

Main category: cs.AI

TL;DR: 本文提出分层上下文驻留图(HCRG)方法,用AST解析+属性图+上下文缓存实现拓扑感知的代码翻译,将API幻觉率从56.4%降至16.2%,但以环复杂度一致性下降为代价。


<details>
  <summary>Details</summary>
Motivation: 企业将遗留单体系统迁移至微服务时大量使用LLM做自动化代码翻译。传统基于向量的标准RAG难以捕捉拓扑关系,只能检索到孤立代码块,割裂继承链,导致高编译失败率。

Method: 提出HCRG方法:pipeline使用tree-sitter提取抽象语法树(AST),将架构边映射进Google Cloud Spanner属性图(Property Graph),再将该结构序列化到Gemini Context Cache中,实现拓扑化、父节点优先(parent-first)的代码翻译。评估上放弃朴素文本重叠指标,改用自定义的7指标软件工程框架。

Result: CodeBLEU等传统指标对两方法均给出91%分数,掩盖了标准RAG的结构性失败。图RAG显著缓解依赖丢失:API幻觉率56.4%→16.2%,依赖解析质量34.8%→65.9%,父子一致性26.7%→45.5%。但存在代价:稠密全局上下文引发LLM防御性过度设计,环复杂度一致性71.6%→46.7%,文档字符串保留率67.0%→61.0%。

Conclusion: 图RAG以牺牲代码复杂度为代价换取幻觉减少,为自动化企业代码库现代化提供了更可行、架构更合理的路径。
...
### [146] [TripPattern: A Pattern-based Text Watermarking Method for Large Language Models](https://arxiv.org/abs/2609.12472)
*Sangjun Moon,Dasom Choi,Jingun Kwon,Hidetaka Kamigaito,Taro Watanabe,Manabu Okumura*

Main category: cs.AI

TL;DR: 提出 TripPattern 文本水印框架，将词表分为一个中立组与两个模式组，通过对两组交替选词嵌入可检测模式，在保持生成质量的同时实现鲁棒的水印可检测性。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 文本水印方法通常将词表二分为绿色与红色标记，并引导生成偏向绿色标记，这会降低文本质量与自然度。需要一种既能嵌入可检测水印、又不损害生成质量与自然性的方案。

Method: TripPattern 将文本水印建模为基于模式的匹配任务，采用三词表划分：一个中立组和两个模式组。生成时模型在两个模式组之间交替选择标记以嵌入可检测模式，中立标记独立选择以提升灵活性与自然度。检测阶段使用基于模式的统计检验，通过度量相邻标记在模式组间交替的频率给出可解释的 p 值；并配套理论分析。

Result: 在四个多语言数据集上的理论分析与实证评估表明，TripPattern 在保持 LLM 生成质量的同时，实现了鲁棒的水印可检测性。

Conclusion: TripPattern 通过三词表划分与模式交替机制，缓解了绿色/红色二分水印对文本质量与自然度的损害，提供了兼顾生成质量与可检测性的文本水印方案。
...
### [147] [When Does AI Augment Work? A Workflow-Level Framework for Human-Agent Collaboration](https://arxiv.org/abs/2609.12482)
...
TL;DR: 本文主张超越当前自动化能力与公众采用率等指标，转而从人机协作改造工作流程的角度评估AI在工作场所的价值，并提出包含六项条件的"AI增强"精确定义，以AI中介社会调查为案例加以阐释。


<details>
  <summary>Details</summary>
Motivation: 现有研究多以AI当前的自动化水平和公众采用度衡量其职场价值，但这些指标忽视了人机协作对工作本质的更深层改造，因而需要扩展分析范围，从当下原子化任务转向未来整体工作流程的增强。

Method: 提出"AI增强"的精确定义，包含六项条件：持久净价值、有意义的人类控制、问责与恢复，以及通过学习、职业路径和工作意义实现的长期人类发展；并对这些条件逐一阐述，将其框架应用于AI中介社会调查的案例研究。

Result: 通过六条件框架的构建与AI中介社会调查案例的应用，展示了该框架如何刻画超越自动化任务层面的人机协作价值。

Conclusion: 组织、研究者和政府决策者可借助该框架理解未来工作的走向，从而更全面地评估AI在工作场所的真实价值。
...
### [148] [Confidence-Gated Transductive Test Generation for Code Reranking](https://arxiv.org/abs/2609.12489)
*Sungjae Lee,Youngsik Yoon,Seockbean Song,Siwei Wang,Wei Chen,Jungseul Ok*

Main category: cs.AI

TL;DR: 提出 CoTT（置信度门控的转导式测试生成）方法，先做高效归纳式生成，仅在归纳置信度低时才调用转导式生成，用于 LLM 代码测试用例合成与程序重排序。


<details>
  <summary>Details</summary>
Motivation: LLM 生成的程序需要高质量测试用例来评估与排序，但可靠期望输出难以获得，构建高质量测试用例仍是难题；同时对每个输入都做转导式生成成本过高。

Method: CoTT 采用自适应设计：先执行高效的归纳式（inductive）测试生成流程，并估计其置信度；当置信度低时，才额外触发转导式（transductive）生成。由此把额外计算仅分配到必要时，以单一大模型完成。

Result: 在代码重排序基准上，CoTT 在报告的各项指标上优于既有基线，同时相较对所有输入都使用转导式生成，显著降低了开销。

Conclusion: 基于置信度分配测试时计算，能在效率与效果之间取得更优权衡，为 LLM 代码评测中的测试用例合成提供了实用方案。
...
### [149] [Information Specialization and Constrained Synthesis in Multi-Agent LLM Forecasting: A Prospective Live-Study of the 2026 FIFA World Cup](https://arxiv.org/abs/2609.12495)
*Julian Varghese,Lucas Bickmann,Sarah Sandmann*

Main category: cs.AI

TL;DR: 在2026年世界杯最后56场比赛的前瞻性实证中，将同一基础模型分饰量化与新闻两类专家预测代理，并加入评论者与元代理进行序贯整合；结果显示新闻专家效用最高、与博彩市场持平，但专家间高度重合，整合阶段未带来互补信息或超越最强专家。


<details>
  <summary>Details</summary>
Motivation: 大语言模型正被组织为具有专业化角色的多智能体系统，但角色专业化是否能产生差异化预测、以及后续的综合（critic与meta-agent）是否真正提升决策效用，尚缺乏实证证据。

Method: 开展一次实时、前瞻性评估，覆盖信息密集的2026年世界杯最后56场比赛。固定同一前沿基础模型，设置两个角色对立的主要预测代理：量化专家（关注结构化表现统计）与新闻专家（关注伤病、战术与发布会信息）；其预测先由独立评论者审查，再由元代理合并，构成四代理序贯模型。以博彩市场预测作为外部基准。

Result: 新闻专家取得最高的平均概率加权Top-3效用，并在Top-3精确比分命中上与博彩市场持平。但两位专家预测者在56场中的50场里，三个比分中至少两个一致；元代理从未产生超过一个位于专家预测集之外的比分。

Conclusion: 快速变化、非结构化信息可与结构化统计并列成为有价值的预测信号；而增加评论者与元代理环节并不必然带来互补信息，也未必优于最强单一专家。
...
### [150] [From Collaboration to Capability: Internalizing Routed LLM Experts into Compact Reasoners](https://arxiv.org/abs/2609.12578)
*Frank Nie,Shuyao Wang,Ethan B. Liu*

Main category: cs.AI

TL;DR: 本文提出 Rivet，通过专家增强强化学习与已验证轨迹内化，训练紧凑控制器在专家移除后仍能生成推理、代码与交互结构，并在数学与科学推理基准上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 紧凑控制器可通过选择咨询对象、构造请求和整合专家回应来协调更强专家。作者希望探究：同时从控制器决策与专家推理、代码中学习，能否在专家移除后提升控制器的生成能力。

Method: 提出 Rivet 框架，包含两部分：专家增强强化学习，将共享结果信号应用于控制器决策和返回的专家片段；已验证轨迹内化，通过格式感知的监督训练整合完整的成功交互。部署后的控制器可自行生成推理、代码和交互结构，并支持本地 Python 执行，不依赖外部 LLM。

Result: 在七个竞赛数学基准上，RIVET-1.7B 与 RIVET-4B 平均准确率分别为 28.25% 和 44.16%；Stage II 在专家移除后使 RIVET-4B 准确率提升 6.49 个百分点；GPQA-Diamond 结果支持其对科学推理的泛化能力。消融实验显示普通轨迹监督和额外格式加权均带来增益。

Conclusion: 对已验证协作的内容与结构进行训练是有效的，说明协作内化有助于紧凑控制器在移除专家后保持并提升生成与推理能力。
...
### [151] [Reproducing and Evaluating the Generalizability of Subliminal Learning in Open-Weight Models](https://arxiv.org/abs/2609.12586)
*Daan van der Weijden,Nathan Brack,Selene Baez Santamaria*

Main category: cs.AI

TL;DR: 本文复现了"潜意识学习"（蒸馏中教师模型经由语义无关数据传递行为偏好）的实验，并沿三条轴线扩展（新偏好类别、新任务、新开源模型），同时进行答案空间大小的消融实验。结果支持原论文结论，但表明其并非普适：传递强度因特质、任务和模型而异。


<details>
  <summary>Details</summary>
Motivation: 原论文的GPT-4.x微调已不可用，需在开放权重模型上验证潜意识学习现象的可复现性与泛化性，并检验其是否具有普适性。

Method: 复现原论文的实验设置（两类特质×三种数据模态×多个模型家族），并扩展：新增偏好类别（演员与政治人物）、新任务（国际象棋走子生成）、新增开源模型Ministral8B，以及对数字任务答案空间大小（1/2/3位数序列）的受控消融。

Result: 复现结果支持原论文的主张；但扩展实验显示传递强度在不同特质与任务间存在差异，某一模型几乎观察不到效应。

Conclusion: 潜意识学习现象可被复现，但其效应并非普遍适用，其强度依赖于具体特质、任务与模型。
...
### [152] [Harness or Model? Isolating the Harness Effect in Agentic Coding with a Contamination-Controlled Private Suite](https://arxiv.org/abs/2609.11987)
*Mohsen Arjmandi*

Main category: cs.AI

TL;DR: 在256个私有任务的配对同模型对比中，厂商原生harness相对中立harness并无平均优势：Opus 4.8为-1.25pp，GPT-5.5为+1.25pp，且差异可被任务分层反向抵消；成本方面中立harness在观测用量下更贵，但因用量记录缺失，计费排序仍未定论。


<details>
  <summary>Details</summary>
Motivation: 业界普遍假设厂商原生harness与其自家模型配套能解决更多任务，作者希望通过受控的配对同模型实验检验这一假设，并厘清harness差异在正确性、完成度与成本上的真实影响。

Method: 在私有、抗污染套件（256个仓库任务与截断后竞赛任务）上做配对同模型对比：80个相同任务分别在claude-agent-sdk与deepagents（claude-opus-4-8）、openai-codex SDK与deepagents（gpt-5.5）下运行，gemini-3.5-flash与deepseek-v3.2作为侧组；800次计划运行中792次由隔离oracle评分；按冻结标价从原始逐轮用量重新计价，并给出任务bootstrap与标签置换检验。

Result: 两个对比均未解出harness的平均优势：Opus 4.8为-1.25pp（48.8% vs 50.0%，CI [-10.0, +7.5]），GPT-5.5为+1.25pp（55.6% vs 54.4%，CI [-4.4, +6.9]）。Opus平均由相反分层构成：原生harness在61个仓库任务落后9.0pp，在19个竞赛任务领先23.7pp（p=0.003）。81次触及时限取消的运行中22次已产出通过补丁。观测用量下中立harness每解一题成本为Opus 4.8的1.3–1.6倍、GPT-5.5的1.2倍；Anthropic账号58次运行无用量记录，若归入任一cell则Opus比值在0.7–2.3间变动。

Conclusion: 厂商原生配对并不保证更优，harness排名高度依赖任务分层，竞赛与仓库任务上呈相反结论；标签置换显著的切分系事后选择，需设计性重复验证。成本结论受用量语义缺陷与缺失记录限制，计费排序尚不明确。本版本纠正了2026年8月稿中基于自身遥测用量语义缺陷的成本数字（5.1节），并公开编排器、评分oracle、复现代码与派生聚合数据，任务保持私有。
...
TL;DR: 本文提出 GeoVAD-Bench 诊断基准与 GeoWeave-8B 模型，系统评估并提升几何问题求解中视觉思维链（VCoT）的中间过程有效性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态几何推理评估通常孤立地考察视觉生成质量与最终答案准确率，无法判断中间视觉辅助是否几何有效、是否被后续推理真正利用、是否与任务成功存在因果关系。

Method: 1) 构建 GeoVAD-Bench 诊断基准，包含感知、辅助质量、利用、演绎推理与最终正确性五个维度的细粒度轨迹诊断；2) 设计 No-Aux、Auto-Aux、GT-Aux 三种受控干预设置，用于分离中间错误模式、视觉辅助的因果增益及自主性差距；3) 基于诊断结论建立涵盖几何感知、图形编辑与交错图文推理轨迹的专项数据构造流程，并采用渐进式 SFT 与多模态 RL 训练框架。

Result: GeoWeave-8B 相比基座模型在最终几何准确率上提升 +25.3%，在四个中间诊断维度的过程平均分上提升 +30.4%。

Conclusion: 高质量辅助视觉对几何求解具有显著理论增益，但自主生成常因几何感知、忠实视觉操作、视觉状态接地与演绎推理的复合误差而受限；所提诊断驱动的数据与训练框架能有效缓解该问题并大幅提升几何推理性能。
...
TL;DR: 本文聚焦全双工语音对话模型的"可控性（steerability）"缺失问题，提出分类体系、SteerDuplex模型、SteerBench基准及两阶段强化学习方案，显著提升音频可控性与打断处理能力。


<details>
  <summary>Details</summary>
Motivation: 全双工语音对话模型虽已支持低延迟轮次切换、打断处理和反馈应答，但"可控性"——即依据用户指令在语气、人格、语速、声音风格等属性上可靠调整对话行为的能力——仍未被充分探索。作者通过构建文本与音频可控性分类体系，识别出现有全双工模型存在显著能力缺口。

Method: 1) 提出文本与音频可控性分类体系；2) 提出SteerDuplex，基于Moshi的全双工语音模型，在自然对话与合成对话数据上微调，覆盖指令遵循、发声方式、推理和双工交互；3) 采用两阶段强化学习，结合混合奖励（可验证的交互检查 + 基于评判模型的语义反馈）以改善时序与响应连续性；4) 构建SteerBench基准，含390条语音提示和1,067条人工编写的音频与文本二元评分标准，覆盖语气、人格、风格/口音、速度/长度四个维度。

Result: 在SteerBench上，监督训练相较最强开放基线将音频可控性平均通过率提升44.5个百分点；在Audio MultiChallenge上任务平均通过率提升7个百分点。强化学习进一步将源干净打断响应率从72.5%提升至82.5%，并将合成停顿抢话率从26.5%降至9%。可控性与总体任务得分保持相当或更高。奖励探针揭示了通过不完整响应进行奖励欺骗（reward hacking）的现象。

Conclusion: 该模型与基准为语音可控性的系统研究提供了支撑；奖励分析表明，时序增益必须与响应完整性联合评估，以避免奖励欺骗导致的评测偏差。
...
TL;DR: 基于德国11位房地产经纪人半结构化访谈的实证研究，梳理生成式AI在房地产营销中的实际应用现状、人机协作模式与采纳障碍。


<details>
  <summary>Details</summary>
Motivation: 生成式AI正改变工作的组织与执行方式，房地产营销是典型场景，但关于经纪人在日常实践中实际使用GenAI的实证证据仍然稀缺，因此需要基于真实从业者的经验来描述现状。

Method: 采用质性研究方法，在德国开展实证研究，对11位房地产经纪人进行半结构化访谈，归纳其GenAI使用场景与协作方式。

Result: GenAI已应用于多项活动，营销传播最为突出；具体用例处于涌现阶段且采纳不均衡，撰写房源文案是唯一被广泛确立的用例。交互以human-in-the-loop为主：GenAI负责起草、结构化与检索，经纪人负责策展、核实与决策。制约因素更多来自与房源系统及文档的集成、数据可得性，以及敏感任务中的合规要求，而非模型能力本身。

Conclusion: 研究提供了现有及潜在用例的扎根式图谱，并提出关于采纳的初步实践启示，为理解GenAI在房地产营销中的真实落地状态提供了实证基础。
...
### [156] [Residual Vector-based Reconstruction as Long-Context Recall Regardless of Context Window Size](https://arxiv.org/abs/2609.12686)
*MyungHoon Ryu,XinYu Piao,Jong-Kook Kim*

Main category: cs.AI

TL;DR: 本文提出一种无需额外训练的长期上下文回忆方法，通过利用LLM前馈层中的参数激活（残差向量）重建事实，在上下文长度增长时保持近恒定的GPU显存占用，可在200万token的故事上下文中回答单事实问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽能处理长文档与长对话，但其token级显存占用随输入长度线性增长；现有模型优化与有损提示压缩方法仍无法解决超出预训练及尺寸受限上下文窗口的长上下文回忆问题。

Method: 核心思想是利用LLM前馈层中的参数激活重建事实——这些层存储了表示源文档事实的残差向量。借助残差向量，LLM无需引用原始文档即可确定性地重建与查询相关的事实，从而在保持高保真度的同时降低显存占用，且无需微调权重、无需额外训练。

Result: 实验表明，该方法能够在200万token的故事上下文中回答单事实问题，而此前的方法在此类场景下均告失败。

Conclusion: 该工作提出了一种免训练、显存近恒定的长上下文回忆方案，突破了现有方法在超长上下文（如200万token）下的回忆瓶颈。
...
### [157] [I Am AdMan: A Pipeline for Automatic Generation of Personalized Advertising Imagery](https://arxiv.org/abs/2609.12694)
*Victor Kolominsky-Rabas,Leopold Müller,Claudius Budcke,Niklas Kühl*

Main category: cs.AI

TL;DR: 提出多智能体流水线 AdMan，将客户数据转化为人物画像并生成个性化广告图像，同时用基于 LLM 的评判智能体进行自动质检；在 4 种产品、6 个名人画像与 100 个真实客户画像上生成 1745 条广告，采用专家焦点小组定性与瑕疵率定量评估，结果表明可生成照片级真实且个性化的广告，但效果受产品复杂度和模型配置影响显著。


<details>
  <summary>Details</summary>
Motivation: 现有个性化方法擅长将合适的产品匹配给合适的客户，但广告的视觉呈现仍高度通用、对个体的定制程度较弱；生成式 AI 在文本个性化广告与规模化图像生产上已有进展，但如何在技术层面系统性地将细粒度客户信息转化为大规模、完全由 AI 生成的个性化广告图像，尚缺乏研究。

Method: 提出 AdMan 多智能体流水线：将客户数据转化为人物画像（personas）；以产品参考图像为条件生成个性化广告图像；引入基于 LLM 的评判智能体（judge agent）进行自动化质量控制。以两种不同模型配置实现该流水线，覆盖 4 种产品，使用 6 个名人画像做定性检查、100 个真实客户画像做规模化生成，共产生 1745 条广告；评估结合定性专家焦点小组与定量瑕疵率（artifact-rate）测评。

Result: 流水线能够生成照片级真实（photorealistic）且个性化程度较高的广告图像；但性能随产品复杂度和模型配置不同而差异显著。

Conclusion: 研究将 AI 个性化广告文献向前推进，验证了广告图像全自动生成的可行性，同时揭示了其当前局限（如对产品复杂度敏感、依赖模型配置），为后续改进方向提供依据。
...
### [158] [Enabling and Understanding Personalization in AI-Generated Advertising Imagery](https://arxiv.org/abs/2609.12697)
*Victor Kolominsky-Rabas,Leopold Müller,Claudius Budcke,Claas Christian Germelmann,Niklas Kühl*

Main category: cs.AI

TL;DR: 该研究开发并实现了一个基于AI的框架，能够直接从客户数据生成个性化广告图像，并通过两阶段被试内实验（N=100，4种产品，3个个性化水平）进行评价。结果显示，中等个性化水平的AI生成广告图像获得最积极的评价；高个性化虽提升感知个性化，但也提升感知毛骨悚然感，且后者的负向效应主导总效应。


<details>
  <summary>Details</summary>
Motivation: 传统个性化营销主要将静态产品与客户匹配，而动态创意优化多聚焦于AI驱动的文本个性化或基础产品图像修改，缺乏直接从客户数据生成个性化广告图像的研究，本文旨在填补这一空白。

Method: 开发并实现基于AI的框架，从客户数据生成个性化广告图像；采用两阶段被试内实验设计，N=100名被试，覆盖4种产品和3个个性化水平（依据所用客户数据的数量与具体性区分）；被试对每张图像就广告态度、产品态度和购买意向进行评分。

Result: 被试能够感知不同个性化水平之间的差异；AI生成的广告图像在中等个性化水平下评价最积极；高个性化提升感知个性化（与三项结果变量均正相关），但同时提升感知毛骨悚然感（与结果变量负相关），且毛骨悚然感的负向效应在总效应中占主导。

Conclusion: 基于AI的个性化广告图像生成具有可行性，但个性化程度并非越高越好，中等个性化水平效果最佳；感知个性化与感知毛骨悚然感并存，后者主导总效应，提示实践中需在个性化收益与隐私侵入感之间权衡。
...
### [159] [Implicit Personality Representations in Humans and LLMs](https://arxiv.org/abs/2609.12704)
*Yilin Geng,Omri Abend,Eduard Hovy,Lea Frermann*

Main category: cs.AI

TL;DR: 该研究以数百个特质词的隐式人格矩阵为基准，检验Qwen 2.5-7B-Instruct的内部特质表征是否复现人类人格印象的稳定关系结构，发现二者高度对齐（Mantel r = 0.77），并可由社会性与智识性两轴解释。


<details>
  <summary>Details</summary>
Motivation: 心理学百年研究表明，人们描述他人的特质词虽多样，但特质间"哪些相近、哪些对立"的关系结构在不同评分者与跨文化间高度一致。作者希望检验大语言模型内部是否自发形成与人类共享人格印象一致的特质几何结构，从而评估模型人格表征的人类基础性。

Method: 一方面，从数百万条众包来源的虚构角色人格评分数据中，构建覆盖数百个特质的人类隐式人格关系矩阵；另一方面，通过对比式模型激活（contrastive model activations）为同一批特质构建对应的模型内部表征矩阵。随后用Mantel检验等方法比较两种关系结构的整体与逐特质一致性，并对模型表征做主成分/方向分析以识别主导轴；最后在留出对话数据上将激活投影至该方向，生成人格画像并与人类评分比对。

Result: 人类矩阵与模型矩阵整体强对齐（Mantel r = 0.77），且该一致性在逐特质层面与总体层面均成立。模型特质表征的两个主导轴分别对应人类社会与智识两个维度——社会温暖性（social warmth）与智识胜任性（intellectual competence）。在留出对话上，将模型激活投影到这两个方向所得人格画像与人类评分一致。

Conclusion: 该工作建立了可比对、以人类为基准的框架，用于系统比较模型内部特质几何结构与人类人格印象的共享结构，表明LLM内部特质表征在关系组织层面与人类高度趋同。
...
### [160] [When Rubrics Fail: Hallucinations Reveal Blind Spots in Medical AI Evaluation](https://arxiv.org/abs/2609.12718)
*Griffin Farrow,Lily Sijia Li,Jack Johnson,Tingyan Wang,Philip Torr,William Bolton,Fabio J. Fehr*

Main category: cs.AI

TL;DR: 该论文研究医学LLM评估中评分量表(rubric)对临床相关幻觉的敏感性，发现rubric普遍漏检临床相关幻觉错误，评分往往不变，表明rubric分数不足以确立临床可靠性。


<details>
  <summary>Details</summary>
Motivation: 幻觉会削弱临床医生对LLM的信任，因此评估方法必须能捕捉临床相关错误。尽管rubric-based评估已成为医学LLM评估的主流方法，但其分数是否反映这类错误尚不清楚。

Method: 首先在受控设置下使用MedHallu研究，发现更具体的rubric更能区分正确与幻觉回答。进而构建医学幻觉类型分类体系，以及经临床医生验证的错误注入流程，生成匹配的正确回答与错误注入回答。在HealthBench、HealthBench Professional和LiveMedBench上进行系统测试。

Result: 在三个基准上，临床相关幻觉常被rubric漏检，评分往往保持不变。rubric在明确核查事实时最有效，而对未预期到的额外或意外错误效果较差。初步的基于检索的事实性检查可恢复部分rubric盲区错误。

Conclusion: 当前医学LLM评估存在系统性盲点，仅靠rubric分数不足以确立临床可靠性，可能削弱临床医生对临床部署的信任与信心；建议采用检索式事实性检查等互补方法。
...
### [161] [Skill Issue: Lessons from Optimizing Repository SKILLs for Coding Agents](https://arxiv.org/abs/2609.12742)
*Mykhailo Kozyrev,Andrei Kozyrev,Anton Podkopaev*

Main category: cs.AI

TL;DR: 该论文研究代码代理读取仓库内SKILL文档的问题，用合并后又被回滚的PR挖掘更难任务，并以"有文档vs无文档"的代理表现差作为评分。三个Kotlin仓库上GEPA提升4.9pp，SkillOpt仅0.1pp；该增益与单仓库可用任务量下的运行方差难以区分，但文档质量获得维护者认可。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过基准优化自动合成SKILL文档，但真实裸仓库没有基准，且先前构造的合成任务过小，能力强的代理即使没有文档也能饱和，无法体现文档价值。

Method: 从仓库历史中挖掘更难的任务：取已在单一冻结基提交上被回滚的已合并PR；用"同一代理有无该文档时表现是否更好"来为候选文档打分；并在三个Kotlin仓库上对比GEPA与SkillOpt两种优化器生成的文档。

Result: GEPA生成的文档将该分数平均提升4.9pp，SkillOpt生成的文档基本无提升（仅高于种子0.1pp）；GEPA增益与先前工作报告的同一优化器结果相当，但在单仓库所能提供的数据规模下，该增益与代理逐次运行的方差无法区分。文档内容质量优于分数所反映的水平，某仓库维护者在其中发现了只有深入参与项目才能获得的知识。

Conclusion: 以裸仓库的困难任务和无文档对照来评估SKILL文档更贴近真实场景，但单个仓库的历史任务量不足以将优化增益与运行方差分离，可能需跨更多任务才能定论；文档的实际实用价值可能被当前指标低估。
...
### [162] [What Drives Recovery in Agentic Text-to-Cypher? LAST-CQ: An LLM Agent Self-Refinement Framework](https://arxiv.org/abs/2609.12746)
*Ioannis Prokopiou,Athanasios Aidinis,Panagiotis-Christos Kyrmpatsos,Pantelis Vikatos*

Main category: cs.AI

TL;DR: 本文以LAST-CQ（五智能体、免训练、执行接地的Text-to-Cypher框架）为工具化试验台，通过三组反事实实验（2471条真实数据库查询、六个骨干模型、三个厂商规模层级）剖析智能体流水线增益来源：增益核心在于"检测失败并路由至重试"，而非反馈精细度或采样数量。


<details>
  <summary>Details</summary>
Motivation: 智能体式结构化查询生成流水线快速扩张，但尚不清楚循环中哪一部分真正带来增益；需要拆解各组件贡献，避免把资源投入到无效环节。

Method: 以LAST-CQ为工具化试验台，设计三组反事实对照：(1)移除纠错模块；(2)用原始数据库错误串替代模式接地的LLM合成反馈；(3)将同等调用预算改为并行采样。评估覆盖2471条真实数据库查询、6个骨干模型、3个厂商规模层级；指标包括aggregate execution-BLEU、naive/set-F1精确匹配、set-F1双侧单边检验；并将LLM评判与盲标人类标签进行校准。

Result: 移除纠错相对单次生成损失3.1% aggregate execution-BLEU，相对无精炼反事实损失12.3%（最弱骨干最高达80.7%）。用原始错误串替换合成反馈几乎无代价（20.9% vs 19.9% naive exact match；端到端<0.2%；set-F1在±0.075内等效）。同等预算用于并行采样反而使质量下降10-11%。LAST-CQ可挽救91.7%单次生成失败的查询，且首次成功的查询仍仅消耗一次LLM调用。此外，序列化结果上的n-gram重叠不是任一方向的界：在65.9%结果上相对集合等价偏高打分，而相对判定语义偏低打分。LLM评判经盲标人类标签校准后偏高9个百分点。

Conclusion: 智能体流水线的关键增益来自失败检测与重试路由机制，反馈的复杂度和采样数量并非决定因素；同时，n-gram重叠指标与LLM评判均存在系统性偏差，需谨慎使用并加以校准。
...
### [163] [Assisted Spatial Cognition Through Vision-Language Models](https://arxiv.org/abs/2609.12747)
*H. Riaz,J. B. Fernandez,I. Mills,D. Hickey,F. Cleary,M. I. Ali*

Main category: cs.AI

TL;DR: 本文提出一个端到端框架，融合LLM、VLM与数字孪生技术，通过手机摄像头视频、SLAM3R密集点云生成、多视角对齐后处理及SpatialLM结构化3D表示，为视障与神经多样性用户提供空间认知导航支持，实验验证了3D场景理解与目标定位的稳定准确性。


<details>
  <summary>Details</summary>
Motivation: 全球超过4300万视障与神经多样性人群在室内外环境中因空间感知有限、环境线索不足而面临持续导航困难；现有导航辅助工具缺乏全面的3D场景理解，多依赖受限的路径式策略，削弱了用户自主性。

Method: 提出集成LLM、VLM与数字孪生技术的端到端框架：用普通手机摄像头采集视频；采用SLAM3R从单目RGB序列实时生成密集3D点云；自研后处理算法在无需预定义参考点的情况下实现多视角点云精确对齐；增强SpatialLM以生成包含建筑元素与有向目标包围盒的结构化3D表示；再由本地部署的LLM处理丰富空间数据，解读3D语境，生成详细场景描述及用户与周边物体的精确距离测量。

Result: 在涵盖多种视角、循环行走视图及多环境的多样视频场景中评估，结果显示3D场景解读与目标定位具有一致的准确性。

Conclusion: 该系统将先进视觉感知与空间推理相结合，有望成为一种变革性的辅助导航解决方案。
...
### [164] [SCQ: Stabilizing Conservative Q-Learning with Sigmoid-Bounded Entropy](https://arxiv.org/abs/2609.12749)
*Xiefeng Wu,Shu Zhang,Zhaojie Chu,Mingyu Hu*

Main category: cs.AI

TL;DR: 本文提出SCQ方法，用sigmoid有界熵项替换可变为负的对数熵项，缓解离线到在线强化学习的值估计不稳定问题，在D4RL、视觉及多种真实机器人平台上取得稳定且不逊于基线的性能。


<details>
  <summary>Details</summary>
Motivation: 离线到在线强化学习虽能降低真实机器人学习的交互成本，但存在持续的值估计不稳定问题。现有方法多通过悲观正则化、下界校准和架构归一化来应对，然而被忽视的不稳定来源在于熵公式：标准对数熵项可能变为负值，从而使策略更新不稳定。

Method: 提出SCQ（Sigmoid-Bounded Conservative Q-Learning）：用严格保持为正的sigmoid有界公式替换标准对数熵项，同时保留保守Q正则化与基于回报的下界校准，从而在不牺牲探索的前提下稳定策略优化。并通过直接截断负对数概率贡献的干预实验以及梯度匹配的正分数对照进行机理验证。

Result: 在D4RL（Minari）基准上，涵盖单示范与标准数据集两种设置，以及仿真和真实世界视觉任务中评估。SCQ匹配或超越基线性能，在基于状态和视觉的基准上训练动态更稳定，并迁移至四类真实机器人平台（操作、轮式、四足、人形）。截断干预与梯度匹配对照表明，正性本身而非特定分数形状是性能提升的主要驱动因素。

Conclusion: SCQ通过sigmoid有界、严格为正的熵公式，稳定了离线到在线强化学习的策略优化，兼具性能与稳定性，并具备跨仿真、视觉及多种真实机器人平台的迁移能力；正性（而非特定评分形状）是改进的关键因素。
...
### [165] [Unified Agentic Video Editing Across Levels of Complexity and Creativity](https://arxiv.org/abs/2609.12769)
*Surabhi S. Nath,Kim Ferres,Milan Petrović,Lion Schulz*

Main category: cs.AI

TL;DR: 提出面向自动视频剪辑的智能体工具方法，覆盖场景预览、视频摘要与电影预告片三类任务，并对输出进行评估，讨论自动化与自主性的影响。


<details>
  <summary>Details</summary>
Motivation: 视频剪辑是视频制作的核心环节，需要在多重约束下进行创意策划与决策，因此有必要探索智能体工具在自动剪辑中的应用。

Method: 针对三种编辑目标、复杂度与创意程度不同的任务（场景预览、视频摘要、电影预告片），设计智能体工具化方法并生成剪辑输出。

Result: 对三类任务的生成输出进行了评估（摘要未给出具体量化结果）。

Conclusion: 讨论了自动视频剪辑对自动化与人机自主性（agency）的意义与影响。
...
### [166] [MPT: Missing Prototype Tracking via Barycentric Reconstruction in Vehicular Federated Learning](https://arxiv.org/abs/2609.12771)
*Hanju Jang,Gyeongmin Han,Sungmin Lee,Kichang Lee,Chunghan Lee,JeongGil Ko*

Main category: cs.AI

TL;DR: 本文提出MPT，一种跨车辆联邦学习框架，通过每轮从隐私保护的类级统计量重建稀有类原型，在车辆中途退出导致目标类样本稀少时维持其识别能力。


<details>
  <summary>Details</summary>
Motivation: 跨车辆联邦学习可保护本地驾驶数据隐私并协同提升感知模型，但车辆参与具有瞬时性，可能在训练收敛前退出并永久带走本地数据。当退出车辆持有目标类的大部分样本时，该类在剩余FL网络中变得稀有，其识别性能会随共享主干继续演化而悄然退化。恢复该类十分困难：剩余样本稀少使原型估计含噪，而FL隐私约束又禁止集中访问原始数据或逐样本特征。

Method: MPT在每轮从隐私保护的类级统计量重建稀有类原型，包含三个组件：重心分解（barycentric decomposition），用于追踪与剩余类原型共享的漂移；基于协方差的残差预测（covariance-based residual prediction），用于估计跨度外（out-of-span）漂移；自适应校准（adaptive calibration），依据可靠性对剩余稀有类样本进行加权。

Result: 在三个车辆分类任务和四种骨干网络上，与代表性校准和漂移补偿基线相比，MPT在稀有类F1上优于所有基线，在nuImages数据集上仅剩1%稀有类样本时达到0.516，且无需原始数据、逐样本特征或重训练。

Conclusion: MPT通过从类级统计量重建稀有类原型，有效缓解了车辆退出导致的稀有类识别退化问题，在严格隐私约束下实现了优越的稀有类识别性能，无需访问原始数据、逐样本特征或进行重训练。
...
### [167] [Interpreting the predictions of neural network classification based on a Taylor Coefficient Analysis (TCA)](https://arxiv.org/abs/2609.12801)
*Markus Klute,Artur Monsch,Lars Sowa,Roger Wolf*

Main category: cs.AI

TL;DR: 提出Taylor系数分析(TCA)框架，基于对神经网络预测的Taylor展开，系统刻画输入特征空间X对预测结果的贡献。


<details>
  <summary>Details</summary>
Motivation: 神经网络在事件分类中缺乏可解释性，需要一种严格且系统的方法来理解输入特征X如何影响预测\hat{y}。

Method: 对神经网络输出\hat{y}关于输入X做Taylor展开，构建完整的内省(introspection)流程，即TCA；并在两个简单基准任务及一个CERN LHC典型分类任务上逐级演示。

Result: 在两个简单任务上展示了TCA揭示X中哪些属性导致特定\hat{y}值的效力；在复杂LHC任务上验证了不同层级内省的实际应用，并讨论了实现中的实践问题；进一步研究表明TCA到二阶即可捕捉对LHC复杂度任务最相关的X属性。

Conclusion: TCA是一种可行且有效的神经网络可解释性方法，对CERN LHC典型分类任务，其二阶展开已足够捕获关键特征贡献。
...
### [168] [K-Bench: A Benchmark for LLM Unlearning in Agentic Deployments](https://arxiv.org/abs/2609.12808)
*Guangsheng Yu,Yanna Jiang,Qin Wang,Baihe Ma,Xu Wang*

Main category: cs.AI

TL;DR: 论文提出 K-Bench，从智能体部署视角评估 LLM 遗忘效果，揭示现有基准（如 TOFU、MUSE）仅凭最终答案通道认证遗忘的局限，并在智能体六个暴露通道中检测秘密泄露。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘基准（TOFU、MUSE）通过读取模型最终答案来认证遗忘，只要模型拒答即视为已遗忘。但这种模型级认证在模型作为智能体部署后不再成立——拒答不等于秘密不可恢复，因此需要面向智能体部署的遗忘评测方法。

Method: 提出 K-Bench 基准：检查 ReAct 智能体暴露的全部六个通道，包括思维链（CoT）、工具调用、工具观测和诱导总结等；秘密出现在任一通道即判为泄露。每次实验将秘密置于智能体的三个来源之一（权重、提示、检索库）中。对每个来源分别计算 K-Score，且仅在智能体仍可用时计入遗忘。评估了二十种已发表方法。

Result: 清除答案通道并不能使秘密不可恢复。在结构化检索中，秘密逐字保留在工具观测通道，聚合泄露率不变。当秘密位于提示或检索库时，TOFU 与 MUSE 报告无泄露，而部署的智能体仍在 22%–86% 的查询中泄露。当秘密位于权重时，二十种已发表方法无一能确证移除，仅输入损坏干预在评估观察者下达到选择性遗忘。排名第一的方法随基础模型变化。某拒答微调方法在被评估的提取下有效，但未验证知识移除。

Conclusion: 模型级的遗忘认证无法迁移到智能体部署场景，拒答不等同于遗忘。遗忘评估必须覆盖智能体的全部暴露通道，K-Bench 提供了这一评测框架，并表明现有方法在智能体部署下普遍失效。
...
### [169] [Scaling Clinical Judgment to Evaluate Medical AI](https://arxiv.org/abs/2609.12822)
...
TL;DR: 提出 PrecepTron——基于 LoRA 微调的 320 亿参数 LLM 医师级评估器，并发布 GRAND-ROUNDS 医师标注基准，实现可扩展、可复现的临床推理评估。


<details>
  <summary>Details</summary>
Motivation: 盲法医师评估被视为评估大语言模型临床推理的金标准，但难以规模化，既往研究多依赖单一机构或专科的小型医师评审团，既限制了研究问题，也无法确认结论换一批评估者是否可复现。

Method: 对 320 亿参数模型使用低秩适配（LoRA），在少量医师标注示例上微调，得到面向开放式回答的医师级评估模型 PrecepTron；同时构建 GRAND-ROUNDS 基准，包含 7 项研究、11 位医师给出的 9,217 条评分。

Result: 前沿 LLM 的常规"LLM-as-a-judge"评分常与医师不一致，且模型间彼此分歧；少量样本微调后的 PrecepTron 能在多任务上实现医师级一致评分，并在无新增人工评分的情况下复现 JAMA、Science、Nature Medicine 五项重要研究的核心结论；还可开展逐块乃至逐 token 提供病例时的诊断准确性测量等新问题。

Conclusion: PrecepTron 与 GRAND-ROUNDS 为可复现、大规模研究 LLM 医学推理提供了基础，代码、数据与标注均免费开放。
...
### [170] [MedRoundsQA: A Persona and Difficulty Aware Evaluation for Multi-Turn Medical Consultations](https://arxiv.org/abs/2609.12851)
...
TL;DR: 本文提出 MedRoundsQA——一个基于 1,387 个 17 专科医学考题构建的多轮诊断基准。病例被转化为结构化 24 槽临床记录，并在固定临床内容的前提下实例化为受控医患双智能体对话，配以不同患者人格；同时基于模型不确定性划分难度。评测 15 个 LLM 医生智能体发现：从单轮诊断转为多轮问诊导致约 13–39 分大幅下降；增加轮次可提升提问相关性，但诊断准确率收益递减，通常 6–12 轮后趋于平台；不同患者人格可使诊断准确率相差约 7–8 分，暴露了单轮基准无法发现的公平性风险。


<details>
  <summary>Details</summary>
Motivation: 现有医学基准以单轮、多选临床病例为主，难以反映真实问诊过程。真实临床中医生需交互式获取证据，且患者沟通方式差异很大，因此需要能刻画多轮交互与患者沟通差异的诊断评测基准。

Method: 提出 MedRoundsQA：从 17 个专科的 1,387 个医学考题病例出发，将每个病例转换为结构化 24 槽临床记录；在保持临床内容不变的前提下，用受控的医患双智能体对话实例化，并设定不同患者人格；利用基于模型的不确定性对病例进行难度分类，以支持由易到难分析；评测 15 个 LLM 医生智能体。

Result: 评测 15 个 LLM 医生智能体显示：(i) 从结构化记录上的单轮诊断转为多轮问诊，性能大幅下降约 13–39 分；(ii) 增加轮次能可靠提升提问相关性，但诊断准确率收益递减，通常在 6–12 轮后达到平台；(iii) 患者人格差异可使诊断准确率变化约 7–8 分（最低到最高教育程度），揭示单轮基准遗漏的公平性风险。

Conclusion: MedRoundsQA 表明多轮交互和患者人格会显著影响 LLM 诊断表现，单轮多选题基准会系统性高估能力并忽视公平性问题；构建受控多轮医患对话基准对更真实地评估医学 LLM 十分必要。
...
### [171] [Tracing and Coordinating Cross-Layer Influence for Multimodal Model Merging](https://arxiv.org/abs/2609.12897)
...
TL;DR: 提出TAC-Merge，通过跨层影响追踪与协调实现多模态模型合并，包含多模态影响映射与耦合合并控制两模块，在多种多模态任务上有效整合专家能力并泛化到未见任务。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型合并方法大多沿用单模态逐层合并专家更新的思路，但专家的更新会改变后续层输入，其影响沿深度传播并影响视觉与文本信息的交互；视觉与语言更新组合时，后续更新作用于已被前序更新修改的输入，导致效果相互耦合。由此带来两个挑战：(1)如何刻画单个专家更新跨深度的多模态影响；(2)如何基于其多模态影响联合合并专家更新。

Method: TAC-Merge包含两个模块：多模态影响映射(MIM)与耦合合并控制(CMC)。MIM构建更新效应图，结合Ricci曲率与专家预测定义共享融合目标；CMC建模系数调整间的交互，联合优化区域权重以合成单一共享模型。

Result: 在多样化多模态任务上的实验表明，TAC-Merge能有效整合互补的专家能力，并支持对未见任务的泛化。

Conclusion: 通过追踪与协调跨层多模态影响来合并专家，可缓解逐层合并造成的效果耦合问题，实现互补能力的整合与未见任务泛化。
...
### [172] [EduFair-Bench: Evaluating Pedagogical Fairness of LLM Tutors Across Student Demographics](https://arxiv.org/abs/2609.12949)
*Jiaxu Zhao,Bahar Radmehr,Fares Fawzi,Tanya Nazaretsky,Tanja Käser*

Main category: cs.AI

TL;DR: 提出 EduFair-Bench 基准，通过受控模拟审计 LLM 辅导者的教学公平性，发现模型能力与人口统计公平性基本正交，且偏见具有维度与训练相关性。


<details>
  <summary>Details</summary>
Motivation: LLM 正被广泛用作辅导者（tutor），但其是否对不同学生群体提供同等质量的教学尚不明确，缺乏系统性审计工具与证据。

Method: 构建 EduFair-Bench：将多领域题库（数学、物理、化学）与受控模拟结合，固定 LLM 学生与各辅导者互动，覆盖性别、移民背景、第一语言、社会经济地位四个维度共九个人口统计层级；用五项轮次级教学指标与四项会话级维度评分，评分由 LLM 评判器完成并在 180 个辅导轮次上以三名标注者共识进行验证；采用配对 Wilcoxon 符号秩检验与 bootstrap 效应量置信区间度量偏见；并设置两组消融（以姓名传递人口线索、辅导者与学生人口信息冲突）以区分辅导者驱动与学生驱动的偏见。

Result: 在五个辅导模型上：模型能力与人口统计公平性大体正交；最小模型表现最一致，而四个更强模型均存在较大人口统计差距，且无明确的能力-公平性排序；面向教学法的强化学习训练只是重新分配而非消除偏见；语言与移民相关线索造成的差距大于性别与社会经济地位相关线索。

Conclusion: LLM 辅导者的教学公平性不能被能力提升自动解决，需要专门审计与针对性干预；EduFair-Bench 提供了可复用的公平性评估框架，并揭示偏见在不同人口维度和训练方式下呈现差异化分布。
...
### [173] [How Good Are Frontier Models at Physics? Expert Re-Grading Reveals Broken Evaluations and Near-Saturation of Leading Benchmarks](https://arxiv.org/abs/2609.13009)
...
TL;DR: 前端语言模型在物理基准上的低分主要源于基准本身缺陷（错参考解、评分错误、题目歧义），经专家修正后得分大幅提升，说明现有基准严重低估了模型的物理推理能力。


<details>
  <summary>Details</summary>
Motivation: 领先物理基准（含 Artificial Analysis Intelligence Index 2026）报告的低分似乎表明前端语言模型仍难以处理高级物理问题，但这一印象与领域专家在实际使用中的体验不符，因此需要重新审视这些评估结果的可信度。

Method: 选取六个广泛使用的物理基准，聚焦有可验证最终答案的纯文本问题，由各物理子领域具备相关专长的教师与研究生逐题审查题干、参考解与模型回答，区分真实模型错误与评分错误、错误参考解、歧义或欠定问题；随后请专家修正错误参考解并修复或剔除有缺陷的题目，在保留子集上重算得分。

Result: GPT-5.6-Sol 在 HLE-Physics 上的 mean@4 由 47.3% 升至 78.7%，在 CMT-Benchmark 上由 61.0% 升至 87.2%；在保留的 54 道 CritPt 挑战上修正后 pass@4 达 94.4%。UGPhysics、PRISM-Physics 和 PHYBench 的受审子集得分在修正后也显著上升。多数最初判为错误的案例实为基准问题而非模型物理推理错误。

Conclusion: 现有基准显著低估了前端模型解决良定义物理问题的能力；这些封闭式任务已接近饱和，亟需更严格、经专家验证的评估体系。
...
### [174] [Diffusion Models and Concept Formation](https://arxiv.org/abs/2609.13047)
*Zekun Wang,Karthik Singaravadivelan,Christopher J. MacLellan*

Main category: cs.AI

TL;DR: 本文将认知模型Cobweb与扩散模型建立对应关系：扩散模型的噪声边缘分布是对数据分布的高斯平滑，其众数形成的层级结构在四个层面等价于Cobweb的概率原型树，并在MNIST与Fashion-MNIST上通过众数搜索验证了二者基本层次的对应。


<details>
  <summary>Details</summary>
Motivation: 人类以嵌套抽象层级组织概念，并在"基本层次"上以最小认知代价识别和命名物体；Cobweb是这一能力的经典认知解释。作者希望说明，为图像合成设计的扩散模型其实隐含地执行了同样的计算，从而把扩散重构为概念形成的认知模型，同时为Cobweb提供连续、可扩展的实现。

Method: 从理论上论证扩散模型的噪声边缘分布（noisy marginals）是数据分布的高斯平滑，其众数构成层级，并在四方面对应Cobweb的层级概率原型树：二者都是层级密度模型、都是带高斯原型（prototype）的层级贝叶斯模型、都把归类视为降低不确定性的"分数跟随"（score-following）、并且都涌现出基本层次。将扩散模型的基本层次定位于某一中间噪声水平，即已有分析表明逆向过程开始确定样本类别身份的位置。实验在MNIST与Fashion-MNIST上，通过众数搜索（mode-finding）恢复扩散层级，并比较两模型的基本层次。两者的主要差异在于分类体系的表示与学习方式：Cobweb增量学习离散树，扩散模型则在单一学习到的分数场（score field）中编码连续、可插值的层级。

Result: 理论上确立了扩散模型噪声边缘的众数层级与Cobweb树之间的四重对应；实验上在MNIST与Fashion-MNIST上成功通过众数搜索恢复扩散层级，并对两个模型的基本层次进行了比较验证。

Conclusion: 扩散模型可被视为概念形成的认知模型，而Cobweb由此获得一种连续、可扩展的实例化形式；二者共享层级密度建模与基本层次涌现的机制，仅在分类体系的表示与学习方式上有所不同。
...
### [175] [Anchoring Clinical Events in Time: UID-Preserving Multimodal Reconstruction and Source-Grounded Adjudication](https://arxiv.org/abs/2609.13062)
*Sayantan Kumar,Nicolas Grimaldi,Jack Cummins,Jeremy C. Weiss*

Main category: cs.AI

TL;DR: 该论文提出一种保留UID的临床时间线构建框架，将叙事事件与其来源文本片段及结构化记录行关联，并引入GAVEL评测器。实验表明多模态GLM 5.2修订版在时间一致性上优于纯文本版本，事件恢复率提升43%，性能可与临床医生标注竞争。


<details>
  <summary>Details</summary>
Motivation: 出院小结常模糊临床事件的时间顺序，结构化EHR表仅覆盖患者病程的一部分，二者均难以支持治疗时间窗分析与无泄漏建模，缺乏事件级可溯源的时间线表示。

Method: 提出保留UID的框架：将每个叙事事件出现与其来源span绑定，并在纯文本估计、结构化证据检索、带时间戳的源行接地及联合修订过程中保持该身份；同时提出LLM评判器GAVEL，针对叙事与结构化记录比较两条UID对齐时间线，以增强事件匹配与时间评估。在6个开放权重模型、40份混合重症监护小结上评测，并进行消融与盲法人工复核。

Result: 多模态GLM 5.2修订版相较纯文本变体在不降低事件恢复的前提下提升时间一致性，与临床医生标注竞争力相当，其他模型修订增益较小、整体性能较低。消融显示UID主要保障事件保留，源行链接支撑时间定位。盲法人工复核支持多数GAVEL结论；受控裁决中多模态GLM 5.2优于纯文本版，DeepSeek V3.2则未体现该优势。整体实现事件恢复率提升43%。

Conclusion: UID与评判器流水线可提供事件级溯源，同时服务于时间线重建与评估，兼顾性能提升与临床可解释性。
...
### [176] [Autonomous Research for Open-Ended Problems: A Case Study on Telecom Ticket Retrieval](https://arxiv.org/abs/2609.13073)
*Junghyun Min,Huseyin Uzunalioglu,Mohamed Trabelsi*

Main category: cs.AI

TL;DR: 本文以电信工单检索（telecom ticket retrieval）为案例，探究自主研究框架能否解决开放式、工业级ML问题。实验表明：自主研究在窄搜索空间（如超参优化）中表现出色，但缺乏人类直觉与创造力，且需要运维开销；在极少人工监督下，它可在10周内以单次活动最高200美元的成本达到SOTA性能的90%（Recall@1为0.34 vs 0.38）。作者建议人类研究者与自主研究框架协同工作。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的系统在问题求解和编程上的进展推动了AI for Science范式，理论上可替代人类在机器学习研究中的部分角色。但已有全自主端到端ML研究框架的成功实现多局限于搜索空间较窄的问题（如语言建模或生物医学ML基准），尚不清楚其能否应对开放式、工业级ML问题。因此作者希望探究自主研究如何适配这类开放性问题。

Method: 选取电信工单检索作为开放式、工业级ML案例，该任务在表示、架构和训练数据生成上均具有自由度。使用商用与开源智能体（agents）开展自主研究实验，并与人类研究过程进行对比，评估其在窄超参数优化与开放性问题上的表现差异、对人工监督与运维开销的需求，以及时间与成本效率。

Result: 自主研究在窄超参数优化方面表现优异，但在开放式问题上仍显不足：缺乏类人的直觉与创造力，并需要额外的运维开销。在极少人工监督下，自主研究达到SOTA性能的90%（Recall@1：0.34 vs. 0.38），耗时10周，而人类工作需10个月；单次Cursor活动成本最高约200美元。

Conclusion: 自主研究在开放式、工业级ML问题上兼具潜力与局限，可作为高效低成本的研究助力，但难以完全替代人类研究者的直觉与创造力。作者据此建议人类研究者与自主研究框架协同配合，以获得ML研究的最佳效果。
...
### [177] [Embodied-BenchForge: A Closed-Loop Agentic Workflow for Embodied Benchmark Construction](https://arxiv.org/abs/2609.13082)
...
TL;DR: 提出 Embodied-BenchForge 智能体框架，将具身基准构建形式化为闭环合成，通过技能编排合成与依赖图验证修复，构建多场景离线与交互基准并验证其质量与有效性。


<details>
  <summary>Details</summary>
Motivation: 现有智能体化基准构建方法仅覆盖孤立阶段或局限于预定义环境与任务族；且多步构建产生的依赖中间产物常缺乏针对特定产物的验证，导致局部缺陷传播至最终基准。

Method: 将构建形式化为闭环基准合成，融合前向产物合成与后向验证修复。技能编排产物合成将类型化可复用技能组合为可执行工作流，并用产物依赖图记录中间输出及依赖关系；需求引导验证与修复在构建全程应用产物特定契约，验证失败时基于溯源触发局部重执行或上游回滚。

Result: 在离线 EQA 赛道构建覆盖多样具身场景的六个基准，并在交互具身赛道构建含 220 个可执行任务的交互基准。对代表性 MLLM 与具身智能体的评测显示，基准能区分模型在观察理解与闭环执行上的能力；质量评估与消融验证了基准质量和验证修复机制的有效性；修复与技能复用分析表明可高效局部恢复并具备跨基准复用性。

Conclusion: Embodied-BenchForge 以闭环合成、产物依赖图与契约化验证修复解决了中间产物缺陷传播问题，实现可复用、可恢复的具身基准自动化构建，并具备良好区分度与可复用性。
...
### [178] [CMA-OT: Hierarchical Expert Supervision for Dance-to-Music Generation](https://arxiv.org/abs/2609.13118)
*Jinting Wang,Chenxing Li,Dong Yu,Li Liu*

Main category: cs.AI

TL;DR: 本文提出CMA-OT范式，利用外部音乐专家对生成器隐特征进行分层监督，在舞蹈到音乐生成任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 舞蹈到音乐生成中，舞蹈线索（节奏、风格）稀疏，而音乐创作所需信息（结构、配器、表现力动态）密集，二者存在语义鸿沟。现有方法仅依赖稀疏线索且只监督最终音频输出，导致音乐表征学习不充分，生成音乐的音乐性与结构连贯性受限。

Method: 提出课程引导的多尺度表征对齐与尺度感知最优传输（CMA-OT）：1）引入外部音乐专家为生成器隐特征提供分层监督，弥合语义鸿沟；2）课程引导的多尺度学习策略，将音乐知识由专家渐进迁移至生成器；3）尺度感知最优传输对齐机制，在不同专家尺度间建模软对应关系，应对时间错配下的细粒度对齐。

Result: 在两个数据集上的大量实验表明，CMA-OT在节奏同步、感知质量和整体音乐生成方面均达到最先进性能。

Conclusion: 通过分层专家监督、课程式多尺度学习与尺度感知最优传输对齐，CMA-OT有效提升舞蹈到音乐生成的表征学习质量与音乐性表现。
...
### [179] [A Hybrid LSTM-XGBoost Framework for Multi-Horizon Stock Return Prediction Across Diversified Equity Portfolios](https://arxiv.org/abs/2609.13125)
*Seif ElDein Mostafa,Yahia Ahmed,Farah Datwish,Marwa Solayman*

Main category: cs.AI

TL;DR: 本文提出一种两阶段混合架构，将LSTM提取的时序嵌入与手工技术指标拼接后输入XGBoost，用于14只美股在30/90/252/365日多期限收益率预测；30日RMSE为0.0949，方向准确率随期限上升至97.6%，但作者指出该指标主要由正向收益基准率驱动，故以超越"恒定看多"基准的差距作为更有效的信号。


<details>
  <summary>Details</summary>
Motivation: 股票收益率预测因金融时间序列的非平稳性、非线性与低信噪比而极具挑战。单一模型难以同时刻画时序动态与非线性特征交互：LSTM擅长序列建模但易过拟合、泛化受限，XGBoost擅长表格型特征但无法直接利用原始序列结构。作者希望结合两者的互补优势，并在缺乏可靠评估基准的情况下，给出对方向准确率更审慎的解读。

Method: 采用两阶段混合架构：第一阶段用两层堆叠、64个隐藏单元的LSTM处理5维市场特征的60日滑动窗口，输出64维时序嵌入；将嵌入与14个手工技术指标拼接为78维混合特征向量；第二阶段输入经3折交叉验证网格搜索调参的XGBoost回归器。模型在14只股票（覆盖6个行业板块）的合并语料上训练，采用严格按时间顺序划分与逐股MinMaxScaling以避免前视偏差，并在30、90、252、365个交易日四个期限上评估。此外构建由多期限预测衍生的复合投资评分框架用于组合排序与决策支持。

Result: 30日期限上混合模型测试RMSE为0.0949，约为单独LSTM基线的三分之一，并在多数股票上略优于或持平于XGBoost-Only基线。方向准确率随预测期限延长而提升，365日达97.6%；但作者进一步表明该现象主要跟随样本中长期限正收益的高基准率，因此引入朴素"恒定看多"预测器作为对照，并将短期限上超越该基准率的差距视为更具信息量的信号。

Conclusion: LSTM时序嵌入与XGBoost非线性回归的混合架构在多期限股票收益预测中有效降低了预测误差，并在多数标的上一并优于或持平于单模型基线。研究同时强调：方向准确率在高期限上的高值具有误导性，须以朴素基准对照加以校正，短期限的超基准差距才是可靠指标。由多期限预测派生的复合投资评分框架可服务于组合排序与决策支持，但受限于样本与低信噪比特性，结论推广需谨慎。
...
### [180] [Rethinking Heterogeneous System Disaggregation for Subquadratic Attention](https://arxiv.org/abs/2609.13134)
...
TL;DR: 本文提出 SQD（SubQuadratic Disaggregation），一种针对次二次注意力 LLM 的细粒度异构解耦推理方案，按二次/次二次注意力而非算子类型拆分 decode，在 DRAM/SRAM 异构系统上显著提升吞吐与能效。


<details>
  <summary>Details</summary>
Motivation: 前沿模型为降低推理时的显存占用与算力需求，正更激进地采用次二次注意力，但仍保持前沿精度。现有系统采用以稠密注意力为中心的解耦服务决策，未针对次二次注意力的算术强度与显存占用特征进行优化，因此无法充分利用新兴 DRAM 与纯 SRAM 异构系统的潜力。

Method: SQD 将 decode 按二次注意力与次二次注意力拆分，而非按算子类型拆分，并可适用于多种次二次注意力变体：对稀疏注意力 LLM，将 decode 拆为 top-k 选择（需遍历完整 KV）与 top-k 注意力加 FFN（显存占用静态）；对线性注意力和滑窗注意力 LLM，将 decode 拆为稠密注意力层与次二次注意力层加 FFN。

Result: 在调整后的 8xB200 异构系统代理上，相较最强纯 GPU 基线，平均 tokens/J 提升为 GLM 5.2 上 53%、Nemotron 3 Ultra 上 31%、Gemma 4 31B 上 56%。在固定功耗预算的 Rubin 加 LPX 系统解析模型中，相较最优的注意力-FFN 解耦基线，可达延迟收紧 1.2x–1.5x，吞吐最高提升 3.6x。

Conclusion: 围绕次二次注意力的算术强度与显存特征进行异构解耦，可在新兴 DRAM 与 SRAM 异构系统上带来显著的吞吐与能效收益；实验还揭示了面向次二次注意力服务的新一代异构系统在芯片与互连配置方面的架构洞见。
...
### [181] [Grid-Free Monte Carlo for Time-Dependent Diffusion](https://arxiv.org/abs/2609.12306)
*Zihong Zhou,Rohan Sawhney,Eugene d'Eon,Wojciech Jarosz*

Main category: cs.GR

TL;DR: 本文提出将无网格蒙特卡洛求解器WoS与WoSt从稳态问题推广到时变热方程，通过为随机游走设置有限时间预算并采样退出时间，实现对任意目标时刻解的估计，无需体网格与顺序时间步进。


<details>
  <summary>Details</summary>
Motivation: 扩散系统的瞬态演化建模在科学计算中需求广泛，但传统PDE稳态分析已受复杂几何体网格划分的瓶颈制约，瞬态分析还需顺序时间步进与步长选择。现有的无网格蒙特卡洛求解器（WoS、WoSt）虽能规避网格划分，却基本局限于稳态问题，因此需要将其推广到时变情形。

Method: 针对纯Dirichlet问题推广WoS、针对混合Dirichlet–Neumann问题推广WoSt，使其处理带初始条件及时间相关源项、边界数据的热方程。为每次随机游走分配有限时间预算，并在每个空间步采样退出时间：若退出时间超过剩余预算，则采样内点并评估初始条件；否则以缩减后的预算继续游走，并累计源项与边界贡献。技术贡献包括一套核采样与方差缩减技术，含低偏差、免查表的退出时间采样器与高效拒绝采样器。

Result: 该方法可直接估计任意给定时刻的解，无需体网格与顺序时间步进，同时保留了WoS/WoSt的并行、渐进式与输出敏感特性，并彻底消除了时间步长选择与时间离散偏差。此外，通过共享游走，可高效估计多个目标时刻的解。

Conclusion: 该工作将无网格蒙特卡洛PDE求解器成功扩展到瞬态热方程，在避免网格划分与时间离散的同时保持输出敏感的高效求解方式，并通过游走共享支持多时刻估计，为复杂几何上的瞬态扩散问题提供了新的求解途径。
...
### [182] [SNAP3D: Physically Grounded 3D Parts for Assembly from a Single Image](https://arxiv.org/abs/2609.13146)
*Yu-Rou Tuan,Hao-Tang Tsui,Nicolas Ugrinovic,Kris Kitani,Xiaoxuan Ma*

Main category: cs.GR

TL;DR: 提出一种物理引导框架，在单图像部件感知3D生成中解决部件穿插、恢复接触图并引入参数化连接件，通过物理仿真反馈优化连接件，实现物理可装配且稳定的装配体。


<details>
  <summary>Details</summary>
Motivation: 现有部件感知3D生成方法虽能生成视觉完整的单个部件，但无法保证其构成有效的物理装配体：相邻部件可能相互穿插、缺乏有效连接、或在重力下坍塌。这限制了编辑、铰接、仿真与制造等下游应用。

Method: 1) 消除部件间穿插；2) 恢复相邻部件间的接触图（contact graph）；3) 在接触面引入参数化连接件（connectors）；4) 利用物理仿真反馈优化连接件的位置、朝向与尺寸，在保持生成几何的前提下提升装配稳定性。

Result: 与多个部件感知3D生成器对比，在保持几何质量的同时，物理可实现性与稳定性显著提升；并通过3D打印与真实装配验证了生成部件的可用性。

Conclusion: 该工作将物理约束与仿真反馈引入部件感知3D生成流程，并提出基于物理的评估协议（直接测试装配有效性与重力下稳定性），弥补了传统几何指标的不足，推动了生成资产向可制造、可装配方向发展。
